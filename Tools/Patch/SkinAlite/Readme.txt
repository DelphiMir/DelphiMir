...
--------
  ...
  TsMonthCalendar,    TsMenuManager,    TsHintManager,   TsCalculator,
  TsEditorsManager,  TsControlsManager,  TsSkinManager, TsSkinProvider
  and some others components...
    Also for DB-aware components users added some DB-aware  components
  which  SStyle-engine  supports (TsDBEdit, TsDBComboBox, TsDBListBox,
  TsDBNavigator and others).

Installation Library
--------------------

  1.  Uninstall  previous  or  evaluation  version  of AlphaControls or
  SStyle  package  from  Delphi IDE. Remove or copy to other directory
  this   files   to   prevent   crossing   this  and  new  version  of
  AlphaControls.  Copy  files with new components to custom directory.
  This  directory  must  be registered in your system search path. And
  add,   (if  need)  AlphaControls  directory  in  (Tools->Environment
  Options->Library-> Library Path).

  2.   Use  "File\Open..."  menu  item  of  Delphi  IDE  to  open  the
  AlphaControls  run-time package RStyleDx.DPK (x - version of Delphi).
  In the "Package..." window click "Compile" button to compile run-time
  library.

  3.   Use  "File\Open..."  menu  item  of  Delphi  IDE  to  open  the
  AlphaControls design-time package DStyleD*.DPK. In the "Package..."
...
----------------------
  ...
  Link to compiled demo : http://www.alphaskins.com/sfiles/askindemo.zip
  Link to sources of the demo : http://www.alphaskins.com/sfiles/askindemo_s.zip
...
Programmer: Serge V. Goncharov and ArtTech software
Home page: http://www.alphaskins.com
           http://www.delphiskins.com
E-mail: support@alphaskins.com                 ICQ # 157064827