...
begin
  HumanGrid.Cells[0, 1] := 'Çø·Ö';
  HumanGrid.Cells[1, 1] := '½ÇÉ«Ãû';
  HumanGrid.Cells[2, 1] := 'µ±Ç°µØÍ¼';
  HumanGrid.Cells[3, 1] := '×ø±êX';
  HumanGrid.Cells[4, 1] := '×ø±êY';
  HumanGrid.Cells[5, 1] := '·½Ïò ';
  HumanGrid.Cells[6, 1] := 'Ö°Òµ';
  HumanGrid.Cells[7, 1] := 'ÐÔ±ð';
  HumanGrid.Cells[8, 1] := 'Í··¢ ';
  HumanGrid.Cells[9, 1] := '½ð±Ò';
  HumanGrid.Cells[10,1]:='ÓÎÏ·±Ò';
  HumanGrid.Cells[11,1]:='»ý·Ö';

  HumanGrid.Cells[0,3]:='Home';
  HumanGrid.Cells[1,3]:='HomeX';
  HumanGrid.Cells[2,3]:='HomeY';
  HumanGrid.Cells[3, 3] := 'µÈ¼¶';
  ...
  HumanGrid.Cells[7,5] := '²Ö¿âÃÜÂë';
  HumanGrid.Cells[8,5] := 'ÊÇ·ñÊÇÓ¢ÐÛ';
  HumanGrid.Cells[9,5] := 'PCµã';
  HumanGrid.Cells[10,5] := 'ÉÏ´ÎµÇÂ¼';
end;

procedure TFrmFDBViewer.sub_49A9DC();
begin
  BagItemGrid.Cells[0, 0] := 'Çø·Ö';
...
begin
  UseMagicGrid.Cells[0, 0] := '¼¼ÄÜID';
  UseMagicGrid.Cells[1, 0] := 'µÈ¼¶';
  UseMagicGrid.Cells[2, 0] := '¼ÛÖµ';
  UseMagicGrid.Cells[3, 0] := 'ÊìÁ·¶È';
end;
...
begin
  ...
  if not HumData.boIsHero then begin
    if HumData.sHeroChrName <> '' then begin
      HumanGrid.Cells[11, 5] := 'Ó¢ÐÛÃû×Ö';
      HumanGrid.Cells[11, 6] := HumData.sHeroChrName;
    end else begin
      HumanGrid.Cells[11, 5] := 'Ã»ÓÐÓ¢ÐÛ';
      HumanGrid.Cells[11, 6] := '';
    end;
  end else begin
    HumanGrid.Cells[11, 5] := 'Ó¢ÐÛ×ÊÖÊ';
...
begin
  ...
  ShowBagItem(1,'ÒÂ·þ',ChrRecord.Data.HumItems[0]);
  ShowBagItem(2,'ÎäÆ÷',ChrRecord.Data.HumItems[1]);
  ShowBagItem(3,'»ð¾æ',ChrRecord.Data.HumItems[2]);
  ShowBagItem(4,'ÏîÁ´',ChrRecord.Data.HumItems[3]);
  ShowBagItem(5,'Í·¿ø',ChrRecord.Data.HumItems[4]);
  ShowBagItem(6,'(×ó)ÊÖïí',ChrRecord.Data.HumItems[5]);
  ShowBagItem(7,'(ÓÒ)ÊÖïí',ChrRecord.Data.HumItems[6]);
  ShowBagItem(8,'(×ó)½äÖ¸',ChrRecord.Data.HumItems[7]);
  ShowBagItem(9,'(ÓÒ)½äÖ¸',ChrRecord.Data.HumItems[8]);
  ShowBagItem(10,'»¤·û',ChrRecord.Data.HumItems[9]);
  ShowBagItem(11,'Ñü´ø',ChrRecord.Data.HumItems[10]);
  ShowBagItem(12,'Ñ¥×Ó',ChrRecord.Data.HumItems[11]);
  ShowBagItem(13,'ÊØ»¤Ê¯',ChrRecord.Data.HumItems[12]);
