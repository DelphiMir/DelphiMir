...
//  end;
  ...
  private
    ...
    procedure ViewChr(var UserInfo: pTUserInfo);
    function RenewHum(sData: string; var UserInfo: pTUserInfo): Boolean;
    procedure OutOfConnect(const UserInfo: pTUserInfo);
    procedure NewChr(sData: String; var UserInfo: pTUserInfo);
    procedure QueryBonze(UserInfo: pTUserInfo);
  ...
  case Msg.Ident of
    ...
    CM_NEWCHR: begin
      if (GetTickCount - UserInfo.dwChrTick) > 1000 then begin
        ...
        OutMainMessage('[Hacker Attack] _NEWCHR ' + UserInfo.sAccount + '/' + UserInfo.sUserIPaddr + '  - (DF: Note this doesn''t really mean much!)');
      end;
    end;
    CM_QUERYROGUE: begin
         QueryBonze(UserInfo);
    ...
    CM_DELCHR: begin
      if (GetTickCount - UserInfo.dwChrTick) > 1000 then begin
        ...
        OutMainMessage('[Hacker Attack] _DELCHR ' + UserInfo.sAccount + '/' + UserInfo.sUserIPaddr);
      end;
    end;
    CM_SELCHR: begin
    ...
    CM_VIEWDELHUM: begin
      if (GetTIckCount - UserInfo.dwChrTick) > 200 then begin
        ...
        if (UserInfo.sAccount <> '') and
         ...
         OutMainMessage('[Hacker Attack] _ VIEWDELHUM ' + UserInfo.sUserIPaddr);
      end;
    end;
      ...
      if (GetTIckCount - UserInfo.dwChrTick) > 200 then begin
        ...
        if (UserInfo.sAccount <> '') and
          ...
          if RenewHum(s18, UserInfo) then
            ...
            OutMainMessage('[Hacker Attack] _ RENEWHUM ' + UserInfo.sUserIPaddr);
      end;
    end;
          ...
          if HumDataDB.OpenEx then begin
            for I:=0 to ChrList.Count -1 do begin
              ...
              if HumChrDB.GetBy(QuickID.nIndex,HumRecord) and not HumRecord.boDeleted then begin
                ...
                if (nIndex < 0) or (nChrCount >= 4) then Continue;
                if HumDataDB.Get(nIndex,ChrRecord) >= 0 then begin
                  ...
                  sLoginTime:= ForMatDateTime('mm岿dd老 hh矫mm盒', ChrRecord.Header.dCreateDate);
                  //DateTimeToStr(ChrRecord.Header.dCreateDate);
                  if HumRecord.boSelected then s40:=s40 + '*';
                    s40:=s40 + sChrName + '/' + sJob + '/' + sHair + '/' + sLevel + '/' + IntToStr(btSex) + '/'{ + sLoginTime + '/'};
                  Inc(nChrCount);
                end;
              end;
            end;
  ...
  try
    if HumChrDB.Open then begin
      ...
      if n10 >= 0 then begin
        ...
        if HumRecord.sAccount = UserInfo.sAccount then begin
          ...
          boCheck := HumChrDB.Update(n10,HumRecord);
        end;
      end;
    end;
    ...
    if HumDataDB.OpenEx then begin//20081224
      ...
      if n10 >= 0 then begin
        ...
        HumDataDB.Update(n10, HumanRCD);        //昏力
      end;
    end;
  finally
    HumChrDB.Close;
...
procedure TFrmUserSoc.ViewChr(var UserInfo: pTUserInfo);
var
  ChrList: TStringList;
...
begin
      ...
      try
        if HumDataDB.OpenEx then begin
          for I := 0 to ChrList.Count - 1 do begin
            QuickID := pTQuickID(ChrList.Objects[I]);
            if QuickID.boIsHero then Continue;
            if HumChrDB.GetBy(QuickID.nIndex, HumRecord) and HumRecord.boDeleted then begin
              sChrName := QuickID.sChrName;
              nIndex := HumDataDB.Index(sChrName);
  ...
  SendUserSocket(UserInfo.Socket,
    UserInfo.sConnID,
    EncodeMessage(MakeDefaultMsg(SM_DELHUM, nChrCount, 0, 0, 0)) + EncodeString(s40));
end;

function TFrmUserSoc.RenewHum(sData: string; var UserInfo: pTUserInfo): Boolean;
var
...
begin
  ...
  try
    if HumChrDB.Open then begin
      ...
      if HumRecord.boDeleted and (sAccount = UserInfo.sAccount) then begin
    ...
    if HumDataDB.OpenEx then begin//20081224
      ...
      if nIndex >= 0 then begin
        ...
        HumDataDB.Update(nIndex, HumanRCD);       //昏力  - 汗备
      end;
    end;
  finally
    HumChrDB.Close;
...
procedure TFrmUserSoc.QueryBonze(UserInfo: pTUserInfo);
var
  sDefMsg: TDefaultMessage;
begin
  if DBShare.nMakeBonze = 0 then begin
    sDefMsg := MakeDefaultMsg(SM_ROGUEDISABLED, 0, 0, 0, 0);
    //OutMainMessage('铰妨 积魂阂啊');
  end else begin
    sDefMsg := MakeDefaultMsg(SM_ROGUEENABLED, 0, 0, 0, 0);
    //OutMainMessage('铰妨 积埃啊瓷');
  end;

  SendUserSocket(UserInfo.Socket,UserInfo.sConnID, EncodeMessage(sDefMsg));
end;
...
begin
  ...
  if not (Length(sChrName) in [4..14])  then nCode:=0;
  ...
  if ncode = -1 then begin
    ...
    try
      if HumChrDB.Open then begin
        if HumChrDB.ChrCountOfAccount(sAccount) < 4 then begin
          ...
          if HumRecord.Header.sChrName <> '' then
            if not HumChrDB.Add(HumRecord) then nCode:=2;
        end else nCode:= 3;
        //if (Str_ToInt(sJob, 0) in [4]) then     //铰妨
        // nCode := 5;
      end;
    finally
      HumChrDB.Close;
...
initialization
  SVNRevision('$Id: UsrSoc.pas 595 2007-03-09 19:31:34Z damian $');
end.