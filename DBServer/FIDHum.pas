...
type
  TFrmIDHum=class(TForm)
    ...
    BtnUserIdSearch: TSpeedButton;
    BtnSelAllDel: TSpeedButton;
    CheckBox1: TCheckBox;
    CheckBox2: TCheckBox;
    CheckBox3: TCheckBox;
    ...
    procedure BtnUserIdSearchClick(Sender: TObject);
    procedure BtnSelAllDelClick(Sender: TObject);
  private
...
begin
  ...
  ChrGrid.Cells[0, 0] := 'Ê±¼ä';
  ChrGrid.Cells[1, 0] := '½ÇÉ«Ãû';
  ChrGrid.Cells[2, 0] := 'ÕËºÅ';
  ChrGrid.Cells[3, 0] := 'ÊÇ·ñÉ¾³ý';
  ChrGrid.Cells[4, 0] := 'É¾³ýÈÕÆÚ';
  ChrGrid.Cells[5, 0] := 'ÊýÁ¿';
  ChrGrid.Cells[6, 0] := 'Ñ¡ÔñID';
  ChrGrid.Cells[7, 0] := 'Ó¢ÐÛ';
end;
          ...
          for i:=0 to ChrList.Count -1 do begin
            ...
            if nIndex >= 0 then begin
              ...
              if CheckBox1.Checked then begin
                ...
                if not HumDBRecord.boIsHero then begin
                  if CbShowDelChr.Checked then RefChrGrid(nIndex,HumDBRecord)
                  else if not HumDBRecord.boDeleted then
                    RefChrGrid(nIndex,HumDBRecord);
                end;
...
procedure TFrmIDHum.BtnSelAllDelClick(Sender: TObject);
var
  sChrName:String;
  ChrList:TStringList;
  I,nIndex:Integer;
...
begin
  ...
  try
    if HumChrDB.OpenEx then begin
      if HumChrDB.FindByAll(ChrList) > 0 then begin
        for I:= 0 to ChrList.Count - 1 do begin
          nIndex:=Integer(ChrList.Objects[I]);
          if HumChrDB.GetBy(nIndex,HumDBRecord) then begin
            if CheckBox3.Checked and CheckBox2.Checked then begin   //À¯Àú ¿µ¿õ ¸ðµÎ
              if HumDBRecord.boDeleted then
              RefChrGrid(nIndex,HumDBRecord);
            end else begin
              if CheckBox3.Checked and (not CheckBox2.Checked) then begin     //¿µ¿õ Ã¼Å© ¾ÈÇß´Ù¸é.
                if HumDBRecord.boDeleted and (not HumDBRecord.boIsHero) then
                 RefChrGrid(nIndex,HumDBRecord);
              end  else begin
                if (not CheckBox3.Checked) and CheckBox2.Checked then begin   //À¯Àú Ã¼Å© ¾ÈÇß´Ù¸é.
                  if HumDBRecord.boDeleted and HumDBRecord.boIsHero then
                   RefChrGrid(nIndex,HumDBRecord);
                end;
              end;
            end;
...
begin
  ...
  if MessageDlg('ÄãÈ·¶¨ÒªÉ¾³ý½ÇÉ«"' + sChrName + '"Âð£¿', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
    try
...
begin
  sChrName:=EdChrName.Text;
  if sChrName = '' then exit;
  if MessageDlg('ÄãÈ·¶¨ÒªÉ¾³ýÑ¡ÖÐµÄ½ÇÉ« "' + sChrName + '" Âð£¿', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
    try
      if HumChrDB.Open then begin
...
begin
  sChrName:=EdChrName.Text;
  if sChrName = '' then exit;
  if MessageDlg('ÄãÈ·¶¨Òª»Ö¸´½ÇÉ« "' + sChrName + '" Âð£¿', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
    try
      if HumChrDB.Open then begin
        nIndex:=HumChrDB.Index(sChrName);
        HumChrDB.Get(nIndex,HumRecord);
        if not HumRecord.boIsHero then begin
          if HumRecord.boDeleted then begin
            HumRecord.boDeleted:=False;
            Inc(HumRecord.btCount);
            HumChrDB.Update(nIndex,HumRecord);
          end else begin
            ShowMessage('½ÇÉ«Ã»ÓÐ±»É¾³ý¡£');
          end;
        end else begin
          ShowMessage('Ó¢ÐÛÎÞ·¨»Ö¸´¡£');
        end;
      end;
    finally
      HumChrDB.Close;
...
begin
  ...
  try
    if HumChrDB.Open then begin
      if HumChrDB.ChrCountOfAccount(FrmCreateChr.sUserId) < 4 then begin
        ...
        HumRecord.btCount          :=0;
        HumRecord.Header.sChrName  :=FrmCreateChr.sChrName;
        if HumRecord.Header.sChrName <> '' then begin
...
begin
  sChrName:=EdChrName.Text;
  if sChrName = '' then exit;
  if MessageDlg('ÄãÈ·¶¨ÒªÓÀ¾ÃÉ¾³ý½ÇÉ«"' + sChrName + '"Âð£¿', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
    try
      if HumChrDB.Open then begin
        HumChrDB.Delete(sChrName);
      end;
...
begin
  ...
  if MessageDlg(IntToStr(nIndex) + 'ÄãÈ·¶¨ÒªÉ¾³ýµÚ¶þ¸ö½ÇÉ«Âð£¿', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
    try
      if HumChrDB.Open then begin
...
* Possible String Reference to: 'éÆ%öÿëø]Ã‹Àƒ-¤ÛJ'
|
...
procedure TFrmIDHum.BtnUserIdSearchClick(Sender: TObject);
var
  sAccount: string;
  ChrList: TStringList;
  I, nIndex: Integer;
...
initialization
  SVNRevision('$Id: FIDHum.pas 531 2006-12-19 13:13:36Z damian $');
end.
