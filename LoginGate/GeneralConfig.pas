...
begin
  ...
  if not IsIPaddr(sGateIPaddr) then begin
    Application.MessageBox('Íø¹ØIPÎÞÐ§','´íÎó',MB_OK + MB_ICONERROR);
    EditGateIPaddr.SetFocus;
    exit;
  end;

  if (nGatePort < 0) or (nGatePort > 65535) then begin
    Application.MessageBox('Íø¹Ø¶Ë¿ÚÎÞÐ§','´íÎó',MB_OK + MB_ICONERROR);
    EditGateport.SetFocus;
    exit;
  end;

  if not IsIPaddr(sServerIPaddr) then begin
    Application.MessageBox('The Server IP is not valid','´íÎó',MB_OK + MB_ICONERROR);
    EditServerIPaddr.SetFocus;
    exit;
  end;

  if (nServerPort < 0) or (nServerPort > 65535) then begin
    Application.MessageBox('The Server Port is not valid','´íÎó',MB_OK + MB_ICONERROR);
    EditServerPort.SetFocus;
    exit;
  end;
  if sTitle = '' then begin
    Application.MessageBox('The Gate Title is not valid','´íÎó',MB_OK + MB_ICONERROR);
    EditTitle.SetFocus;
    exit;
  end;
