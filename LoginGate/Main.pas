...
type
  TUserSession = record
    ...
    dwConnctCheckTick :LongWord;  //Á¬½ÓÊý¾Ý´«Êä¿ÕÏÐ³¬Ê±¼ì²â
  end;
  ...
  TFrmMain=class(TForm)
    ...
    procedure CnTrayIconDblClick(Sender: TObject);
  private
...
begin
  ...
  if IsBlockIP(sRemoteIPaddr) then begin
    MainOutMessage('Â÷´ÜµÈ IPÁ¢¼Ó: ' + sRemoteIPaddr,1);
    Socket.Close;
    exit;
  end;
  ...
  if IsConnLimited(sRemoteIPaddr) then begin
    ...
    MainOutMessage('Çã¿ë°¹¼ö ÃÊ°ú IPÁ¢¼Ó½Ãµµ : ' + sRemoteIPaddr,1);
    exit;
  end;
  ...
  if boGateReady then begin
    ...
    if Socket.nIndex >= 0 then begin
      ClientSocket.Socket.SendText('%O' +
                                   ...
                                   sLocalIPaddr +
                                   '$');
      MainOutMessage('Á¦ÇÑ: ' + sRemoteIPaddr,5);
    end else begin
      Socket.Close;
      MainOutMessage('´ÝÈû: ' + sRemoteIPaddr,1);
    end;
...
begin
  ...
  if Application.MessageBox('Á¾·áÇÏ½Ã°Ú½À´Ï±î?',
                            'Á¾·á',
...
begin
  ...
  if not boGateReady then begin
    StatusBar.Panels[1].Text:='ÁØºñ';
  end else begin
    if boKeepAliveTimcOut then begin
      StatusBar.Panels[1].Text:='Å¸ÀÓ¾Æ¿ô';
    end else begin
    ...
    while (True) do begin
     ...
     for nSocketIndex:= 0 to GATEMAXSESSION - 1 do begin
       ...
       if (GetTickCount - g_SessionArray[nSocketIndex].dwConnctCheckTick) > dwKeepConnectTimeOut then begin
         ...
         MainOutMessage('Á¢¼Ó½Ã°£ ÃÊ°ú/Â÷´Ü: ' + sRemoteIPaddr,1);
         Continue;
       end;
...
begin
  ...
  frmMain.Left := Conf.ReadInteger('LoginGate','Left',(Screen.Width Div 2)-(frmMain.Width Div 2));
  frmMain.Top := Conf.ReadInteger('LoginGate','Top',(Screen.Height Div 2)-(frmMain.Height Div 2)-30);
  Conf.Free;
...
begin
try
    MainOutMessage('¼­ºñ½º ½ÃÀÛ...',3);
    SendGameCenterMsg(SG_STARTNOW, '·Î±×ÀÎ°ÔÀÌÆ® ½ÃÀÛÁß');
    boServiceStart:=True;
    boGateReady:=False;
    boServerReady:=False;    
    nSessionCount:=0;
    ...
    MainOutMessage('¼ÒÄÏ ¿ÀÇÂ...',3);
    SendGameCenterMsg(SG_STARTOK, '·Î±×ÀÎ°ÔÀÌÆ® ½ÃÀÛ¿Ï·á');
  except
...
begin
  if Application.MessageBox('ÁßÁöÇÏ½Ã°Ú½À´Ï±î?',
                            'ÁßÁö',
...
begin
  if Application.MessageBox('·Î±×¸¦ »èÁ¦ÇÏ½Ã°Ú½À´Ï±î?',
                            '·Î±×»èÁ¦',
