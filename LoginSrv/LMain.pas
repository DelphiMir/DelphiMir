...
uses
  ...
  Menus,Grobal2,LSShare, SDK{, HDDInfo};
type
  ...
  //  function GetLisence(MacS, HddS: string; Method: integer; DName: string; cb: integer): Boolean;       //辑滚焊龋
...
uses IDDB, MasSock, FrmFindId, HUtil32, EDcode, GateSet,
  FAccountView, GrobalSession, FrmVarsion;
      ...
      if GateInfo.Socket = Socket then begin
        for II:=0 to GateInfo.UserList.Count -1 do begin
          ...
          if Config.boShowDetailMsg then
            MainOutMessage('关闭: ' + UserInfo.sUserIPaddr);
          Dispose(UserInfo);
        end;
...
//  g_MainMsgList:=TStringList.Create;
  ...
  LoadAddrTable(Config);
  MonitorGrid.Cells[0,0]:='服务器名称 ';
  MonitorGrid.Cells[1,0]:='用户数';
  MonitorGrid.Cells[2,0]:='状态';
  SendGameCenterMsg(SG_STARTNOW, '正在启动账号服务器...');
end;
...
try
  ...
  if (ServerList.Count div 2) < 2 then begin
    MonitorGrid.RowCount:=3;
    MonitorGrid.Cells[0,1]:='';
    MonitorGrid.Cells[1,1]:='';
    MonitorGrid.Cells[2,1]:='';
    MonitorGrid.Cells[0,2]:='';
    MonitorGrid.Cells[1,2]:='';
    MonitorGrid.Cells[2,2]:='';
  end else begin
  ...
  for I := 0 to ServerList.Count - 1 do begin
    ...
    if sServerName <> '' then begin
      if MsgServer.nServerIndex = 99 then begin
        MonitorGrid.Cells[0,2]:=sServerName + ' - (选择角色 ) ';
        MonitorGrid.Cells[1,2]:=IntToStr(MsgServer.nOnlineCount);
        if (GetTickCount - MsgServer.dwKeepAliveTick) < 30000  then
        MonitorGrid.Cells[2,2]:='连接'
        else MonitorGrid.Cells[2,2]:='断开';
      end else
        MonitorGrid.Cells[0,1]:=sServerName + ' - (连接游戏)';
        MonitorGrid.Cells[1,1]:=IntToStr(MsgServer.nOnlineCount);
        if (GetTickCount - MsgServer.dwKeepAliveTick) < 30000  then
        MonitorGrid.Cells[2,1]:='连接'
        else MonitorGrid.Cells[2,1]:='断开';
...
begin
try
 // sPassword:=GetValidStr3(DecodeString(sData),sLoginID,['!']); //备盒磊 郴波 甫 *肺 函版
 // sPassword:=GetValidStr3(DecodeString(sData),sLoginID,['!']); //厘傣泅 郡府
 // sPassword:=GetValidStr3(DecodeString(sData),sLoginID,['&']); //惫刮辑滚 鼻瓤沥
  sPassword:=GetValidStr3(DecodeString(sData),sLoginID,['^']); //沫辑滚  唱辆籍
 // sPassword:=GetValidStr3(DecodeString(sData),sLoginID,['#']); //俊叼胶(捞悼霖)
 // sPassword:=GetValidStr3(DecodeString(sData),sLoginID,['$']); //脚归龋 唱畴(捞霖龋)
 // sPassword:=GetValidStr3(DecodeString(sData),sLoginID,['%']); //唱畴
 // sPassword:=GetValidStr3(DecodeString(sData),sLoginID,['@']); //胶鸥老(沥措泅)
 // sPassword:=GetValidStr3(DecodeString(sData),sLoginID,['-']); //辫泅扁() 去辑滚
 // sPassword:=GetValidStr3(DecodeString(sData),sLoginID,['+']); //怕剧辑滚 NH辑滚
  nCode:=0;
  boNeedUpdate:=False;
  try
    if AccountDB.Open then begin
...
{
function TFrmMain.GetLisence(MacS, HddS: string; Method: integer; DName: string; cb: integer): Boolean;           //辑滚焊龋
...
begin
  Result := False;
  if HddS <> '' then begin
    hdd.Method := TGetInfoMethod(Method);
    if hdd.Method = gimByName then begin
      if DName = '' then begin
        Result := False;
      end;
      hdd.DeviceName := DName;
    end else begin
      hdd.Drive := TDrive(cb);
    end;
    if (hdd.Serial[1] <> #0) and (MacS = EncodeString(GetMacAddress(''))) then begin
      if HDDS = EncodeString(hdd.Serial) then begin
        Result := True;
      end;
    end else begin
      if MacS = EncodeString(GetMacAddress('')) then begin
        Result := True;
      end;
    end;
  end else begin
...
begin
  ...
  PosFile := LoadConfigString('Setup','Positions','.\Positions.ini');
  ini := TMemIniFile.Create(PosFile);
  frmMain.Left := ini.ReadInteger('LoginServer','Left',(Screen.Width Div 2)-(frmMain.Width Div 2));
  frmMain.Top := ini.ReadInteger('LoginServer','Top',(Screen.Height Div 2)-(frmMain.Height Div 2)-30);
  ini.Free;
...
begin
  ini := TMemIniFile.Create(PosFile);
  ini.WriteInteger('LoginServer','Left',frmMain.Left);
  ini.WriteInteger('LoginServer','Top',frmMain.Top);
  ini.UpdateFile;
  ini.Free; 
end;
...
initialization
  SVNRevision('$Id: LMain.pas 258 2006-08-16 14:18:46Z Dataforce $');
end.
