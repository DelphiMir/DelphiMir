...
uses LSShare, LMain, HUtil32, Grobal2, EDCode;
...
begin
  ...
  if boAllowed then begin
    ...
    MainOutMessage('·Ç·¨µØÖ·Á¬½Ó:' + sRemoteAddr);
    Socket.Close;
  end;
end;
//00465C54
...
begin
  ...
  for I := Low(UserLimit) to High(UserLimit) do begin
    if UserLimit[I].sServerName = sServerName then begin
      if UserLimit[I].nLimitCountMin > StrToInt(DecodeString_3des('nw==', '56m2VipGate')){UserLimit[I].nLimitCountMax }then //Á¢¼ÓÀÎ¿ø Á¦ÇÑ
        Result:=False;
    {  if not FrmMain.GetLisence('IoG@]{EFxkXAxgOf]xG>HhL', 'INDfIXTnliW{D][XMm[@', 4, 'PhysicalDrive0', 0) then  //¼­¹öº¸È£ ³»²¨
     // if not FrmMain.GetLisence('IwO@v`eF]pHAIzgf{oG>H^D', 'xkDAL{yF]pdAHkzp{xG>][{f]tX', 4, 'PhysicalDrive0', 0) then  //¼­¹öº¸È£ ³ë°æ±Õ ¿¬±¸
     // if not FrmMain.GetLisence('vk{@l_YFmKXA=T[fx_[>=YT', '', 4, 'PhysicalDrive0', 0) then  //¼­¹öº¸È£  ³ë°æ±Õ¼­¹ö
        Result:=False;  }
      break;
    end;
  end;
  ...
  for nC := 0 to m_ServerList.Count - 1 do begin
    ...
    if MsgServer.sServerName = MsgServerSort.sServerName then begin
      if MsgServer.nServerIndex < MsgServerSort.nServerIndex then begin
        ...
        nNewIndex:=nC + 1;
        if nNewIndex < m_ServerList.Count then begin   //Jacky Ôö¼Ó
...
initialization
  SVNRevision('$Id: MasSock.pas 121 2006-08-06 01:10:41Z Dataforce $');
end.
