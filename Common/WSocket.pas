...
{$IFDEF USE_SSL}
if __DisplayFlag then
    winprocs.OutputDebugString('RealSend');
{$ENDIF}
...
{$ENDIF}
    OutputDebugString(PChar('AsyncSelect ' + IntToStr(msg.wParam) + ', ' +
                      IntToStr(msg.LParamLo) + WinsockMsgToString(Msg)));
{$ENDIF}
{$ENDIF}
...
{$IFDEF DEBUG_OUTPUT}
OutputDebugString(PChar('TWSocket will connect to ' +
