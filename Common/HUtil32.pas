...
uses
	{svn,} Classes, Math, SysUtils, StrUtils, WinTypes, WinProcs, Graphics, Messages, Dialogs;
...
//These 2 functions are same, so use either
...
var
   nPos: integer;
begin
   nPos := Pos (tag, Copy (src, 2, Length(src)));
   rstr := Copy (src, 2, nPos-1);
   Result := Copy (src, nPos+2, length(src)-nPos);
end;
...
function BoolToCStr(b:Boolean):String;
begin
  if b then result:='적용' else result:='해제';
end;
...
initialization
  //SVNRevision('$Id: HUtil32.pas 500 2006-11-01 20:40:19Z damian $');
end.
