...
begin
  ...
  m_boSend := False;
  LabelHint.Caption := FormatDateTime('数据信息 : yyyy年mm月dd日 hh时mm分ss秒', Now);
  RefTopGrid;
end;
...
begin
  ...
  if Application.MessageBox('你确定要退出排行榜插件？', '警告', MB_OKCANCEL + MB_ICONQUESTION) = IDOK then begin
    Timer1.Enabled := False;
    CanClose := True;
  end else begin
    CanClose := False;
...
begin
  ...
  SendGameCenterMsg(SG_STARTNOW, '开始排名');
  ...
  Radio1Click(Radio1);
  RefTopGrid;
  SendGameCenterMsg(SG_STARTOK, '排名刷新成功');
end;
...
var
  ...
  TopInfo: pTTopInfo;
begin
  TopGrid.Cells[0, 0] := '排行';
  if TabControl.TabIndex = 13 then begin
    TopGrid.Cells[1, 0] := '英雄';
  end else
  TopGrid.Cells[1, 0] := '角色名';
  TopGrid.Cells[2, 0] := '职业';
  TopGrid.Cells[3, 0] := '性别';
  case TabControl.TabIndex of
    0..5: TopGrid.Cells[4, 0] := '等级';
    6: TopGrid.Cells[4, 0] := '攻击';
    7: TopGrid.Cells[4, 0] := '魔法';
    8: TopGrid.Cells[4, 0] := '道术';
    9: TopGrid.Cells[4, 0] := '金币';
    10: TopGrid.Cells[4, 0] := '经验值';
    11: TopGrid.Cells[4, 0] := '杀敌(Kill)';
    12: TopGrid.Cells[4, 0] := '死亡(Death)';
    13: TopGrid.Cells[4, 0] := '英雄等级';
    14: TopGrid.Cells[4, 0] := '点券';
    15: TopGrid.Cells[4, 0] := 'PC点';
    16: TopGrid.Cells[4, 0] := '防御';
    17: TopGrid.Cells[4, 0] := '魔法防御';
  end;
  if TabControl.TabIndex = 13 then begin
    TopGrid.Cells[5, 0] := '主人';
  end else
    TopGrid.Cells[5, 0] := '无主';
