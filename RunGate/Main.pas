...
uses
  ...
  Dialogs, JSocket, ExtCtrls, StdCtrls, Grobal2, IniFiles, Menus, GateShare, WinSock,
    ...
    try
      if (GetTickCount - dwRefConsoleMsgTick) >= 1000 then begin
        ...
        if not boShowBite then begin
          LabelReviceMsgSize.Caption:='MSG: ' + IntToStr(nReviceMsgSize  div 1024) + ' KB';
          LabelBufferOfM2Size.Caption:='M2Buffer: ' +IntToStr(nBufferOfM2Size  div 1024) + ' KB';
          LabelProcessMsgSize.Caption:='ProcMsg: ' +IntToStr(nProcessMsgSize  div 1024) + ' KB';
          LabelLogonMsgSize.Caption:='LogOn: ' +IntToStr(nHumLogonMsgSize  div 1024) + ' KB';
          LabelPlayMsgSize.Caption:='PMsg: ' +IntToStr(nHumPlayMsgSize  div 1024) + ' KB';
          LabelDeCodeMsgSize.Caption:='DCode: ' +IntToStr(nDeCodeMsgSize  div 1024) + ' KB';
          LabelSendBlockSize.Caption:='SBlock: ' +IntToStr(nSendBlockSize  div 1024) + ' KB';
          {
          ...
          LabelReviceMsgSize.Caption:='MSG: ' + IntToStr(nReviceMsgSize) + ' B';
          LabelBufferOfM2Size.Caption:='M2Buffer: ' +IntToStr(nBufferOfM2Size) + ' B';
          LabelSelfCheck.Caption:='SCheck: ' + IntToStr(dwCheckServerTimeMin) + '/' + IntToStr(dwCheckServerTimeMax);
          LabelProcessMsgSize.Caption:='ProcMsg: ' +IntToStr(nProcessMsgSize) + ' B';
          LabelLogonMsgSize.Caption:='LogOn: ' +IntToStr(nHumLogonMsgSize) + ' B';
          LabelPlayMsgSize.Caption:='PMsg: ' +IntToStr(nHumPlayMsgSize) + ' B';
          LabelDeCodeMsgSize.Caption:='DCode: ' +IntToStr(nDeCodeMsgSize) + ' B';
          LabelSendBlockSize.Caption:='SBlock: ' +IntToStr(nSendBlockSize) + ' B';
...
//        if (GetTickCount - dwCheckServerTick) > 60 * 1000 then begin
    ...
    if (GetTickCount - dwRefConsolMsgTick) > 1000 then begin
      ...
      LabelReviceLimitTime.Caption:='Recv Limit: ' + IntToStr(dwProcessReviceMsgTimeLimit);
      LabelSendLimitTime.Caption:='Send Limit: ' + IntToStr(dwProcessSendMsgTimeLimit);
      LabelReceTime.Caption:='Recv: ' + IntToStr(dwProcessClientMsgTime);
...
begin
  ...
  if Application.MessageBox('你确定要退出游戏网关？',
                            '确认信息',
...
begin
  ...
  AddMainLogMsg('游戏网关已关闭...',2);
end;

procedure TFrmMain.MENU_CONTROL_STOPClick(Sender: TObject);
begin
  if Application.MessageBox('你确定要停止服务吗？',
                            '确认信息',
...
begin
  ...
  if boGateReady then begin
    ...
    if IsBlockIP(sRemoteAddress) then begin      //挤汗规绢
      AddMainLogMsg('在线连接IP: ' + sRemoteAddress, 3);
      Socket.Close;
      Exit;
    end;

    if IsConnLimited(sRemoteAddress) then begin    //挤汗规绢
      case BlockMethod of
        mDisconnect: begin
            if Socket <> nil then
              Socket.Close;
      ...
      AddMainLogMsg('IP连接尝试超过允许的数量: ' + sRemoteAddress, 3);
      Exit;
    end;
...
begin
  ...
  if not boGateReady then begin
    ...
    if boCheckServerFail then begin
      StatusBar.Panels[1].Text:='关闭';
    end else begin//00457320
      StatusBar.Panels[1].Text:='打开';
...
begin
  if Application.MessageBox('你确定要清除日志记录吗？',
                            '确认信息',
...
function TFrmMain.IsConnLimited(sIPaddr: string):
  Boolean;
var
  i: Integer;
  IPaddr: pTSockaddr;
...
begin
  if Application.MessageBox('你确定要重新读取吗？',
                            '确认信息',
                            MB_OKCANCEL + MB_ICONQUESTION
                            ) <> IDOK then exit;
  LoadConfig();
