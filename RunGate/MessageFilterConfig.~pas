...
begin
  if (ListBoxFilterText.ItemIndex >=0) and (ListBoxFilterText.ItemIndex < ListBoxFilterText.Items.Count) then begin
    ...
    if not InputQuery('Modify', 'Input word to replace the existing:', sInputText) then exit;
  end;
  if sInputText = '' then begin
     Application.MessageBox('You must input a word!','Error', MB_OK + MB_ICONERROR );
     exit;
  end;
...
var
  sInputText:String;
begin
//  sInputText:= InputBox('Ôö¼Ó¹ýÂËÎÄ×Ö', 'ÇëÊäÈëÐÂµÄÎÄ×Ö:', '');
  if not InputQuery('Ãß°¡', 'ÇÊÅÍ ¸Þ½ÃÁö¸¦ ½áÁÖ¼¼¿ä:', sInputText) then exit;

  if sInputText = '' then begin
     Application.MessageBox('¿å ¸Þ½ÃÁö¸¦ ÀÛ¼ºÇÏ¼¼¿ä!','¿À·ù', MB_OK + MB_ICONERROR );
     exit;
  end;
