...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, SDK, Spin, ItmUnit;
...
begin
  ...
  begin
    ...
    if not InputQuery(Format('%s - %s ¾õÐÑ ', [sItemName, sCaption]), format('ÐèÒª [%s] ½ð±Ò :', [sCaption]), sValue) then
      exit;
  ...
  begin
    EditCompoundInfoForm.Caption := Format('%s - %s ¾õÐÑ', [sItemName, sCaption]);
...
begin
  ...
  try
    ...
    for i := 0 to UserEngine.StdItemList.Count - 1 do begin
      ...
      if (Stditem.StdMode in [5,6,90,99,10,11,12,15,19,20,21,22,23,24,26,62,63,64,101,102]) and StdItem.boCompound then
