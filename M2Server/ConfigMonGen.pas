...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, Envir;
...
begin
  for I := 0 to UserEngine.m_MonGenList.Count - 1 do begin
    ...
    ListBoxMonGen.Items.AddObject(MonGen.sMapName + '(' + IntToStr(MonGen.nX) + ':' + IntToStr(MonGen.nY) + ')'
      + ' - ' + MonGen.sMonName + '(' + IntToStr(I + 1) + ')' ,TObject(MonGen));
  end;
  Self.ShowModal;
end;
...
begin
  ...
  if   FindFirst('.\Envir\MonItems'+'\'+ Edit1.Text+ '.*',   faAnyFile,   sr)   =   0   then
  begin
