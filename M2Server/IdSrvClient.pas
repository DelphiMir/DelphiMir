...
uses
  ...
  Dialogs, ExtCtrls, IniFiles, JSocket, WinSock, Grobal2,SDK, M2Share,mudutil, ObjBase;
...
//    boConnected:Boolean;
  ...
  public
    ...
    function GetAdmission(sChrName,sAccount,sIPaddr:String;nSessionID:Integer;var nPayMode:Integer;var nPayMent:Integer):pTSessInfo;
...
begin
    ...
    if FileExists(sConfigFileName) then begin
      ...
      ShowMessage('文件未找到<'+sConfigFileName+'>');
...
function TFrmIDSoc.GetAdmission(sChrName,sAccount,sIPaddr:String;nSessionID:Integer;var nPayMode:Integer;var nPayMent:Integer):pTSessInfo;//0048DE80
var
  I        :Integer;
  SessInfo :pTSessInfo;
  boFound  :Boolean;
...
//  exit;
  ...
  if g_Config.boViewAdmissionFailure and not boFound then begin
    UserObject := UserEngine.GeTPlayObject(sChrName);
    if UserObject <> nil then UserObject.SendDefMessage(SM_OUTOFCONNECTION,0,0,0,0,'');
...
begin
  ...
  MainOutMessage('登录服务器已连接(' + Socket.RemoteAddress + ':' +  IntToStr(Socket.RemotePort) + ')');
end;
...
begin
  if g_Config.boIDSocketConnected then begin
    ClearSession();
    g_Config.boIDSocketConnected:=False;
    MainOutMessage('登录服务器未连接(' + Socket.RemoteAddress + ':' +  IntToStr(Socket.RemotePort) + ')');
  end;
end;
{$IF IDSOCKETMODE = TIMERENGINE}
...
var
  ...
  dwRunTick  :LongWord;
ResourceString                    
  sIDServerConnected = '登陆服务器已连接(%s:%d)';
begin
  s:=INVALID_SOCKET;
...
initialization
  //SVNRevision('$Id: IdSrvClient.pas 500 2006-11-01 20:40:19Z damian $');
end.
