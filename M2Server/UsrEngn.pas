...
type
  TUserEngine = class
    ...
    m_LoadPlayList            :TStringList;       //´ÓDB¶ÁÈ¡ÈËÎïÊý¾Ý
    m_LoadHeroList            :TStringList; //´ÓDB¶ÁÈ¡ÈËÎïÊý¾Ý
    ...
    m_nProcHumIDx             :Integer;   //0x44 ´¦ÀíÈËÎï¿ªÊ¼Ë÷Òý£¨Ã¿´Î´¦ÀíÈËÎïÊýÏÞÖÆ£©
    ...
    m_ColumnList              :TGList;     //È«º¸´Ü
    m_MerchantList            :TGList;     //List_68
    m_DefiniensConst          :TStringList;
    QuestNPCList              :TList;     //0x6C
    //QuestInfoList             :TList;       //Äù½ºÆ® Å×½ºÆ®
    List_70                   :TList;
    m_ChangeServerList        :TList;
    m_MagicEventList          :TList;    //0x78
    MonCount             :Integer; //¹ÖÎï×ÜÊý
    MonCurCount    :Integer; //0x80´¦Àí¹ÖÎï×ÜÊýÎ»ÖÃ£¬ÓÃÓÚ¼ÆËã¹ÖÎï×ÜÊý
    MonRunCount               :Integer;
    MonCurRunCount      :Integer; //0x88´¦Àí¹ÖÎïÊý£¬ÓÃÓÚÍ³¼Æ´¦Àí¹ÖÎï¸öÊý
  ...
  public
    ...
    function GetOnlineCount: string;        //°ü¸®ÀÚ½Ã½ºÅÛ
    procedure Run();
   ...
   // function  GetStdItem2(nIdx: Integer): TStdItem;
    ...
    procedure StoreCry(wIdent:Word;pMap:TEnvirnoment;nX,nY,nWide:Integer;btFColor,btBColor:Byte;sMsg:String);  //°³ÀÎ»óÁ¡ ¿ÜÄ¡±â
    procedure CryCry(wIdent:Word;pMap:TEnvirnoment;nX,nY,nWide:Integer;btFColor,btBColor:Byte;sMsg:String);
    procedure GuildAgitCry (msgtype: integer; env: TEnvirnoment; x, y, wide: integer; saying: string);     //Àå¿ø ¿ÜÄ¡±â
    ...
    function  CopyToUserItemFromName(sItemName: String;Item:pTUserItem; count:integer = 1): Boolean; //¹°¾à
    function  CopyToUserItemFromName2(sItemName: String;Item,sitem:pTUserItem;amont:integer): Boolean; //¹°¾à °Å·¡Ã¢
    ...
    function  GetHumColumn(sUserName:String;var sIPaddr:String; var btColumn:Byte):Boolean;    //È«º¸´Ü

    procedure AddUserChat(sSay:String; ChatMode:integer); //Ã¤ÆÃ½Ã½ºÅÛ
    ...
    function _AllowUpgradeItem(Item: pTUserItem): Boolean;   //º¯È¯
    function AccAllowUpgradeItem(Item: pTUserItem): Boolean;   //Àå½Å±¸º¯È¯
    ...
    function  GetMapMonster2(Envir:TEnvirnoment;List:TList):Integer;   //°ü¸®ÀÚ½Ã½ºÅÛ ¼öÁ¤
    ...
    procedure SendBroadTopMsg(sMsg: string); //ÀÌµ¿ È­¸é°øÁö
    procedure SendBroadTopDelayMsg(sMsg: string; dwDelay: LongWord); //ÀÌµ¿ È­¸é°øÁö
    procedure SendBroadCastMsgExt(sMsg:String;MsgType:TMsgType);
    procedure SendMoveMsg(sMsg: string);    //È­¸é°øÁö (npc)
    ...
    function MakeItemToMap( DropMapName : string ; ItemName: String; Amount :integer; dx, dy:integer ):integer; //¸¶·æ
...
begin
  ...
  m_LoadHeroList            := TStringList.Create; //´ÓDB¶ÁÈ¡ÈËÎïÊý¾Ý
...
begin
  ...
  {for I := 0 to QuestInfoList.Count - 1 do begin   //Äù½ºÆ® Å×½ºÆ®
    ...
    for I := m_MerchantList.Count - 1 downto 0 do begin
      ...
      if Merchant.m_PEnvir <> nil then begin
        ...
        if Merchant.m_boAddtoMapSuccess and (not Merchant.m_boIsHide) then begin
          MainOutMessage('ÉÌÈË¼ÓÔØÊ§°Ü - ' + Merchant.m_sCharName + ' ' + Merchant.m_sMapName + '(' + IntToStr(Merchant.m_nCurrX) + ':' + IntToStr(Merchant.m_nCurrY) + ')');
          m_MerchantList.Delete(i);
          Merchant.Free;
        end else begin
          Merchant.AddMapCount;       //°ü¸®ÀÚ½Ã½ºÅÛ
          Merchant.LoadNPCScript();
          Merchant.LoadNPCData();
        end;
      end else begin
        MainOutMessage(Merchant.m_sCharName + ' - ÉÌÈË¼ÓÔØÊ§°Ü (m.PEnvir=nil)');
        m_MerchantList.Delete(i);
        Merchant.Free;
      end;
      FrmMain.Caption:=sCaption + '[NPC¼ÓÔØÖÐ...(' + IntToStr(m_MerchantList.Count-I) + '/' + IntToStr(m_MerchantList.Count) + ')]';
      // Do Not UnComment!
  ...
  for I := QuestNPCList.Count - 1 downto 0 do begin
    ...
    if NormNpc.m_PEnvir <> nil then begin
      ...
      if NormNpc.m_boAddtoMapSuccess and (not NormNpc.m_boIsHide) then begin
        MainOutMessage(NormNpc.m_sCharName + ' Npc¼ÓÔØÖÐ... ');
        QuestNPCList.Delete(i);
        NormNpc.Free;
      end else begin
        NormNpc.AddMapCount;        //°ü¸®ÀÚ½Ã½ºÅÛ
        NormNpc.LoadNPCScript();
      end;
    end else begin
      MainOutMessage(NormNpc.m_sCharName + ' Npc¼ÓÔØÖÐ... (npc.PEnvir=nil) ');
      QuestNPCList.Delete(i);
      NormNpc.Free;
    end;
...
function TUserEngine.GetOnlineCount: string;    //°ü¸®ÀÚ½Ã½ºÅÛ
var
...
begin
  ...
  Result := Format('%d / ÔÚÏßÓ¢ÐÛ: %d', [nOnlineCount, nOnlineCount2]);
end;
    ...
    try
      ...
      if SwitchDataInfo = nil then begin
        ...
        if Envir <> nil then begin
          ...
          if Envir.Flag.boFight3Zone then begin //¹®ÆÄ ´ë·Ã ÀÌº¥Æ® ¹æ¿¡ ÀÖÀ½.
            if (PlayObject.m_Abil.HP <= 0) and (PlayObject.m_nFightZoneDieCount < 3) then begin    //Á×Àº °æ¿ì
          ...
          if Envir.Flag.VailantZone then begin       //¿ë¸ÍÀÇÀüÀå ¾È¿¡ ÀÖ´ÂÁö °Ë»ç
        ...
        if g_MapManager.FindMap(PlayObject.m_sMapName) = nil then PlayObject.m_Abil.HP:=0; //¸ÊÀÌ ¿À·ù ³­ °æ¿ì   //Á×Àº °ÍÀ¸·Î Ã³¸®
        if PlayObject.m_Abil.HP <= 0 then begin   //Á×Àº °æ¿ì
          PlayObject.ClearStatusTime();
          if PlayObject.PKLevel < 2 then begin
            Castle:=g_CastleManager.IsCastleMember(PlayObject);
            if (Castle <> nil) and Castle.m_boUnderWar then begin  //°ø¼ºÁßÀÌ¸ç ¸É¹öÀÏ°æ¿ì
              PlayObject.m_sMapName:=Castle.GetMapName;
              PlayObject.m_nCurrX:=Castle.GetHomeX;
              PlayObject.m_nCurrY:=Castle.GetHomeY;
            end else begin       //°ø¼º¾ÈÇÒ¶§
              PlayObject.m_sMapName:=PlayObject.m_sHomeMap;
              PlayObject.m_nCurrX:=PlayObject.m_nHomeX - 2 + Random(5);
              PlayObject.m_nCurrY:=PlayObject.m_nHomeY - 2 + Random(5);
            end;
          end else begin //004B0201  //ÇÇÄÉÀÌ´Â ¼º¹Û¿¡¼­ ½ÃÀÛÇÑ´Ù.
            PlayObject.m_sMapName:=g_Config.sRedDieHomeMap{'3'};        //À¯¹èÁö
        ...
        for i:=0 to m_LoadPlayList.Count -1 do begin
          ...
          if not FrontEngine.IsFull and not IsLogined(UserOpenInfo.sAccount, m_LoadPlayList.Strings[i]) then begin
            ...
            if PlayObject <> nil then begin
              PlayObject.AddMapCount;       //°ü¸®ÀÚ½Ã½ºÅÛ
              //PlayObject.m_boClientFlag:=UserOpenInfo.LoadUser.boClinetFlag; //½«¿Í»§¶Ë±êÖ¾´«µ½ÈËÎïÊý¾ÝÖÐ
  ...
  try
    for I := 0 to m_PlayObjectFreeList.Count - 1 do begin
      ...
      if (GetTickCount - PlayObject.m_dwGhostTick) > g_Config.dwHumanFreeDelayTime {5 * 60 * 1000} then begin
        try
          TPlayObject(m_PlayObjectFreeList.Items[i]).Free;        //ÀÜ»óÀÌ ³²´Â´Ù¸é ¿¡·¯°¡³¯ ¼ö ÀÖ´Ù.
        except
        ...
        if PlayObject.m_boSwitchData and (PlayObject.m_boRcdSaved) then begin           //ÀúÀåÀ» ÇÏ°í ³­ ÈÄ¿¡ ¼­¹ö ÀÌµ¿À» ½ÃÅ²´Ù.
                ...
                l_Create: begin
                    case UserOpenInfo.nResult of
                     ...
                     //     MainOutMessage('¿µ¿õ»ý¼º ¼º°ø');
                      ...
                      2: begin  //ÀÌ¹Ì Á¸ÀçÇÏ°Å³ª ±ÝÁöµÈ ÀÌ¸§
                          PlayObject.m_sTempHeroName := '';
                          if g_FunctionNPC <> nil then begin
                            PlayObject.m_nScriptGotoCount := 0;
                ...
                l_Delete: begin
                    case UserOpenInfo.nResult of
                      0: begin     //»èÁ¦ ½ÇÆÐ
                        ...
                        //  MainOutMessage('¿µ¿õ»èÁ¦ ½ÇÆÐ');
                        end;
                      1: begin    //¿µ¿õ »èÁ¦ ¿Ï·á
                          PlayObject.m_sHeroCharName := '';
                        //  MainOutMessage('¿µ¿õ»èÁ¦ ¿Ï·á');
                          if UserOpenInfo.NPC <> nil then begin
                            PlayObject.m_nScriptGotoCount := 0;
...
procedure TUserEngine.ProcessMonsters;
  function GetZenTime(tMap: TEnvirnoment; dwTime:LongWord):LongWord;      //»ç¿ëÀÚÀÇ ¼ö¿¡ µû¶ó¼­ Á¨ÀÇ ºü¸£±â°¡ ¹Ù²ñ.
  var
    r:Real;
  begin
    if tMap <> nil then begin
      if tMap.m_nMobZenDelay >= 100 then begin                          //°ü¸®ÀÚ½Ã½ºÅÛ º¯°æ
    ...
    if ((GetTickCount - dwRegenMonstersTick) > g_Config.dwRegenMonstersTime) then begin
      ...
      if (MonGen <> nil) and (MonGen.sMonName <> '') and not g_Config.boVentureServer then begin
        ...
        if (MonGen.dwStartTick = 0) or (nTemp > GetZenTime(Map, MonGen.dwZenTime)) then begin
          nGenCount := GetGenMonCount(MonGen);   //ÇöÀç »ÌÇôÀÖ´Â ¸÷
          boRegened := True;
          nGenModCount := MonGen.nCount;                                        //°ü¸®ÀÚ½Ã½ºÅÛ º¯°æ
          if nGenModCount >= g_Config.nMobGenCount then begin                   //°ü¸®ÀÚ½Ã½ºÅÛ º¯°æ
            nGenModCount := Round(MonGen.nCount * (Map.m_nMobZenRate / 100));        //¸ñÇ¥ ¸÷ Á¨ ¼ö·®  //°ü¸®ÀÚ½Ã½ºÅÛ º¯°æ
          end else begin
        ...
        g_sMonGenInfo1:=MonGen.sMonName + ',' + IntToStr(m_nCurrMonGen) + '/' + IntToStr(m_MonGenList.Count)     //°ü¸®ÀÚ½Ã½ºÅÛ º¯°æ
...
begin
  ...
  if BaseObject <> nil then begin
    ...
    BaseObject.AddMapCount;   //°ü¸®ÀÚ½Ã½ºÅÛ
  end;
  Result:=BaseObject;
end;
...
begin
    ...
    try
      if (GetTickCount() - dwShowOnlineTick) > g_Config.dwConsoleShowUserCountTime then begin
        ...
        MainOutMessage('ÔÚÏßÈËÊý: ' + IntToStr(GetUserCount) + ' / '+ 'ÔÚÏßÓ¢ÐÛÊý: ' +IntToStr(GetHeroCount));
...
//004AEA00
procedure TUserEngine.StoreCry(wIdent: Word; pMap: TEnvirnoment; nX, nY,    //°³ÀÎ»óÁ¡ ¿ÜÄ¡±â
...
procedure TUserEngine.GuildAgitCry (msgtype: integer; env: TEnvirnoment; x, y, wide: integer; saying: string);     //Àå¿ø ¿ÜÄ¡±â
var
   i: integer;
   hum: TPlayObject;
begin
   for i:=0 to m_PlayObjectList.Count-1 do begin
      ...
      if (not hum.m_boGhost) and (hum.m_boBanShout) then begin
         // ¹®ÁÖ°¡ Àå¿ø ³»¿¡ ÀÖÀ¸¸é, °°Àº ¹øÈ£ÀÇ Àå¿ø ³»¿¡ ÀÖ´Â ¸ðµç »ç¶÷¿¡°Ô °øÁö.
...
begin
   ...
   if ItemList <> nil then begin
      for i:=0 to ItemList.Count-1 do begin
         ...
         //  RealMaxPoint := Random(_MAX(1,  MonItem.MaxPoint div 5 ))  //µå·ÓÀ² 5¹è ÀÌº¥Æ®(Å×¼·)

           RealMaxPoint := Random(MonItem.MaxPoint);

         if MonItem.SelPoint >= RealMaxPoint then begin  //¾ÆÀÌÅÛ µå·ÓÀ² µå¶øÀ²
           if CompareText(MonItem.ItemName, sSTRING_GOLDNAME) = 0 then begin
             mon.IncGold( (MonItem.Count div 2) + Random(MonItem.Count) );
               ...
               if CopyToUserItemFromName (iname, UserItem) then begin
                  ...
                  if StdItem.StdMode in [15,22,23,24,26] then begin    //¹ÌÁöÅÛ  ¿É¼Ç  ¸÷¾ÆÀÌÅÛ
...
function TUserEngine._AllowUpgradeItem(Item: pTUserItem): Boolean;    //º¯È¯
var
  StdItem: TItem;
  PlayObject: TPlayObject;
begin
...
function TUserEngine.AccAllowUpgradeItem(Item: pTUserItem): Boolean;     //Àå½Å±¸º¯È¯
var
  StdItem: TItem;
  PlayObject: TPlayObject;
begin
...
//004AC404
function TUserEngine.CopyToUserItemFromName(sItemName:String;Item:pTUserItem ;count:integer):Boolean;  //¹°¾à
var
  I: Integer;
  StdItem:TItem;
begin
...
function TUserEngine.CopyToUserItemFromName2(sItemName: String;Item,sitem:pTUserItem;amont:integer):Boolean; //¹°¾à °Å·¡Ã¢
var
  I: Integer;
  StdItem:TItem;
begin
...
begin
  ...
  try
    ...
    case DefMsg.Ident of
      ...
      CM_USERMAKEITEMSEL,  //Á¦Á¶½ºÃÄ
      CM_USERMAKEITEM,     //Á¦Á¶½ºÃÄ

      CM_RENTALTRY,
      CM_RENTALADDITEM,
      CM_RENTALDELITEM,

      CM_BUYSTOREITEM,     //°³ÀÎ»óÁ¡
      CM_USERSTORAGEITEM,          //Ã¢°í
      CM_USERTAKEBACKSTORAGEITEM,     //Ã¢°í
      CM_AUTOADDITEM,
      CM_AUTODELITEM,
      
      CM_STORAGEPASSWORD,     //Ã¢°í¾ÏÈ£
      CM_CHECKSTORAGEPW,      //Ã¢°í¾ÏÈ£

      CM_UPONFISH,//³¬½Ã
      CM_UPONFISH2,//³¬½Ã

      CM_MASTERBAGTOHEROBAG, //ÁÖÀÎ > ¿µ¿õ °¡¹æÀ¸·Î
      CM_HEROBAGTOMASTERBAG, //¿µ¿õ > ÁÖÀÎ °¡¹æÀ¸·Î

      CM_HEROSTORAGEITEM,  //ÁÖÀÎ > ¿µ¿õÃ¢°í·Î
      CM_HEROTAKEBACKSTORAGEITEM, //¿µ¿õÃ¢°í > ÁÖÀÎ °¡¹æ
      CM_BUNDLEITEM, //¹°¾à ÇÕÄ¡±â
      ...
      CM_FISHING1,    //³¬½Ã ½ÃÀÛ
      CM_FISHING2: begin  //³¬½Ã ³¡
        if g_Config.boActionSendActionMsg then begin //Ê¹ÓÃUpdateMsg ¿ÉÒÔ·ÀÖ¹ÏûÏ¢¶ÓÁÐÀïÓÐ¶à¸ö²Ù×÷
    ...
    if PlayObject.m_boReadyRun then begin
      case DefMsg.Ident of
        ...
        CM_WIDEHIT,CM_FIREHIT,CM_CRSHIT,CM_TWINHIT,CM_WBHIT,CM_WBHIT2,CM_BALDO,CM_BALDO2,CM_FISHING1, //³¬½Ã ½ÃÀÛ
        CM_FISHING2,CM_DEATHHIT: begin //³¬½Ã ³¡
...
begin
  ...
  if not Map.m_boMonGen then exit;         //°ü¸®ÀÚ½Ã½ºÅÛ
  case nMonRace of              //·¹ÀÌ½º

    SUPREGUARD: Cert:=TSuperGuard.Create;             //°æºñº´(11)
    PETSUPREGUARD: Cert:=TPetSuperGuard.Create;       //°æºñº´(12)
    13: Cert := TMoveSuperGuard.Create;               //°æºñº´(13)
    ARCHER_POLICE: Cert:=TArcherPolice.Create;        //±Ã¼ö°æºñ(20)

    ANIMAL_CHICKEN: begin                             //´ß(51)
      Cert:=TMonster.Create;
      Cert.m_boAnimal:=True;
      Cert.m_nMeatQuality:=Random(3500) + 3000;
      Cert.m_nBodyLeathery:=50;
    end;
    ANIMAL_DEER: begin                                //»ç½¿(52)
    ...
    ANIMAL_WOLF: begin                                //´Á´ë(53)
      Cert:=TATMonster.Create;
      Cert.m_boAnimal:=True;
      Cert.m_nMeatQuality:=Random(8000) + 8000;
      Cert.m_nBodyLeathery:=150;
    end;
    RC_RUNAWAY: Cert:=TChickenDeer.Create;            //´Þ¾Æ³ª´Â³ð(54)
    TRAINER: begin                                    //¼ö·Ã±«¹°(55)
      Cert:=TTrainer.Create;
    end;
    56: Cert:= TSaveGuardMonster.Create;   //¼öÈ£±Í»ç
    57: Cert:= TIceGuard.Create;   //ºù¿°±ÍÀå
    58: Cert:= TGoldGuard.Create;  //È¯»ç±ÍÀå
    59: Cert:= TAglyGuard.Create;  //ºñÈ£±Í¸¶
    60: Cert:= TAgainst.Create;    //¿ªÃµ±Í
    61: Cert:= TDarkShine.Create;  //ÈæÃµ±Í
    62: Cert:= TLeafhopper.Create; //¸ê±Í
    63: Cert:= THanShine.Create;   //ÇÑÃµ±Í
    64: Cert:= TCrazyShine.Create;   //±¤ÇÑ½Ã
    65: Cert:= TAutumn.Create;      //¹é»ó
    66: Cert:= TRhinoceros.Create;    //¼­¿ì
    67: Cert:= TNightattack.Create;    //¾ß°ø¼öÈ£±Í
    68: Cert:= TConvene.Create;   //¿°È­Àû±Í
    69: Cert:= TFireConvene.Create;   //¿­ÀåÀû±Í
    70: Cert:= TBanyaGuardMonster.Create;  //¹Ý¾ßÁÂ»ç ¿ì»ç
    71: Cert:= TDeadCowKingMonster.Create;      //»ç¿ìÃµ¿Õ
    72: Cert:= TNammanKingMonster.Create;      //´Ü¹¬
    73: Cert:= TOldKingMonster.Create;      //°í´ë¼öÈ£Àå
    74: Cert:= TFaceMob.Create;   //ÀÎ¸é¼ö
    75: Cert:= TExplosionDark.Create;   //Ä¥Èæ¼ö
    76: Cert:= TGoblinKing.Create;   //¿ø¿À
    77: Cert:= TCatKing.Create;   //¹¦Àå±º
    78: Cert:= TBigPotKing.Create;   //È£ÁßÃµ
    79: Cert:= TGeneralCat.Create;   //Áø¹¦Àå±º
    MONSTER_OMA: Cert:=TMonster.Create;               //±âº»(80)
    MONSTER_OMAKNIGHT: Cert:=TATMonster.Create;       //±âº»(81)
    ...
    92: Cert:=TCowKingMonster.Create;  //¿ì¸é±Í¿Õ
    ...
    98: Cert:= TBigSnakeWarm.Create; //´ë¸Á;
    99: Cert:= TJumaWizard.Create;   //ÁÖ¸¶¼ú»ç
    MONSTER_WHITESKELETON: Cert:=TWhiteSkeleton.Create;  //¹é°ñ (100)
    MONSTER_SCULTURE: begin
      Cert:=TScultureMonster.Create;
      Cert.bo2BA:=True;
    end;
    MONSTER_SCULTUREKING: Cert:=TScultureKingMonster.Create; //ÁÖ¸¶¿Õ (102)
    MONSTER_BEEQUEEN: Cert:=TBeeQueen.Create;
    104: Cert:=TArcherMonster.Create;
    105: Cert:=TGasMothMonster.Create;//½û±â³ª¹æ
    106: Cert:=TGasDungMonster.Create;
    107: Cert:=TCentipedeKingMonster.Create;     //ÃË·æ½Å
    108: Cert:=TWildKing.Create;   //¾ß¼ö¿Õ
    109: Cert:=TLegendScultureKingMonster.Create;    //°í´ëÁÖ¸¶¿Õ
    110: Cert:=TCastleDoor.Create;        //¼º¹®
    111: Cert:=TWallStructure.Create;     //¼ºº®
    MONSTER_ARCHERGUARD: Cert:=TArcherGuard.Create;        //±Ã¼ö°æºñ
    MONSTER_ELFMONSTER: Cert:=TElfMonster.Create;    //½Å¼ö
    MONSTER_ELFWARRIOR: Cert:=TElfWarriorMonster.Create;     //½Å¼ö
    ...
    119: Cert:=TBigPoisionSpider.Create;    //Ã»½Å½Ã
    120: Cert:=TSoccerBall.Create;
    121: Cert:=TRobberMon.Create;  //°ÅµµÀû±Í
    122: Cert:=TSideArms.Create;   //ÆÐ°ËÀû±Í
    123: Cert:=TDaggerMon.Create;   //µ¶ÀÎÀû±Í
    124: Cert:=TDarkGuard.Create;   //°ßºù¼öÈ£Àå ¸¶¿°¼öÈ£Àå ¾ÏÈ¥¼öÈ£Àå ¿­Áø¼öÈ£Àå
    125: Cert:=TFireCat.Create; //ºÒ±ªÀÌ
    126: Cert:=THammerCat.Create; //¾ó·è¸ÁÄ¡±ªÀÌ
    127: Cert:=TBlackHammerCat.Create; //°ËÀº¸ÁÄ¡±ªÀÌ
    128: Cert:=TArmorCat.Create; //°©±ªÀÌ
    129: Cert:=TShamanCat.Create; //¹«´ç±ªÀÌ
    130: Cert:=TDoubleCriticalMonster.Create;   //ÈæÃµ¸¶¿Õ
    131: Cert:=TRonObject.Create;
    132: Cert:=TSandMobObject.Create;
    133: Cert:=TMagicMonObject.Create;
    134: Cert:=TSkeletonKingMonster.Create;   //ÇØ°ñ¹Ý¿Õ
    135: Cert:=TBamTreeMonster.Create;      //³ª¹«
    MONSTER_DARKANGEL: Cert:=TDarkAngelMon.Create;      //È¥·É
    137: Cert:=TPBKingMonster.Create;  //ÆÄÈ²¸¶½Å ¿Õ
    138: Cert:=TDragon.Create;       //ÆÄÃµ¸¶·æ
    139: Cert:=TDragonStatue.Create;   //¿ë¼®»ó
    140: Cert:=TDragonBody.Create;     //ÆÄÃµ¸¶·æ ¸ö
    MONSTER_ANGEL: Cert:=TAngelMon.Create;      //¿ù·É
    RC_CLONE: Cert := TCloneMon.create;    //142
    143: Cert:=TJumaThunder.Create;      //ÁÖ¸¶°Ý·ÚÀå(143)
    144: Cert:=TCrystalSpider.Create;
    145: begin
     Cert:=TPushedMon.Create; //È£±â¿¬
     TPushedMon(Cert).AttackWide := 1;
    end;
    146: Cert:=TGoldenImugi.Create;         //ºÎ·æ±Ý»ç
    147: Cert:=TNoseMonkey.Create;          //±äÄÚ¿ø¼þÀÌ
    148: Cert:=TFoxWarrior.Create;          //ºñ¿ùÈæÈ£
    149: begin
     Cert:=TPushedMon.Create; //È£±â¿Á
     TPushedMon(Cert).AttackWide := 2;
    end;
    150: Cert := TStickBlockMonster.Create;  //È£È¥¼®
    151: Cert:=TFoxWizard.Create;  //ºñ¿ùÀûÈ£
    152: Cert:=TFoxTaoist.Create;  //ºñ¿ù¼ÒÈ£
    MONSTER_STONE: Cert := TStoneMonster.Create;      //¾ç¸¶Áö¼® À½¸¶Áö¼®
    RC_DARKCLONE: Cert := TDarkClone.create;    //154
    155: Cert:=TFoxBead.Create;         //ºñ¿ùÃµÁÖ
    156: Cert:=TFoxPillar.Create;   //È£È¥±â¼®
    157: Cert:=TGhostAdmiral.Create;   //¾ÏÈæ»ç·É
    158: Cert:=TGhostAdmiral.Create;   //¾ÏÈæ»ç·É
    159: Cert:=TGhostAdmiral.Create;   //¾ÏÈæ»ç·É
    160: Cert:=THellGuard.Create;      //Áö¿Á¹®Áö±â
    161: Cert:=TMillstoneKing.Create;    //¼®¸¶¼ö
    ...
    166: Cert:=TGuJangHwa.Create;                 //°ÅÀåÈ­
    MONSTER_SCULTURE2: Cert:=TScultureMonster2.Create; //167
    168: Cert:=TGumiBoss.Create;
    169: Cert:=TImpressionKingMon.Create;   //Á¶ÀÎ»ó
    170: Cert:=TPotMon.Create;   //È£Áß±Í2
    171: Cert:=THellMonster.Create;   //°­Á¶±Í Âü¿ø±Í
    172: Cert:=THellMonster2.Create;  //¼öµÎ±Í
    173: Cert:=THellMonster3.Create;  //ÀÎ¸é±Í
    174: Cert:=THellMonster4.Create;  //½Ä½Å±Í
    175: Cert:=TPotMon1.Create;   //È£Áß±Í1
    176: Cert:=TIceHellMonster.Create;  //°Ýºù¾ßÂ÷
    177: Cert:=TIceHellMonster2.Create;  //¾ßÂ÷¼öÈ£Àå
    178: Cert:=TIceHellKing.Create;  //ÇÑºù¼öÈ£±Í
    179: Cert:=TWaterSlave.Create;   //ÀÍ»çÇÑ³ë¿¹
    180: Cert:=TSpitSpider.Create; //¹ìÀÇ¶Ç¾Æ¸®
    //2005/12/14
    181: begin
       Cert := TATMonster.Create;
    ...
    183: Cert:=TBossTurtle.Create;     //Çö¹«Çö½Å
    184: Cert:=TMermaidMon.Create;   //¾î¹ýÀÎ
    185: Cert:=TMermaidMon2.Create;  //°¢¼¶
    186: Cert:=TGreatKing.Create;  //¿°¸¶ÅÂÀÚ
    187: Cert:=TMermaidMon3.Create;  //ÀÎ¿ë
    188: Cert:=TMerMaidKingMon.Create;      //¼ö¾î±Í
    189: Cert:=TEarthToad.Create; //¶¥µÎ²¨ºñ
    190: Cert:=THumMon.Create;   //ÀÎ°£¼ö
    191: Cert:=TBirdMan.Create;  //¹®Á¶ÀÎ
    192: Cert:=TBigRhino.Create;    //Å«»Ô¼­¿ìÀÎ
    193: Cert:=TStonegolem.Create;    //µ¹°ÅÀÎ
    194: Cert:=TEarthGolem.Create;    //Èë°ÅÀÎ
    195: Cert:=TGanesha.Create; //°¡³×¼ö
    196: Cert:=TBossTreeman.Create; //¿©ÇÏ¼ö
    197: Cert:=TPeacockspider.Create; //°øÀÛÁÖ
    198: Cert:=TLegendFoxWizard.Create;  //°í´ëºñ¿ùÀûÈ£
    199: Cert:=TLegendFoxTaoist.Create;  //°í´ëºñ¿ù¼ÒÈ£
    200: Cert:=TElectronicScolpionMon.Create;  //Èæ»ç¿Õ
    201: Cert:=THeal.Create;
    202: Cert:=TLegendFoxWarrior.Create;    //°í´ëºñ¿ùÈæÈ£
    203: Cert:=TTeleMonster.Create;
    204: Cert:=TElectBossMon.Create;       //¾ÏÈæ¼±Àå
    205: Cert:=TWindLead.Create;           //¸¶Ç³¼®±«
    206: Cert:=TKhazard.Create;            //¼³ÀÎ´ëÃæ

    207: Cert:=TEvilMir.Create;            //ÆÄÃµ¸¶·æ (±¸)

    208: Cert:=TGreenMonster.Create;
    209: Cert:=TRedMonster.Create;
    210: Cert:=TFrostTiger.Create;    //È¯¿µÇÑÈ£
    211: Cert:=TGhostShipMon.Create;       //¼±¹ÚÀÇ¾Ç·É
    212: Cert:=TFramNoMon.Create;          //³ó³ë±º
    213: Cert:=TTransMon.Create;           //º¯ÀÌµÈ°¨½Ãº´

    214: Cert:=TFireMonster.Create;
    215: Cert:=TFireBallMonster.Create;

    216: Cert:=TDarkWolf.Create;                    //°ËÀºÈæ¼º¼º, ÀÌµÎÈæÈ£, ¿©±Í
    217, 220: Cert:=TTeguOmaMonster.Create;         //ÅÂ±¸¿À¸¶   ¿À¸¶Àûº´
    218: Cert:=TGegunOmaMon.Create;                 //°Å±Ç¿À¸¶
    219: Cert:=TDamukOmaMon.Create;                 //´Ù¸ñ¿À¸¶, ¿À¸¶Èæº´
    221: Cert:=TOmaJeSaMonster.Create;              //¿À¸¶Á¦»çÀå
    222,223,224: Cert:=TDigOutGusle.Create;         //±¸½½
    225: Cert:=TOmaDarkKing.Create;                 //¿À¸¶Èæ·É
    226: Cert:=TGhostFire.Create;                   //È¥ºÒ
    227: Cert:=TJumaFireArc.Create;                 //ÁÖ¸¶È­±Ã»ç
    228: Cert:=TJumaWindArc.Create;                 //ÁÖ¸¶Ç³±Ã»ç
    229: Cert:=TJumaWarrior.Create;                 //ÁÖ¸¶°Ë»ç
    230: Cert:=TGukguljang.Create;                   //°Ý±«Àå
    231: Cert:= TExplosion2Dark.Create;            //°­È£
    232: Cert := TSkeletonSoldier.Create;           //ÇØ°ñº´Á¹
    JIN_WHITESKELETON: Cert := TJinWhiteSkeleton.Create;         //Áø°ñ
    234: Cert:=TDragonPart.Create;

    240: Cert:=TPetObject.Create;               //¿µ¹°

    246: Cert:=TNammanStructure.Create;
    247: Cert:=TSabukStructure.Create;          //¼ºº®
    248: Cert:=TNammanCastleDoor2.Create;       //³²¸¸³»¼º¹®
    249: Cert:=TNammanCastleDoor.Create;        //³²¸¸¿Ü¼º¹®
    250: Cert:=TWallFlag.Create;  //Á¡·É±ê¹ß
    RC_BONZECLONE: Cert := TBonzeClone.create;    //251

    //RC_HEROOBJECT(255)
    
    //====Çö¿ù¸²===
    261: Cert:=TTreeTuttle.Create;     //½£°ÅºÏÀÌ
    262: Cert:=TTreeJungBack.Create;   //»ï³ª¹«Á¤¹é
    263: Cert:=TBear.Create;           //°õ, Ç¥¹ü
    264: Cert:=TWarriorM.Create;       //Çö¿ù´ÜÀü»ç
    265: Cert:=TWizardM.Create;        //Çö¿ù´Ü¼ú»ç
    266: Cert:=TTaoistM.Create;        //Çö¿ù´Üµµ»ç
    267: Cert:=TAngelMon2.Create;      //Çö¿ù·É
    268: Cert:=TAssassinM.Create;      //Çö¿ù´ÜÀÚ°´
    269: Cert:=TWarriorM2.Create;      //Àü½ÂÀü»ç
    270: Cert:=TWizardM2.Create;       //¼º¼ú»ç
    271: Cert:=TTaoistM2.Create;       //Á¤¿¬µµ»ç
    272: Cert:=TAssassinM2.Create;     //Çõ¹®°´
    273: Cert:=TAchorMonster.Create;   //À¯»ó±ÃÀå
    274: Cert:=TJobking.Create;        //¾ç¿ë¿Õ
    //¼³¹éÇö ÇÊµå
    275: Cert := TIceSoldier.Create;   //ºù¿ø±Íº´
    276: Cert := TIceArc.Create;       //ºù¿ø±Í±Ã»ç
    277: Cert := IIceZone.Create;      //ºù¿ø±ÍÁ¸
    278: Cert := IIceDevil.Create;     //¾Ç±Í
    279: Cert := TIceKingBoss.Create;  //¼³ÅÂ¶û
  end;
  ...
  if Cert <> nil then begin
   ...
   // if not (Cert.m_btRaceServer in [RC_PLAYOBJECT, RC_HEROOBJECT]) then
    ...
    if Cert.m_boAddtoMapSuccess then begin      //Á¨ÀÚ¸®°¡ ¸ø¿òÁ÷ÀÌ´Â ÀÚ¸®
      p28:=nil;
...
function TUserEngine.GetHumColumn(sUserName:String;var sIPaddr:String; var btColumn:Byte):Boolean;//È«º¸´Ü
var
  I         :Integer;
...
procedure TUserEngine.AddUserChat(sSay:String; ChatMode:integer);  //Ã¤ÆÃ½Ã½ºÅÛ
var
  sMsg: String;
begin
  sMsg := Trim(sSay);
  ...
  case ChatMode of
     0:  FrmMain.GeneralMsg.Lines.Add(sSay);   //ÀÏ¹Ý
     1:  FrmMain.CommandMsg.Lines.Add(sSay);   //¸í·É
     2:  FrmMain.WhisperMsg.Lines.Add(sSay);   //±Ó¼Ó¸»
     3:  FrmMain.SellMsg.Lines.Add(sSay);      //»óÁ¡
     4:  FrmMain.GroupMsg.Lines.Add(sSay);     //±×·ì
     5,6:  FrmMain.GuildMsg.Lines.Add(sSay);   //¹®ÆÄ, µ¿¸Í
     7:  FrmMain.CryMsg.Lines.Add(sSay);       //¿ÜÄ¡±â
     8:  FrmMain.LoverMsg.Lines.Add(sSay);     //¿¬ÀÎ
     9:  FrmMain.MasterMsg.Lines.Add(sSay);    //»çÁ¦
     10: FrmMain.ForGmMsg.Lines.Add(sSay);     //¿î¿µÀÚ¿¡°Ô
     11: FrmMain.VoiceMsg.Lines.Add(sSay);     //°í¼º
  end;
end;
...
begin
  ...
  PlayObject.m_Abil.KillPoint     := HumanRcd.Abil.KillPoint;     //Å³/µ¥½º
  PlayObject.m_Abil.DeathPoint    := HumanRcd.Abil.DeathPoint;    //Å³/µ¥½º
  PlayObject.m_Abil.KillPointCurr := HumanRcd.Abil.KillPointCurr;     //Å³/µ¥½º
  PlayObject.m_Abil.DeathPointCurr:= HumanRcd.Abil.DeathPointCurr;    //Å³/µ¥½º
  ...
  PlayObject.m_btReLevel          := HumanRcd.btReLevel;       //ÀüÁ÷

  PlayObject.m_sMasterName        := HumanRcd.sMasterName;

  PlayObject.m_sStoragePwd        := HumanRcd.sStoragePwd;     //Ã¢°í¾ÏÈ£
  if PlayObject.m_sStoragePwd <> '' then
    PlayObject.m_boStoragePwd := True;                         //Ã¢°í¾ÏÈ£
  ...
  PlayObject.m_nPkPoint           := HumanRcd.nPkPoint;     //ÇÇÄÉÀÌ

  PlayObject.m_nKillPoint         := HumanRcd.nKillPoint;    //Å³/µ¥½º
  PlayObject.m_nDeathPoint        := HumanRcd.nDeathPoint;   //Å³/µ¥½º

  if HumanRcd.btAllowGroup > 0 then PlayObject.m_boAllowGroup:=True      //±×·ì
  else PlayObject.m_boAllowGroup  := False;      //±×·ì
  PlayObject.m_btAttatckMode      := HumanRcd.btAttatckMode;      //°ø°Ý¸ðµå
  ...
  PlayObject.m_boAllowGroupReCall := HumanRcd.boAllowGroupReCall;  //±×·ì
  ...
  PlayObject.m_MissionFlag        := HumanRcd.MissionFlag;      //Äù½ºÆ®
  PlayObject.m_MissionInfo        := HumanRcd.MissionInfo;
  PlayObject.m_MissionIndex       := HumanRcd.MissionIndex;        //Äù½ºÆ®
  PlayObject.m_MissionArithmometer := HumanRcd.MissionArithmometer;                //Äù½ºÆ®
  ...
  PlayObject.m_UseItems[U_TIGER]     := HumItems[U_TIGER];  //È£¶ûÀÌ

  PlayObject.m_UseItems[U_PETS1]     := HumItems[U_PETS1];     //È£¶ûÀÌ
  ...
  PlayObject.m_UseItems[U_PETS5]     := HumItems[U_PETS5];      //È£¶ûÀÌ

  PlayObject.m_UseItems[U_FISH1]     := HumItems[U_FISH1];    //³¬½Ã
...
begin
  ...
  HeroObject.m_Abil.KillPoint     := HumanRcd.Abil.KillPoint;     //Å³/µ¥½º
  HeroObject.m_Abil.DeathPoint    := HumanRcd.Abil.DeathPoint;    //Å³/µ¥½º
  HeroObject.m_Abil.KillPointCurr := HumanRcd.Abil.KillPointCurr;     //Å³/µ¥½º
  HeroObject.m_Abil.DeathPointCurr:= HumanRcd.Abil.DeathPointCurr;    //Å³/µ¥½º
  ...
  HeroObject.m_btReLevel          := HumanRcd.btReLevel;       //ÀüÁ÷

  HeroObject.m_nPkPoint := HumanRcd.nPKPOINT;
  HeroObject.m_nKillPoint         := HumanRcd.nKillPoint;    //Å³/µ¥½º
  HeroObject.m_nDeathPoint        := HumanRcd.nDeathPoint;   //Å³/µ¥½º
...
begin
  ...
  PlayObject.m_boHearMsg      := SwitchData.boHearMsg;      //ÀÏ¹Ý ´ëÈ­
  ...
  for i:=0 to EXTRAABIL_SIZE-1 do begin
      ...
      PlayObject.ExtraAbilTimes[i] := SwitchData.ExtraAbilTimes[i] + GetTickCount  //ÀúÀåµÈ ½Ã°£Àº ³²Àº ½Ã°£ÀÓ
      else
...
begin
    for I := 0 to MonsterList.Count - 1 do begin
      ...
      if CompareText(Monster.sName,sMonName) = 0 then begin
        BaseObject.m_btRaceServer:=Monster.btRace;        //¸÷·¹ÀÌ½º
...
begin
  try
    if (GetTickCount() - dwProcessUsrLogTick) > 8*60*1000 {8 mins} then begin
      ...
      WriteLn(LogFile, ';½ÇÉ«Ãû, ÐÔ±ð, Ö°Òµ, ¹¥»÷Ä£Ê½, µÈ¼¶, µØÍ¼, x y ×ø±ê, µÇÂ½ID, IP, È¨ÏÞ');
  ...
  for I := m_MagicEventList.Count - 1 downto 0 do begin
    ...
    if MagicEvent <> nil then begin
      for II := MagicEvent.BaseObjectList.Count - 1 downto 0 do begin
        BaseObject:=TBaseObject(MagicEvent.BaseObjectList.Items[II]);       //°á°è¿¡ ¸ó½ºÅÍ Á×¾ú³ª , Ç®·È´ÂÁö È®ÀÎ
      ...
      if (MagicEvent.BaseObjectList.Count <= 0) or
         ...
         ((GetTickCount - MagicEvent.dwStartTick) > 180000) then begin    //3ºÐ  °á°è
...
function TUserEngine.GetMapMonster2(Envir: TEnvirnoment; List: TList): Integer; //004AE20C   //°ü¸®ÀÚ½Ã½ºÅÛ ¼öÁ¤
var
  I,II: Integer;
  MonGen:pTMonGenInfo;
  BaseObject:TBaseObject;
...
procedure TUserEngine.SendMoveMsg(sMsg: string);  //È­¸é°øÁö (npc)
var
  I: Integer;
  PlayObject: TPlayObject;
begin
    ...
    if s10[1] = '$' then begin
      ...
      if n10 > 0 then begin
        ...
        if nIdx > -1 then begin
          ...
          MainOutMessage('½Å±¾³£Á¿£º ' + s10 + ' Î´ÕÒµ½');
          sMsg := s14 + '????' + s18;
        end;
      end;
    end;
   ...
   try
      ...
      if ps <> nil then begin
         ...
         if CopyToUserItemFromName(ps.Name, newpu) then begin
            ...
            if ItemName = sSTRING_GOLDNAME{'±ÝÀü'} then begin
               pmi.Name := sSTRING_GOLDNAME{'±ÝÀü'};
               pmi.Count := Amount;
               pmi.Looks := GetGoldShape (Amount);
               pmi.OfBaseObject := nil;
               pmi.dwCanPickUpTick := GetTickCount;
               pmi.DropBaseObject := nil;
               MainOutMessage( '[½ð±ÒÊý] ');
            end else begin
               // Ä«¿îÆ® ¾ÆÀÌÅÛ
               if (ps.StdMode in [0,1,3,45,46]) then begin
                  iTemp := newpu.Amount;
                  if iTemp > 1 then
                     pmi.Name := ps.Name + '(' + IntToStr(iTemp) + ')'  // gadget :Ä«¿îÅÍ¾ÆÀÌÅÛ
                  else
                     pmi.Name := ps.Name;
               end else
                  pmi.Name := ps.Name;
