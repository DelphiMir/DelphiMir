...
type
  THeroObject = class(TAnimalObject)
    ...
    m_boDupMode: Boolean;         //ÈËÎïÖØµþÁË
    m_nTargetX: Integer;          //Ä¿±ê×ø±êX
    m_nTargetY: Integer;          //Ä¿±ê×ø±êY
    m_boRunAwayMode: Boolean;     //ÔËÐÐÔ¶ÀëÄ£Ê½
    m_dwRunAwayStart: LongWord;   //
    m_dwRunAwayTime: LongWord;    //

    m_wHitMode: Word;             //¹¥»÷·½Ê½

    m_dwRunIntervalTime: LongWord;       //¶Ù´Â ¼Óµµ?
    m_dwWalkIntervalTime: LongWord;      //×ß¼ä¸ôÊ±¼ä
    m_dwTurnIntervalTime: LongWord;      //×ª¶¯¼ä¸ôÊ±¼ä

    m_SkillUseTick: array[0..200 - 1] of LongWord; //Ä§·¨Ê¹ÓÃ¼ä¸ô

    m_nItemBagCount: Integer;         //°ü¹üÈÝÁ¿

    m_btStatus: Byte;                 //°ø°Ý¹æ½Ä 0-°ø°ÝÇü 1-¹Ý°ÝÇü 2-´ë±â  4-»ç¿ëÀÚÁ¤ÀÇ 5-ÈÞ½Ä
    m_boProtectStatus: Boolean;       //°­Á¦ÀÌµ¿ÀÏ¶§
    m_boTarget: Boolean;              //°­Á¦°ø°Ý ¸í·É
    m_boSpellTarget: Boolean;
    m_nProtectTargetX, m_nProtectTargetY: Integer; //ÊØ»¤×ø±ê
    m_dwAutoAvoidTick: LongWord;      //×Ô¶¯¶ã±Ü¼ä¸ô
    m_btReLevel: Byte;               //×ªÉúµÈ¼¶

    m_dwMagicAttackInterval: LongWord;//Ä§·¨¹¥»÷¼ä¸ôÊ±¼ä(Dword)
    m_dwMagicAttackTick: LongWord;    //Ä§·¨¹¥»÷Ê±¼ä(Dword)
    m_dwMagicAttackCount: LongWord;   //Ä§·¨¹¥»÷¼ÆÊý(Dword)
    m_nSelectMagic: Integer;          //²éÑ¯Ä§·¨
    m_boIsUseMagic: Boolean;          //ÊÇ·ñ¿ÉÒÔÊ¹ÓÃµÄÄ§·¨(True²Å¿ÉÄÜ¶ã±Ü)

    m_btLastDirection: Byte;          //×îºóµÄ·½Ïò

    m_nSessionID: Integer; //0x5C0
    m_dwSaveRcdTick: LongWord;
    m_dLogonTime: TDateTime;    //°ü¸®ÀÚ½Ã½ºÅÛ
    m_dwLogonTick: LongWord;    //°ü¸®ÀÚ½Ã½ºÅÛ
    ...
    m_dwQueryBagItemsTick        :LongWord; //°¡¹æÁ¤¸®
    procedure Herowalk();

  private
    function  AutoAvoid(): Boolean; //×Ô¶¯¶ã±Ü
    function  CheckTargetXYCount(nX, nY, nRange: Integer): Integer;
    function  CheckTargetXYCount1(nX, nY, nRange: Integer): Integer;//Õ½Ê¿ÅÐ¶ÏÄ¿±êÊ¹ÓÃ 20080924
    function  CheckTargetXYCount2(): Integer;//°ëÔÂÍäµ¶ÅÐ¶ÏÄ¿±êº¯Êý 20081207
    function  CheckMasterXYOfDirection(TargeTBaseObject: TBaseObject;nX, nY, nDir, nRange: Integer): Integer;{¼ì²âÖ¸¶¨·½ÏòºÍ·¶Î§ÄÚ,Ö÷ÈËÓëÓ¢ÐÛµÄ¾àÀë}
    ...
    function  CheckUserMagic(wMagIdx: Word): pTUserMagic;//¼ì²éÊ¹ÓÃÄ§·¨
    ...
    function  WarrAttackTarget(wMagIdx, wHitMode: Word): Boolean; virtual;    //¿µ¿õ°ø°Ý
    function  WarrorAttackTarget(wMagIdx: Word): Boolean; virtual;    //¿µ¿õÀü»ç
    function  WizardAttackTarget(wMagIdx: Word): Boolean; virtual;    //¿µ¿õ¼ú»ç
    function  TaoistAttackTarget(wMagIdx: Word): Boolean; virtual;    //¿µ¿õµµ»ç
    function  AssassinAttackTarget(wMagIdx: Word): Boolean; virtual;  //¿µ¿õÀÚ°´

    function  IsSearchTarget: Boolean;

    function  IsNeedAvoid(): Boolean;   //ÊÇ·ñÐèÒª¶ã±Ü
  ...
  public
    ...
    function  IsNeedGotoXY(): Boolean; //Å¸ÄÏ¿¡°Ô µû¶ó°¡´Â?
    ...
    procedure ClientaddBundleItems(mode,MaxAmount:integer;ItemIdx:String); //¹°¾à2
    procedure ClientSeparateItems(bMakeIndex,namount:integer);    //¹°¾à2
    ...
    function AddItemToBagCategories(UserItem: PTUserItem;var boot:Boolean): Boolean; //¹°¾à2
    ...
    procedure CmdUpgradeItem (seedname, jewelryname: string; seedindex, jewelryindex, ExecCount : integer);        //½ºÃÄ
    function CheckJewelryItem (iStdMode: integer) : Boolean;          //½ºÃÄ
    function CheckSeedItem (puSeed: PTUserItem ;psSeed, psJewelry: TItem) : Integer;       //½ºÃÄ
    ...
    function  GetTotalValueOfOption( pu: PTUserItem; pstd, psJewelry: TItem; var strResult, strEtc: string ) : integer;     //½ºÃÄ
    function  DoUpgradeItem( puSeed: PTUserItem; psSeed, psJewelry: TItem ): integer;   //½ºÃÄ
    ...
    function  RepairItemNormaly( psSeed: TItem; puSeed: PTUserItem ): Boolean;   //»À¸ÁÄ¡, ¹Ù´ÀÁú
    function  RepairItemSuper( psSeed: TItem; puSeed: PTUserItem ): Boolean;     //Àå½Å±¸, Àç´Ü
    function SumOfOptions(puSeedItem: PTUserItem; psSeedItem: TItem) : integer;       //½ºÃÄ
    procedure SendDelItemWithFlag (UserItem: pTUserItem; wBreakdown: Word);   //½ºÃÄ
    function  GemItem(Gem:Integer;ItemIdx:Integer):integer;    //º¸¿Á(±¸ ½Ã½ºÅÛ)
    procedure GemItemFail(UserItem:pTUserItem;msg:String);      //º¸¿Á(±¸ ½Ã½ºÅÛ)
  end;
  //{$ENDREGION}
...
begin
  ...
  m_dwRunIntervalTime := GetTickCount(); //¶Ù´Â ¼Óµµ?
  ...
  m_nWalkSpeed := g_Config.dwHeroWalkTime; //¶Ù´Â ¼Óµµ
  m_nWalkStep := 10;
  m_dwWalkWait := 0;
  ...
  m_nNextHitTime := 300;//ÏÂ´Î¹¥»÷Ê±¼ä

  m_nItemBagCount := 42;

  m_btStatus := 0;
  m_boProtectStatus := False; //°­Á¦ÀÌµ¿ÀÏ¶§

  m_boTarget := False; //°­Á¦°ø°Ý ¸í·É
  m_boSpellTarget := False;
  m_nProtectTargetX := -1; //ÊØ»¤×ø±ê
  m_nProtectTargetY := -1; //ÊØ»¤×ø±ê
  ...
  m_boIsUseMagic := False;//ÊÇ·ñÄÜ¶ã±Ü 20080714
...
begin
  ...
  case ProcessMsg.wIdent of
    ...
    CM_HEROBUNDLEITEM: begin //1003
      ClientaddBundleItems(ProcessMsg.nParam2,ProcessMsg.nParam1,ProcessMsg.sMsg);  //¹°¾à2
    end;
    CM_HEROBUNDLESTORAGEITEM: begin //1003
      ClientaddBundleStorageItems(ProcessMsg.nParam2,ProcessMsg.nParam1,ProcessMsg.sMsg);
    end;

    CM_HEROSEPARITEM: begin   //¹°¾à2
        ClientSeparateItems(ProcessMsg.nParam1,ProcessMsg.nParam2) //¹°¾à2
    end;
    ...
    CM_GEMHEROITEM:begin             //º¸¿Á(±¸ ½Ã½ºÅÛ)
     ...
     RM_STRUCK: begin
        if (m_Master <> nil) and (not m_Master.m_boGhost) and (not m_boHeroLogOut) and (not m_boGhost) then begin
          if (ProcessMsg.BaseObject = Self) and (TBaseObject(ProcessMsg.nParam3 {AttackBaseObject}) <> nil) then begin
            ...
            BreakHolySeizeMode();
            if (m_Master <> nil) and
              (TBaseObject(ProcessMsg.nParam3) <> m_Master) and   //ÁÖÀÎÀÌ ÀÖ´Â ¸÷
              (TBaseObject(ProcessMsg.nParam3).m_btRaceServer = RC_PLAYOBJECT) then begin      //ÁÖÀÎ¿¡°Ô Á¤´ç¹æÀ§
    ...
    CM_QUERYHEROBAGITEMS: begin
        if GetTickCount - m_dwQueryBagItemsTick > 30 * 1000 then begin
          ...
          SysMsg(IntToStr(30 - (GetTickCount - m_dwQueryBagItemsTick) div 1000) + 'ÃëÒÔºó¿ÉÒÔÔÙ´ÎÊ¹ÓÃ¡£',c_RedWhite,t_Hint);
      end;
...
procedure THeroObject.ClientQueryStorageItems(Idx:Integer);   //¿©°ü      //Ã¢°í
var
  I,II:Integer;
  Item:TItem;
  sSendMsg:String;
...
procedure THeroObject.ItemTimeRun;   //¿©°ü       //Ã¢°í
var
 I:Integer;
begin
...
//Ó¢ÐÛÎÞ¼«ÕæÆø  20080323 ÐÞ¸Ä
//0¼¶ÌáÉýµÀÊõ40%   1¼¶ÌáÉý60%   2¼¶ÌáÉý80%  3¼¶ÌáÉý100%  Ê±¼ä¶¼ÊÇ6Ãë
...
begin
  ...
  {  if g_Config.boAbilityUpFixMode then begin//ÎÞ¼«ÕæÆøÊ¹ÓÃ¹Ì¶¨Ê±³¤Ä£Ê½ 20081109
      n14:= g_Config.nAbilityUpFixUseTime;//ÎÞ¼«ÕæÆøÊ¹ÓÃ¹Ì¶¨Ê±³¤ 20081109
    end else n14:=(UserMagic.btLevel * 2)+ 2 + g_Config.nAbilityUpUseTime;//20080603
    m_dwStatusArrTimeOutTick[2] := GetTickCount + n14 * 1000;
    m_wStatusArrValue[2] := Round(HiWord(m_WAbil.SC)*(UserMagic.btLevel * 0.2 + 0.4));//ÌáÉýÖµ 20080827
    SysMsg('(Ó¢ÐÛ) µÀÊõË²Ê±ÌáÉý' + IntToStr(m_wStatusArrValue[2]) + '£¬³ÖÐø ' + IntToStr(n14) + ' Ãë', c_Green, t_Hint);
                       }
    RecalcAbilitys();
  //  CompareSuitItem(False);//Ì××°ÓëÉíÉÏ×°±¸¶Ô±È 20080729
...
function THeroObject.AutoAvoid(): Boolean; //×Ô¶¯¶ã±Ü
  ...
  begin
    ...
    if n10 > m_nCurrX then begin
      Result := DR_RIGHT;//¶«
      if n14 > m_nCurrY then Result := DR_DOWNRIGHT;//¶«ÄÏÏò
      if n14 < m_nCurrY then Result := DR_UPRIGHT;//¶«±±Ïò
    end else begin
      if n10 < m_nCurrX then begin
        Result := DR_LEFT;//Î÷
        if n14 > m_nCurrY then Result := DR_DOWNLEFT;//Î÷ÄÏÏò
        if n14 < m_nCurrY then Result := DR_UPLEFT;//Î÷±±Ïò
      end else begin
        if n14 > m_nCurrY then Result := DR_DOWN//ÄÏ
        else if n14 < m_nCurrY then Result := DR_UP;//Õý±±
      end;
    end;
  end;
...
function THeroObject.AttackTarget(): Boolean;    //¿µ¿õ°ø°Ý
var
...
begin
  ...
  try
    ncode := 1;
  if m_btStatus = 1 then begin  //°ø°Ý°¡´É »óÅÂ°¡ ¾Æ´Ï¸é (´ë±â»óÅÂ, ÈÞ½Ä»óÅÂ)
    if m_TargetCret <> nil then m_TargetCret:= nil;
    Exit;
  end;
  ncode := 2;
  if InSafeZone and (m_TargetCret <> nil) then begin//¾ÈÀü¿¡¼­ °ø°Ý´ë»óÀÌ ÀÖÀ»¶§
    if (m_TargetCret.m_btRaceServer = RC_PLAYOBJECT) or (m_TargetCret.m_btRaceServer = RC_HEROOBJECT) then begin      //»ç¶÷°ú ¿µ¿õÀº Å¸°ÙÀ» ¾ø¾ÖÁØ´Ù.
       m_TargetCret:= nil;
       Exit;
    end;
  end;
  ...
  case m_btJob of
    0: begin  //¿µ¿õÀü»ç AttackTarget
        ...
        if (GetTickCount - m_dwHitTick > dwAttackTime) and (GetTickCount - m_SkillUseTick[0] > (Integer(g_Config.dwHeroWarrorAttackTime) - m_nHitSpeed * g_Config.dwHeroItemAttackTime)) then begin
          ...
          ncode := 8;
          if (nSelectMagic > 0) and (nSelectMagic <= High(m_SkillUseTick)) then begin     //Á¢¼Ó ÈÄ 2ÃÊÈÄ¿¡ then begin
  ...
  Try
    for I := m_ItemList.Count - 1 downto 0 do begin
      ...
      if (UserItem.wIndex = wIndex) and (UserItem.MakeIndex = MakeIndex) then begin
        ...
        m_ItemList.Delete(I);
        Break;//20081014 Ö»ÕÒµ½Ò»¼þÔòÍË³ö£¬Ìá¸ßÐ§ÂÊ
      end;
    end;
  except
    MainOutMessage('{Òì³£} THeroObject.ClearCopyItem');
  end;
end;
...
begin
  ...
  //¼ì²âÈËÎï°ü¹üÊÇ·ñ´æÔÚ¶¾,»¤Éí·û
  if m_ItemList.Count > 0 then begin//20080628
    for I := 0 to m_ItemList.Count - 1 do begin //ÈËÎï°ü¹ü²»Îª¿Õ
      UserItem := m_ItemList.Items[I];
    ...
    on E: Exception do begin
      MainOutMessage('{Òì³£} THeroObject.CheckHeroAmulet');
    end;
  end;
end;
...
begin
  Result:=False;
  Idx:=0;
  if m_UseItems[U_ARMRINGL].wIndex > 0 then begin       //ÆÈÂîÀÚ¸®
    AmuletStdItem:=UserEngine.GetStdItem(m_UseItems[U_ARMRINGL].wIndex);
    if (AmuletStdItem <> nil) and (AmuletStdItem.StdMode = 25) then begin
      case nType of
        1: begin       //ºÎÀû
  ...
  if m_UseItems[U_BUJUK].wIndex > 0 then begin    //ºÎÀû ÀÚ¸®
    AmuletStdItem:=UserEngine.GetStdItem(m_UseItems[U_BUJUK].wIndex);
    if (AmuletStdItem <> nil) and (AmuletStdItem.StdMode = 25) then begin
      case nType of    //
        1: begin
  ...
  if (not Result) and (mode = 1) then begin
    if (nType = 1) and (GetTickCount - m_BujukUseTick > 10 * 1000) then begin
       SysMsg('Ó¢ÐÛÖ»ÓÐ×°±¸»¤Éí·ûÊ±²ÅÄÜÊ¹ÓÃ´Ë¼¼ÄÜ¡£',c_Red,t_Hint);  m_BujukUseTick := GetTickCount;
    end;
    if (nType = 2) and (GetTickCount - m_PosionUseTick > 10 * 1000) then begin
       SysMsg('Ó¢ÐÛÖ»ÓÐ×°±¸¶¾·ÛÊ±²ÅÄÜÊ¹ÓÃ´Ë¼¼ÄÜ¡£',c_Red,t_Hint); m_PosionUseTick := GetTickCount;
    end;
    if (nType in [5,7,8]) and (GetTickCount - m_PosionUseTick > 10 * 1000) then begin
       SysMsg('Ó¢ÐÛÖ»ÓÐ×°±¸¶¾·û°üÊ±²ÅÄÜÊ¹ÓÃ´Ë¼¼ÄÜ¡£',c_Red,t_Hint); m_PosionUseTick := GetTickCount;
    end;
  end;
end;
        ...
        case StdItem.Shape of
          ...
          14: begin     //ºÎÈ°ÁÖ¹®¼­
            if m_boDeath then begin
              if (Not m_PEnvir.Flag.boNOREVIVAL) then begin
                ...
                SysMsg('ÎÞ·¨Ê¹ÓÃ¡£',c_RedWhite,t_Hint);
              end;
            end;
          end;
          15: begin     //À½¾çº¯È¯¼­
            if not m_boDeath then begin
              if m_UseItems[U_DRESS].wIndex = 0 then begin
                ...
                FeatureChanged();
                SysMsg('Ó¢ÐÛµÄÐÔ±ð±äÁË¡£',c_Green,t_Hint);
              end else begin
                Result := False;
                SysMsg('´©×Å¿ø¼×Ê±²»ÄÜ×ª»»ÐÔ±ð¡£',c_Red,t_Hint);
              end;
            end;
          end;
          16: begin       //°æÇèÄ¡°¡»ê
            if not m_boDeath then begin
              ...
              // SysMsg(format(g_sChangeKillMonExpRateMsg,[m_nKillMonExpRate2 / 100,m_dwKillMonExpRateTime2]),c_Green,t_Hint);
               SysMsg('Ó¢ÐÛ»ñµÃµÄ¾­ÑéÖµÔö¼ÓÁË',c_GreenWhite,t_Hint);
               SendMsg (Self,RM_BUFF, 8, 0, 300, StdItem.DuraMax * 3600 , '¾­ÑéÖµ + ' + IntToStr(nRate - 100) +'%\³ÖÐø: ' );    //°æÇèÄ¡°¡»ê ¹öÇÁ
            end;
          end;
          22: begin     //°ø·Â¼ö
            if not m_boDeath then begin
              ...
              SysMsg('Ó¢ÐÛµÄ¹¥»÷Á¦Ôö¼ÓÁË¡£',c_GreenWhite,t_Hint);
              SendMsg (Self,RM_BUFF, 49, 0, 306, StdItem.DuraMax * 3600, '¹¥»÷Á¦ + ' + IntToStr(StdItem.AC - 100) +' \³ÖÐø: ' );    //°ø·Â¼ö ¹öÇÁ
            end;
          end;
          24: begin     //Á¤È®ÀÇ¹°¾à
            if not m_boDeath then begin
              ...
              SysMsg('Ó¢ÐÛµÄ×¼È·Ôö¼ÓÁË¡£',c_GreenWhite,t_Hint);
              SendMsg (Self, RM_BUFF, 17, 0, 320, StdItem.DuraMax * 3600, format('×¼È· + %d: ',[StdItem.AC]));    //Á¤È®ÀÇ¹°¾à ¹öÇÁ
            end;
          end;

          25: begin     //¹Ì¿ë»ç¸¶À½´ë·Î1
            if not m_boDeath then begin
              m_btHair := Random(3);
              Case m_btHair of
                 0: Str := '¹âÍ·';
              ...
              Result := True;
              FeatureChanged();
              SysMsg('Ó¢ÐÛµÄ·¢ÐÍ±äÎª ' + Str + '¡£',c_Green,t_Hint);
            end;
          end;
          26: begin     //¹Ì¿ë»ç¸¶À½´ë·Î2
            if not m_boDeath then begin
              m_btHair := 1 + Random(8);
              Case m_btHair of
                 1: Str := '¶Ì·¢';
          ...
          29: begin  //µå¶ø¹æÁö ¹°¾à
            if not m_boDeath then begin
              if StdItem.AniCount > 0 then begin
                ...
                SendMsg(Self, RM_BUFF, 121, 0, 301, StdItem.DuraMax * 3600, '[ ·ÀÖ¹±¬³ö ]\³ÖÐø: ');
              end;
              Result := True;
              RecalcAbilitys();
            end;
          ...
          30: begin //¼ö·ÃÀÇ¹°¾à
            if not m_boDeath then begin
              ...
              SysMsg('Ó¢ÐÛµÄ¼¼ÄÜÊìÁ·¶ÈÔö¼ÓÁË¡£',c_GreenWhite,t_Hint);
              RecalcAbilitys();
              SendMsg (Self,RM_BUFF, 53, 0, 318, StdItem.DuraMax * 3600, '¾­ÑéÖµ±¶Êý2±¶: ');   //¼ö·ÃÀÇ¹°¾à ¹öÇÁ
            end;
          end;

          4: begin           //Ãàº¹ÀÇ ±â¸§ Ãà±â
            if not m_boDeath then
            if WeaptonMakeLuck() then Result:=True;
          end;
          ...
          if not m_boDeath then begin
            if StdItem.DC > 0 then begin
              ...
              if StdItem.MAC2 >= 3600 then
                SysMsg('Ó¢ÐÛµÄÁÙÊ±¹¥»÷Ôö¼Ó' + IntToStr(StdItem.MAC2 div 3600) + 'Ð¡Ê±¡£',c_Green,t_Hint)
              else
                SysMsg('Ó¢ÐÛµÄÁÙÊ±¹¥»÷Ôö¼Ó' + IntToStr(StdItem.MAC2 div 60) + '·ÖÖÓ¡£',c_Green,t_Hint) ;
              SendMsg (Self,RM_BUFF, 3, 0, 307, StdItem.MAC2 , format('¹¥»÷ + %d\³ÖÐø: ' ,[StdItem.DC]) );   //ÆÄ±«ÀÇ¹°¾à ¹öÇÁ
              boNeedRecalc:=True;
            end;
            if StdItem.MC > 0 then begin
              ...
              if StdItem.MAC2 >= 3600 then
                SysMsg('Ó¢ÐÛµÄÁÙÊ±Ä§·¨Ôö¼Ó' + IntToStr(StdItem.MAC2 div 3600) + 'Ð¡Ê±¡£',c_Green,t_Hint)
              else
                SysMsg('Ó¢ÐÛµÄÁÙÊ±Ä§·¨Ôö¼Ó' + IntToStr(StdItem.MAC2 div 60) + '·ÖÖÓ¡£',c_Green,t_Hint)  ;
              SendMsg (Self,RM_BUFF, 5, 0, 308, StdItem.MAC2 , format('Ä§·¨ + %d\³ÖÐø: ' ,[StdItem.MC]) );  //¸¶·ÂÀÇ¹°¾à ¹öÇÁ
              boNeedRecalc:=True;
            end;
            if StdItem.SC > 0 then begin
              ...
              if StdItem.MAC2 >= 3600 then
                SysMsg('Ó¢ÐÛµÄÁÙÊ±µÀÊõÔö¼Ó' + IntToStr(StdItem.MAC2 div 3600) + 'Ð¡Ê±¡£',c_Green,t_Hint)
              else
                SysMsg('Ó¢ÐÛµÄÁÙÊ±µÀÊõÔö¼Ó' + IntToStr(StdItem.MAC2 div 60) + '·ÖÖÓ¡£',c_Green,t_Hint);
              SendMsg (Self,RM_BUFF, 6, 0, 309, StdItem.MAC2 , format('µÀÊõ + %d\³ÖÐø: ' ,[StdItem.SC]) );  //µµ·ÂÀÇ¹°¾à ¹öÇÁ
              boNeedRecalc:=True;
            end;
            if StdItem.AC2 > 0 then begin
              ...
              if StdItem.MAC2 >= 3600 then
                SysMsg('Ó¢ÐÛµÄÁÙÊ±¹¥»÷ËÙ¶ÈÔö¼Ó' + IntToStr(StdItem.MAC2 div 3600) + 'Ð¡Ê±¡£',c_Green,t_Hint)
              else
                SysMsg('Ó¢ÐÛµÄÁÙÊ±¹¥»÷ËÙ¶ÈÔö¼Ó' + IntToStr(StdItem.MAC2 div 60) + '·ÖÖÓ¡£',c_Green,t_Hint);
              SendMsg (Self,RM_BUFF, 7, 0, 312, StdItem.MAC2, format('¹¥»÷ËÙ¶È + %d\³ÖÐø: ' ,[StdItem.AC2]) );  //±¤Ç³¹°¾à ¹öÇÁ
              boNeedRecalc:=True;
            end;
            if StdItem.AC > 0 then begin
              ...
              if StdItem.MAC2 >= 3600 then
                SysMsg('Ó¢ÐÛµÄÁÙÊ±ÉúÃüÖµÔö¼Ó' + IntToStr(StdItem.MAC2 div 3600) + 'Ð¡Ê±¡£',c_Green,t_Hint)
              else
                SysMsg('Ó¢ÐÛµÄÁÙÊ±ÉúÃüÖµÔö¼Ó' + IntToStr(StdItem.MAC2 div 60) + '·ÖÖÓ¡£',c_Green,t_Hint);
              SendMsg (Self,RM_BUFF, 10, 0, 310, StdItem.MAC2 , format('MAX HP + %d\³ÖÐø: ' ,[StdItem.AC]) );  //À°Ã¼ ¹öÇÁ
              boNeedRecalc:=True;
            end;
            if StdItem.MAC > 0 then begin
              ...
              if StdItem.MAC2 >= 3600 then
                SysMsg('Ó¢ÐÛµÄÁÙÊ±Ä§·¨ÖµÔö¼Ó' + IntToStr(StdItem.MAC2 div 3600) + 'Ð¡Ê±¡£',c_Green,t_Hint)
              else
                SysMsg('Ó¢ÐÛµÄÁÙÊ±Ä§·¨ÖµÔö¼Ó' + IntToStr(StdItem.MAC2 div 60) + '·ÖÖÓ¡£',c_Green,t_Hint);
              SendMsg (Self,RM_BUFF, 11, 0, 311, StdItem.MAC2 , format('MAX MP + %d\³ÖÐø: ' ,[StdItem.MAC]) );  //Á¤½Å ¹öÇÁ
              boNeedRecalc:=True;
            end;
...
begin
  ...
  if (lwExp >= dwAddExp) and (m_Abil.MaxExp > m_Abil.Exp) then begin   //³²Àº °æÇèÄ¡º¸´Ù µé¾î¿Â °æÇèÄ¡°¡ Å¬°æ¿ì + ÇöÀç °æÇèÄ¡°¡ ÇÊ¿ä°æÇèÄ¡ º¸´Ù ÀÛÀº°æ¿ì
    ...
    if m_Abil.MaxExp > m_Abil.Exp then begin     //ÀÏ¹ÝÀûÀÎ °æÇèÄ¡ µé¾î¿À´ÂºÎºÐ
...
begin
  if m_btStatus = 1 then begin //ÈÞ½Ä»óÅÂÀÏ °æ¿ì
    m_SkillUseTick[0] := GetTickCount;  //¹«°ø¹Ð¸®µíÀÌ ¾²´Â°Å ¹æÁö..
    m_btStatus := 0;
  end else begin
    m_btStatus := 1;
  end;

  m_Master.m_boSlaveRelax := m_btStatus = 1;   //ÈÞ½Ä
  case m_btStatus of
      0: begin
         if m_Master.m_SlaveList.Count < 1 then
          SysMsg('ÏÂÊôÐÐ¶¯: ¹¥»÷', c_Green, t_Hint);
        end;
      1: begin
          m_boProtectStatus := False;
          m_boHeroBackCall := False;
          if m_Master.m_SlaveList.Count < 1 then
          SysMsg('ÏÂÊôÐÐ¶¯: ÐÝÏ¢', c_Green, t_Hint);
      end;
  end;
end;
...
begin
  Result :=m_sCharName +'\'+ m_Master.m_sCharName + 'µÄÓ¢ÐÛ';
end;
  ...
  case nCode of
    0:begin
      if (abs(m_nCurrX - nTargetX) >= 2{1}) or (abs(m_nCurrY - nTargetY) >= 2{1}) then begin
        ...
        Result := WalkToTargetXY2(nTargetX, nTargetY); //×ªÏò
      end;
    end;//0
    1:begin//¶ã±ÜÄ£Ê½
...
function THeroObject.IsNeedGotoXY(): Boolean; //Å¸ÄÏ¿¡°Ô µû¶ó°¡´Â?
var
  dwAttackTime: LongWord;
begin
  Result := False;
  if (m_TargetCret <> nil) and (GetTickCount - m_dwAutoAvoidTick > 1100) then begin    //¿µ¿õÀü»ç ¿µ¿õÀÚ°´
    if m_btJob in [1,2] then begin   //¿µ¿õ¼ú»ç ¿µ¿õµµ»ç
...
begin
  ...
  if ((GetTickCount - m_dwAutoAvoidTick) > 1100) and m_boIsUseMagic then begin
    if (m_btJob in [1,2]) and (m_btStatus <> 1) and ((m_nSelectMagic = 0) or (m_WAbil.HP <= Round(m_WAbil.MaxHP * 0.25))) then begin  //¿µ¿õ¼ú»ç ¿µ¿õµµ»ç
      m_dwAutoAvoidTick := GetTickCount();
      case m_btJob of
        1:if CheckTargetXYCount(m_nCurrX, m_nCurrY, 4) > 0 then begin      //¿µ¿õ¼ú»ç
            Result := True;
            Exit;
          end;
        2:if CheckTargetXYCount(m_nCurrX, m_nCurrY, 4{3}) > 0 then begin     //¿µ¿õµµ»ç
            Result := True;
            Exit;
          end;
      end;
...
begin
  Result := False;
  if (m_TargetCret <> nil) then begin   //Å¸°ÙÀÌ ÀÖ´Ù..
    if (m_TargetCret = Self) then m_TargetCret := nil;       //Å¸°ÙÀÌ ³ª´Ù? //Å¸°ÙÀ» ¾ø¾ÖÁØ´Ù.
  end;
  if (m_TargetCret = nil) and ((GetTickCount - m_dwSearchEnemyTick) > 400) and (m_btStatus = 0) then begin    //Å¸°ÙÀÌ ¾ø´Ù. 0.4ÃÊ¸¶´Ù Å¸°ÙÀ» Ã£À½,, °ø°Ý°¡´É»óÅÂÀÏ¶§
    m_dwSearchEnemyTick := GetTickCount;
    Result := True;
    Exit;
  end;
...
begin
  nLevel := m_Abil.Level;
  case m_btJob of
    3: begin   //¿µ¿õÀÚ°´    THeroObject.RecalcLevelAbilitys();
      ...
      m_Abil.DC:= MakeLong(_MAX((nLevel div 5) - 1, 1), _MAX(1, (nLevel div 5)));      //ÆÄ±«
      m_Abil.SC:= 0;
      m_Abil.MC:= 0;
    ...
    2: begin   //¿µ¿õµµ»ç     THeroObject.RecalcLevelAbilitys();
    ...
    1: begin    //¿µ¿õ¼ú»ç     THeroObject.RecalcLevelAbilitys();
    ...
    0: begin    //¿µ¿õÀü»ç  THeroObject.RecalcLevelAbilitys();
...
begin
  ...
  try
    m_boHeroLogOnOK := True;
    m_dwHeroLogOnTick := GetTickCount();
    m_dLogonTime := Now();                  //°ü¸®ÀÚ½Ã½ºÅÛ
    m_dwLogonTick := GetTickCount();        //°ü¸®ÀÚ½Ã½ºÅÛ
    m_nHeroLogOnCount := 0;
    nError := 1;
    if m_boNewHero then begin
      New(UserItem);
      nError := 3;
      if m_btJob = 4 then begin      //Á÷¾÷º¯°æ
    ...
    RecalcHeroAddAbilitys();
    TPlayObject(m_Master).RecalcHeroAddAbilitys;
    if m_Master.m_boSlaveRelax then m_btStatus := 1;   //ÁÖÀÎÀÌ ÀÌ¹Ì ÈÞ½Ä¸í·ÉÀ» ³»·È´Ù¸é
...
begin
  ...
  try
    ...
    if (m_btRaceServer = RC_HEROOBJECT) then begin
      ...
      if StdItem <> nil then begin
        if ( (StdItem.StdMode = 63) and (StdItem.Shape = 5) ) then begin    //º¹Á¶¸®
          tCode:=1;
          ...
          m_boNoDropBagItem := True;  //°¡¹æÅÛ º¸È£
          tCode:=6;
    ...
    for I := Low(THeroItems) to High(THeroItems) do begin
      if Random(nRate) = 0 then begin
        ...
        StdItem:=UserEngine.GetStdItem(m_UseItems[I].wIndex);
        if StdItem <> nil then begin
          if (m_btRaceServer = RC_HEROOBJECT) and (StdItem.Reserved and 10 = 0) then begin  //Á×¾îµµ ¶³¾îÁöÁö ¾Ê´Â ¾ÆÀÌÅÛÀº Á¦¿Ü(bug¼öÁ¤ sonmg 2006/01/27)
            tCode:=9;
...
begin
  DelList := nil;
  if m_PEnvir.Flag.boNODROPITEM or m_boDropDurg then Exit; //²»ËÀ½äÖ¸
  ...
  try
    ...
    for I := m_ItemList.Count - 1 downto 0 do begin
      ...
      if (pstd.StdMode = 48) then continue;    //»óÇö¾ÆÀÌÅÛ
      tCode:=3;
      if (m_btRaceServer = RC_HEROOBJECT) and ((not pstd.boCanDeathDrop) or (pu.btValue[19] > 0)) then continue;    //±â°£Á¦, ºÀÀÎ
      tCode:=4;
...
procedure THeroObject.LogOut(); //·Î±×¾Æ¿ô
var
  K :Integer;
resourcestring
...
begin
 // UnHeroGroup;
  ...
  for K:=Low(m_dwStatusArrTick) to High(m_dwStatusArrTick) do begin  //½ºÅ×ÀÌÆ® ²¨ÁÖ±â
    if K in [0..5,7..13,15..19,20..26] then begin   //½ºÅ×ÀÌÆ®
...
procedure THeroObject.RecalcHeroAddAbilitys();//¿µ¿õ ¹öÇÁ
var
  str:string;
  btJob: byte;
  btLevel: Integer;
begin
   if (not m_boDeath) and (not m_boHeroLogOut) then begin
       ...
       0: begin  //¿µ¿õ Àü»ç
           if m_btLimitLevel <> 1 then begin
             if btLevel >= 10 then begin
               m_nPowerRate5 := 100 + btLevel div 10 + (m_nHeroGrade);    //°ø·Â
             end else
             m_nPowerRate5 := 100 + 1 + btLevel div 10;   //°ø°Ý·Â
             m_wStatusArrValue[85] := 5 + (btLevel div 10) * (m_nHeroGrade);  //ÃÖ´ëHP»ó½Â
           end else begin
             m_nPowerRate5 := 100 + 1;
             m_wStatusArrValue[85] := 5;
           end;
           str := str + 'MAXHP + ' + IntToStr(m_wStatusArrValue[85]) + '\¹¥»÷ ' + IntToStr(m_nPowerRate5 - 100);
         end;
       1: begin //¿µ¿õ ¼ú»ç
           if m_btLimitLevel <> 1 then begin
             m_nKillMonExpRate9 := 100 + 1 + btLevel div 10;
             m_wStatusArrValue[86] := 5 + (btLevel div 10) * (m_nHeroGrade);  //ÃÖ´ëMP»ó½Â
           end else begin
             m_nKillMonExpRate9 := 100 + 1;
             m_wStatusArrValue[86] := 5;
           end;
           str := str + 'MAXMP + ' + IntToStr(m_wStatusArrValue[86]) + '\¾­ÑéÖµ + ' + IntToStr(m_nKillMonExpRate9 - 100) + '%';
         end;
       2: begin //¿µ¿õ µµ»ç
           if m_btLimitLevel <> 1 then begin
             if btLevel >= 10 then begin
             ...
             m_nPowerRate5 := 100 + 1 + btLevel div 10;   //°ø°Ý·Â
             m_wStatusArrValue[87] := 5 + (btLevel div 10) * (m_nHeroGrade);  //Ã¼·ÂÈ¸º¹
           end else begin
             m_nPowerRate5 := 100 + 1;
             m_wStatusArrValue[87] := 5;
           end;
           str := str + 'ÉúÃü»Ö¸´ + ' + IntToStr(m_wStatusArrValue[87]) + '%\¹¥»÷ ' + IntToStr(m_nPowerRate5 - 100);
         end;
       3: begin  //¿µ¿õ ÀÚ°´
           if m_btLimitLevel <> 1 then begin
             m_wStatusArrValue[88] := 1 + btLevel div 10;    //Á¤È®
             m_wStatusArrValue[89] := 5 + (btLevel div 10) * (m_nHeroGrade);  //¸¶·ÂÈ¸º¹
           end else begin
             m_wStatusArrValue[88] := 1;
             m_wStatusArrValue[89] := 5;
           end;
           str := str + 'Ä§·¨»Ö¸´ + ' + IntToStr(m_wStatusArrValue[89]) + '%\×¼È· ' + IntToStr(m_wStatusArrValue[88]);
         end;
     end;
     SendMsg(self,RM_BUFF, 115, 0, 44, 1, '[ ¿µ¿õ¹öÇÁ ]\' + str); //¿µ¿õ ¹öÇÁ
   end else begin
     ...
     m_nPowerRate5 := 100;
     m_nKillMonExpRate9 := 100;
     SendMsg(self,RM_BUFF, 115, 0, 44, 0, ''); //¿µ¿õ ¹öÇÁ
   end;
   RecalcAbilitys;
   SendMsg(Self,RM_ABILITY,0,0,0,0,'');
   SendMsg(Self,RM_SUBABILITY,0,0,0,0,'');
...
begin
  ...
  case ProcessMsg.wIdent of
    RM_STRUCK: begin//ÊÜÎïÀí´ò»÷
      if (ProcessMsg.BaseObject = Self) and (TBaseObject(ProcessMsg.nParam3 ) <> nil) then begin
        if (TBaseObject(ProcessMsg.nParam3).m_btRaceServer = RC_PLAYOBJECT) then begin//20080531 Ôö¼Ó
          if (not TBaseObject(ProcessMsg.nParam3).InSafeZone) and (not InSafeZone) then begin
             SetLastHiter(TBaseObject(ProcessMsg.nParam3 ));//ÉèÖÃ×îºó´ò»÷×Ô¼ºµÄÈË
        ...
        if (m_Master <> nil) and
          ...
          ((TBaseObject(ProcessMsg.nParam3).m_btRaceServer = RC_PLAYOBJECT) or
           (TBaseObject(ProcessMsg.nParam3).m_btRaceServer = RC_HEROOBJECT)) then begin//Ó¢ÐÛ»ÒÉ« 20080721
           m_Master.SetPKFlag(TBaseObject(ProcessMsg.nParam3));
        end;
        if g_Config.boMonSayMsg then MonsterSayMsg(TBaseObject(ProcessMsg.nParam3), s_UnderFire);
      end;
    ...
    if not IsTrainingSkill(Magic.wMagicId) then begin
      if (Magic.btJob = 99) or (Magic.btJob = m_btJob) then begin
        if m_Abil.Level >= Magic.TrainLevel[0] then begin
          ...
          SysMsg('Ó¢ÐÛÒÑ¾­Ñ§¹ý¼¼ÄÜ' + Magic.sMagicName + 'ÁË¡£', c_Green, t_Hint);
          Result := True;
        end;
      end; //else SysMsg('(Ó¢ÐÛ) ²»ÄÜÑ§Ï°´Ë¼¼ÄÜ£¡£¡£¡', c_Red, t_Hint);
    end; //else SysMsg('(Ó¢ÐÛ) ÒÑ¾­Ñ§¹ý´Ë¼¼ÄÜ,²»ÄÜÔÙÑ§Ï°£¡£¡£¡', c_Red, t_Hint);
  end;
end;
...
begin
  Result := False;
  if m_boTransparent and (m_boHideMode) then m_wStatusTimeArr[STATE_TRANSPARENT ] := 1;//20080902 ÒþÉí,Ò»¶¯¾ÍÏÔÉí
  if (m_wStatusTimeArr[POISON_STONE] <> 0) and (not g_Config.ClientConf.boParalyCanSpell) then Exit;//Âé±Ô²»ÄÜÅÜ¶¯
...
begin
  ...
  try
    if (not m_boHeroLogOut) and CanMove and (not m_boFixedHideMode) and (not m_boStoneMode) then begin
      ...
      ItemTimeRun;
      nCheckCode:= 13;
      if m_boFireHitSkill and ((GetTickCount - m_dwLatestFireHitTick) > 20000) then begin     //¿°È­°á ÇØÁ¦..
        m_boFireHitSkill := False;
        SysMsg(sSpiritsGone, c_Red, t_Hint);
        SendSocket(nil, '+UFIR');
      end;

      if m_boTwinHitSkill = 2 then begin  //½Ö·æÂü ÇØÁ¦
        m_boTwinHitSkill := 0;
        SendSocket(nil,'+UTWN');
      end;
      ...
      if GetTickCount - m_dwRateTick > 1000 then begin
        ...
        if m_dwKillMonExpRateTime > 0 then begin
          ...
          if m_dwKillMonExpRateTime = 0 then begin
            m_nKillMonExpRate := 100;
            SysMsg('Ó¢ÐÛµÄ¾­ÑéÖµ»Ö¸´Õý³£¡£', c_Green, t_Hint);
          end;
        end;
        if m_dwPowerRateTime > 0 then begin
          ...
          if m_dwPowerRateTime = 0 then begin
            m_nPowerRate := 100;
            SysMsg('Ó¢ÐÛµÄ¹¥»÷Á¦»Ö¸´Õý³£¡£', c_Green, t_Hint);
          end;
        end;
      ...
      if IsSearchTarget then SearchTarget(); //Å¸°Ù Ã£±â
      ...
      if not m_boWalkWaitLocked and ((GetTickCount - m_dwWalkTick) > GetWalkSpeed) then begin
        ...
        if not m_boRunAwayMode then begin
          if not m_boNoAttackMode then begin
            ...
            if (m_TargetCret <> nil) and ((m_btAiMode in [0,1]) or (m_boTarget and (m_btAiMode = 2))) and (not m_boProtectStatus) and (not m_boHeroBackCall) then begin
              ...
              if IsNeedAvoid then begin //°ø°ÝÇÏ¸ç ¿òÁ÷ÀÎ´Ù. ¸÷ÀÌ ¸¹¾ÆÁö¸é?
                nCheckCode:= 71;
                AutoAvoid();
                inherited Run;
                Exit;
                ...
                nCheckCode:= 112;
                if IsNeedGotoXY then begin//Å¸ÄÏ¿¡°Ô µû¶ó°¡´Â?
                  nCheckCode:= 72;
                  m_nTargetX:= m_TargetCret.m_nCurrX;
                  m_nTargetY:= m_TargetCret.m_nCurrY;

                  if (m_btJob in [1, 2]) then begin  //¿µ¿õ¼ú»ç ¿µ¿õµµ»ç 3Ä­µÚ·Î
                    if (m_Abil.Level < 22) then begin//¼ú»çµµ»ç´Â ·¹º§ 22 ÀÌÈÄ¿¡.. 3Ä­µÚ·Î ÀÌµ¿ÇÑ´Ù
                    end else GetGotoXY(m_TargetCret, 3);
                  end else begin
                    GotoTargetXY( m_nTargetX, m_nTargetY, 1);  //Å¸°Ù¿¡°Ô °¡´Â°Í..
                  end;
                  inherited Run;
                  Exit;
                 end;
          ...
          if m_Master <> nil then begin
            if m_boHeroBackCall then begin //Åð°¢
              m_Master.GetBackPosition(nX, nY);
              if (abs(m_nCurrX - nX) > 1) or (abs(m_nCurrY - nY) > 1) then begin//¿µ¿õ°ú ÁöÁ¤ÇÑ °Å¸®°¡ 9ÀÌ»ó?
                m_nTargetX := nX;
                m_nTargetY := nY;
                m_TargetCret := nil;
              end else begin
            ...
            if m_boProtectStatus then begin //°­Á¦ÀÌµ¿ÀÏ¶§
              if (abs(m_nCurrX - m_nProtectTargetX) > 2) or (abs(m_nCurrY - m_nProtectTargetY) > 2) then begin//¿µ¿õ°ú ÁöÁ¤ÇÑ °Å¸®°¡ 9ÀÌ»ó?
            ...
            end else begin        //Áö¿ª¹æ¾î°¡ ¾Æ´Ò¶§
              if (m_TargetCret = nil) and (not m_boProtectStatus) and (not m_boHeroBackCall) and (m_btStatus <> 1) then begin      //ÈÞ½Ä¸ðµå ¾Æ´Ò¶§
                nCheckCode:= 95;
                m_Master.GetBackPosition(nX, nY);
                if (abs(m_nTargetX - nX) > 2{2}) or (abs(m_nTargetY - nY ) > 2{2}) then begin//ÁÖÀÎµÚ·Î...°¡´Â
                  m_nTargetX := nX;
                  m_nTargetY := nY;
            ...
            if (not m_boProtectStatus) and (not m_boHeroBackCall) and (m_btStatus <> 1) and //ÈÞ½Ä¸ðµå°¡ ¾Æ´Ï¸ç
              ((m_PEnvir <> m_Master.m_PEnvir) or
              (abs(m_nCurrX - m_Master.m_nCurrX) > 10) or
              (abs(m_nCurrY - m_Master.m_nCurrY) > 10)) then begin     //ÁÖÀÎ°ú °°Àº¸Ê¿¡ ¾øÀ¸¸ç 10Ä­ ¹ú¾îÁ³À»¶§
              nCheckCode:= 99;
              SpaceMove(m_Master.m_PEnvir.sMapName, m_nTargetX, m_nTargetY, 1);
            end;
            if (m_TargetCret <> nil) then begin  //Å¸°ÙÀÌ ÀÖ´Ù.
              if (m_TargetCret.m_PEnvir <> m_PEnvir) then DelTargetCreat();  //Å¸°Ù°ú ´Ù¸¥¸ÊÀÌ¸é Å¸°ÙÀ» ¾ø¾ÖÁØ´Ù.
            end;
        ...
        nCheckCode:= 10;
        if (m_TargetCret = nil) and (m_btStatus <> 1) then begin     //ÈÞ½Ä¸ðµå ¾Æ´Ï¸ç Å¸°ÙÀÌ ¾øÀ»¶§
      ...
      if (not m_boGhost) and m_boDeath then begin
        if GetTickCount - m_dwDeathTick > 1000 * 30 then begin  //Á×°í³ª¼­ 30ÃÊÈÄ »ç¶óÁø´Ù..
          LogOut;
        end;
      end;
    end;
...
begin
  ...
  if boRepair then begin
    SysMsg('Ó¢ÐÛµÄËùÓÐ×°±¸¶¼ÒÑ¾­ÐÞÀíÍê±Ï¡£', c_Green, t_Hint); //20071229
  end;
end;
...
begin
  ...
  if nDura > 0 then begin
    ...
    if m_btRaceServer = RC_HEROOBJECT then begin
      ...
      SysMsg('Ó¢ÐÛµÄÎäÆ÷ÒÑ±»ÐÞ¸´¡£', c_Green, t_Hint);
    end;
    Result := True;
  end;
end;
...
begin
  ...
  if (Item.Reserved and 16 <> 0) then begin
    ...
    if (self.m_btJob in [g_Bonze,g_BonzeUp]) then begin         //½Â·Á
  ...
  if (Item.Reserved and 32 <> 0) then begin
    ...
    if (self.m_btJob in [g_Monk,g_MonkUp]) then
      UserItem.wIndex:=UserEngine.GetStdItemIdx(stditemname + '3');
    if (self.m_btJob in [g_Assassin,g_AssassinUp]) then                     //ÀÚ°´
      UserItem.wIndex:=UserEngine.GetStdItemIdx(stditemname + '4');
    if (self.m_btJob in [g_Bonze,g_BonzeUp]) then                         //½Â·Á
      UserItem.wIndex:=UserEngine.GetStdItemIdx(stditemname + '5');
    Item:=UserEngine.GetStdItem(UserItem.wIndex);
  end;
...
procedure THeroObject.SendAddItemMode(UserItem:pTUserItem;idx:integer);      //¹°¾à2
var
  Item: TItem;
  ClientItem:TClientItem;
  sUserItemName: string;
...
procedure THeroObject.SendAddStorage(UserItem:pTUserItem); //004D0824      //¿©°ü  //Ã¢°í
var
  Item:TItem;
  StdItem:TStdItem;
  ClientItem:TClientItem;
...
procedure THeroObject.SendAddStorageMode(UserItem:pTUserItem;idx:integer);      //¹°¾à2
var
  Item:TItem;
  StdItem:TStdItem;
  ClientItem:TClientItem;
...
procedure THeroObject.SendAddAutoItemMode(UserItem:pTUserItem);      //¹°¾à2
var
  StdItem: TItem;
  ClientItem:TClientItem;
  opt: integer;
...
begin
  if (not m_boTarget) and (m_btAiMode <> 2) then begin  //´ë±â»óÅÂ°¡ ¾Æ´Ï¿©¾ßÇÔ,,
...
var
  ...
  I, nC, n10: Integer;
begin
  if (m_TargetCret = nil) and m_boTarget then m_boTarget := False;    //°­Á¦°ø°Ý ¸í·ÉÀ» ³»·È´Âµ¥ Å¸°ÙÀÌ ¾øÀ¸¸é ²¨ÁÖ±â.
  if (m_TargetCret <> nil) and m_boTarget then begin    //°­Á¦°ø°Ý ¸í·ÉÁß Å¸°ÙÀÌ ÀÖ´Ù.
    if m_TargetCret.m_boDeath then m_boTarget := False;    //Å¸°ÙÀÌ Á×¾ú´Ù¸é ²¨ÁØ´Ù.
  end;
  ...
  if (m_btStatus = 0) and (m_btAiMode = 0) and (not m_boProtectStatus) and (not m_boTarget) and (GetTickCount - m_dwCallTargetTick > 2500)  {and (not m_boSpellTarget)} then begin  //°ø°Ý°¡´É»óÅÂ¿¡¼­ °­Á¦¸í·ÉÀÌ ¾ø´Ù¸é.
    BaseObject18 := nil;
    n10 := 15;  //¹üÀ§ 15Ä­??
    for I := 0 to m_VisibleActors.Count - 1 do begin
      BaseObject := TBaseObject(pTVisibleBaseObject(m_VisibleActors.Items[I]).BaseObject);
    ...
    if BaseObject18 <> nil then begin
      SetTargetCreat(BaseObject18);     //Å¸°ÙÀ¸·Î ÁöÁ¤
    end;
  end;
end;
  ...
  try
    if m_VisibleItems.Count > 0 then begin//20080629
      for I := 0 to m_VisibleItems.Count - 1 do begin
        ...
        VisibleMapItem:= pTVisibleMapItem(m_VisibleItems.Items[I]);//20080526 Ôö¼Ó
        if VisibleMapItem <> nil then VisibleMapItem.nVisibleFlag := 0;//20080526 Ôö¼Ó
      end;
    end;
  except
  ...
  try
    if m_VisibleActors.Count > 0 then begin//20080629
      for I := 0 to m_VisibleActors.Count - 1 do begin
        ...
        VisibleBaseObject:= pTVisibleBaseObject(m_VisibleActors.Items[I]); //20080526 Ôö¼Ó
        if VisibleBaseObject <> nil then VisibleBaseObject.nVisibleBFlag := 0;//20080526 Ôö¼Ó
      end;
    end;
  except
    MainOutMessage(Format('THeroObject::SearchViewRange Code:%d', [nCheckCode]));
      ...
      for n1C := nStartY to nEndY do begin
        ...
        if m_PEnvir.GetMapCellInfo(n18, n1C, MapCellInfo) and (MapCellInfo.ObjList <> nil) then begin
          ...
          while (True) do begin
            ...
            nCheckCode := 51;
            if OSObject = nil then begin//´¦ÀíÒì³£ ´«Ææ´úÂë 20080916
            ...
            if OSObject <> nil then begin
              ...
              nCheckCode := 6;
              if btType = OS_MOVINGOBJECT then begin//20081017 ÐÞ¸Ä
                if (GetTickCount - OSObject.dwAddTime) >= 60 * 1000 then begin
                  nCheckCode := 7;
                  MapCellInfo.ObjList.Delete(nIdx);//20080916 ÐÞ¸Ä
                  DisPoseAndNil(OSObject);
                  if MapCellInfo.ObjList.Count > 0 then Continue;//20080916 ÐÞ¸Ä
                  FreeAndNil(MapCellInfo.ObjList);
                  Break;
                end;
                nCheckCode := 8;
                ...
                if BaseObject <> nil then begin
                  if (not BaseObject.m_boGhost) and (not BaseObject.m_boFixedHideMode) and (not BaseObject.m_boObMode) then begin
                    if (m_btRaceServer < RC_ANIMAL) or (m_Master <> nil) or m_boCrazyMode or m_boWantRefMsg or
                      ((BaseObject.m_Master <> nil) and (abs(BaseObject.m_nCurrX - m_nCurrX) <= 6) and (abs(BaseObject.m_nCurrY - m_nCurrY) <=  6)) or//20080815 ÓÉ3¸ñ±ä6¸ñ(ÐÞÕý,³àÔÂ¶ñÄ§3¸ñºó,¾Í²»ÄÜ¹¥»÷Ó¢ÐÛ)
              ...
              //Ôö¼Ó·ÖÉíÈËËÑË÷µØÃæÎïÆ·  20080707 ×¢ÊÍ
              nCheckCode := 10;
              if (m_btRaceServer = RC_HEROOBJECT) and (m_Master <> nil) then begin //20080729 ÐÞ¸Ä
                if (OSObject <> nil) and (OSObject.btType = OS_ITEMOBJECT) then begin
                  ...
                  if (GetTickCount - OSObject.dwAddTime) > g_Config.dwClearDropOnFloorItemTime {60 * 60 * 1000} then begin
                    Dec(m_PEnvir.m_nItemCount);           //°ü¸®ÀÚ½Ã½ºÅÛ
                    Dec(g_MapManager.m_nItemCount);       //°ü¸®ÀÚ½Ã½ºÅÛ
                    if PTMapItem(OSObject.CellObj) <> nil then DisPoseAndNil(PTMapItem(OSObject.CellObj)); //·ÀÖ¹Õ¼ÓÃÄÚ´æ²»ÊÍ·ÅÏÖÏó 20080702 ÐÞ¸Ä
                    DisPoseAndNil(OSObject);
                    MapCellInfo.ObjList.Delete(nIdx);
                    if MapCellInfo.ObjList.Count > 0 then Continue;//20080916 Ôö¼Ó
                    FreeAndNil(MapCellInfo.ObjList);
                    Break;
                  end;
  ...
  try
   ...
   n18 := 0;
    while (True) do begin
      ...
      nCheckCode := 17;
      if VisibleMapItem = nil then begin//20080526 Ôö¼Ó
      ...
      if {VisibleMapItem.nVisibleFlag}nVisibleFlag = 0 then begin//20080901 ÐÞ¸Ä
        nCheckCode := 19;
        m_VisibleItems.Delete(n18);
        nCheckCode := 20;
    ...
    while (True) do begin
      ...
      nCheckCode := 23;
      if VisibleBaseObject = nil then begin //20080526 Ôö¼Ó
      ...
      if {VisibleBaseObject.nVisibleFlag}nVisibleFlag = 0 then begin//20080901 ÐÞ¸Ä
        nCheckCode := 25;
        m_VisibleActors.Delete(n18);
        nCheckCode := 26;
...
begin
  Result := 0;
  case m_btJob of               //Á÷¾÷º¯°æ
    0: begin     //¿µ¿õÀü»ç    SelectMagic
        if (m_TargetCret <> nil) then begin
          if AllowUseMagic(12) then begin
            if GetAttackDir(m_TargetCret, 1, btDir) and GetAttackDir2(2) and ((GetTickCount - m_SkillUseTick[12]) > 1 * 1000) then begin //¾î°Ë¼ú
              if m_boUseThrusting then begin
                Result := 12;
                Exit;
              end;
            end;
            if GetAttackDir(m_TargetCret, 2, btDir) and ((GetTickCount - m_SkillUseTick[12]) > 1 * 1000) then begin //¾î°Ë¼ú
          ...
          if AllowUseMagic(130) and (GetTickCount - m_SkillUseTick[130] > 1000 * 20) then begin  //¸¶Ç÷±¤¼¶
          ...
          if AllowUseMagic(132) and (GetTickCount - m_SkillUseTick[132] > 1000 * 15) then begin      //È¸Ãµ°Ý
            if (CheckTargetXYCount(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, 3) >= 3) then begin  //È¸Ãµ°Ý Å¸°ÙÁÖÀ§ 3 Àû 5¸í ÀÌ»ó
              Result := 132; //È¸Ãµ°Ý
              Exit;
            end;
          end;

          if AllowUseMagic(56) and (GetTickCount - m_SkillUseTick[56] > 1000 * 10) then begin  //ÀÏ¼¶
            nDirRangeTargetCount := GetRangeTargetCountByDir(m_btDirection, m_nCurrX, m_nCurrY, 1);
            if (nDirRangeTargetCount >= 1) then begin
              Result := 56;
              Exit;
          ...
          nRangeTargetCount := GetNearTargetCount(1);     //³» ÁÖº¯Å¸°Ù
          if (nRangeTargetCount >= 2) then begin //¿µ¿õ ³­È­Ç÷Ç³Âü
          ...
          if (Random(6) = 0) and AllowUseMagic(26) and ((GetTickCount - m_dwLatestFireHitTick) > 10 * 1000) and ((GetTickCount - m_SkillUseTick[26]) > 3 * 1000) then begin //¿µ¿õ ¿°È­°á
            AllowFireHitSkill;
            Result := 26;
            Exit;
          end;
          ...
          if (Random(6) = 0) and AllowUseMagic(7) then begin //¿µ¿õ ¿¹µµ°Ë¹ý
            m_boPowerHit := True;
            Result := 7;
            Exit;
          end;

          if(Random(6) = 0) and AllowUseMagic(38) and ((GetTickCount - m_SkillUseTick[38]) > 6 * 1000) and ((GetTickCount - m_SkillUseTick[26]) > 3 * 1000) then begin //¿µ¿õ ½Ö·æÂü
            AllowTwinHitSkill;
            Result := 38;
            Exit;
          end;
          ...
          nRangeTargetCount := GetNearTargetCount(1);     //³» ÁÖº¯Å¸°Ù
          if (nRangeTargetCount >= 2) then begin //¿µ¿õ ¹Ý¿ù°Ë¹ý
    ...
    1: begin //¿µ¿õ¼ú»ç    SelectMagic
        ...
        nRangeTargetCount := GetNearTargetCount(1);     //³» ÁÖº¯Å¸°Ù
        if AllowUseMagic(133) and (GetTickCount - m_SkillUseTick[133] > 1000 * 7) then begin      //È­·æÁøÇ³
          if (nRangeTargetCount >= 2) then begin  //È­·æÁøÇ³ ÁÖÀ§ 3 Àû 2¸í ÀÌ»ó
            Result := 133; //È­·æÁøÇ³
            Exit;
          end;
        end;
        ...
        if AllowUseMagic(134) and (GetTickCount - m_SkillUseTick[134] > 1000 * 10) then begin      //Ãµ»óÆø¿°
          if (CheckTargetXYCount(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, 3) >= 3) then begin  //Ãµ»óÆø¿° Å¸°ÙÁÖÀ§ 3 Àû 5¸í ÀÌ»ó
            Result := 134; //Ãµ»ó³«¿°
            Exit;
          end;
        end;

        if AllowUseMagic(135) and (GetTickCount - m_SkillUseTick[135] > 1000 * 15) then begin      //³«Ãµ·Ú
          if (CheckTargetXYCount(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, 3) >= 2) then begin  //³«Ãµ·Ú Å¸°ÙÁÖÀ§ 3 Àû 2¸í ÀÌ»ó
            Result := 135; //³«Ãµ·Ú
            Exit;
          end;
        end;

        if AllowUseMagic(53) and (GetTickCount - m_SkillUseTick[53] > 1000 * 5) then begin      //Ãµ»ó³«¿°
          if (CheckTargetXYCount(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, 3) >= 2) then begin  //Ãµ»ó³«¿° Å¸°ÙÁÖÀ§ 3 Àû 5¸í ÀÌ»ó
            Result := 53; //Ãµ»ó³«¿°
            Exit;
          end;
        end;
        ...
        if CheckTargetXYCount(m_nCurrX, m_nCurrY, 2) > 1 then begin //¹üÀ§ 2 Àû 1¸í ÀÌ»ó
          if AllowUseMagic(22) and (GetTickCount - m_SkillUseTick[22] > 10000) and (g_EventManager.GetRangeEvent(m_PEnvir, Self, m_nCurrX, m_nCurrY, 6, ET_FIRE) <> 0) then begin
            if (m_TargetCret.m_btRaceServer <> 101) and (m_TargetCret.m_btRaceServer <> 102) and (m_TargetCret.m_btRaceServer <> 104) then begin//³ý×æÂê¹Ö,²Å·Å»ðÇ½ 20081217
          ...
          case Random(3) of
            ...
            1: begin
                ...
                if AllowUseMagic(33) and (CheckTargetXYCount(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, 3) > 1) then begin//»ðÇòÊõ,´ó»ðÇò,µØÓü»ð,±¬ÁÑ»ðÑæ,±ùÅØÏø  ´Ó¸ßµ½µÍÑ¡Ôñ
                  Result := 33;//ºù¼³Ç³
                  Exit;
                end else
                if AllowUseMagic(11) then begin
            ...
            2:begin
                ...
                if AllowUseMagic(33) and (CheckTargetXYCount(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, 3) > 1) then begin  //»ðÇòÊõ,´ó»ðÇò,µØÓü»ð,±¬ÁÑ»ðÑæ ´Ó¸ßµ½µÍÑ¡Ôñ
                  Result := 33; //ºù¼³Ç³
                  Exit;
                end else
                if AllowUseMagic(5) then begin
          ...
          if CheckTargetXYCountOfDirection(m_nTargetX, m_nTargetY, m_btDirection, 3) = 1 then begin     //¹æÇâ¿¡ µû¸¥ °ø°Ý?
             if AllowUseMagic(22) and (GetTickCount - m_SkillUseTick[22] > 10000) and (g_EventManager.GetRangeEvent(m_PEnvir, Self, m_nCurrX, m_nCurrY, 6, ET_FIRE) = 0) then begin
               if (m_TargetCret.m_btRaceServer <> 101) and (m_TargetCret.m_btRaceServer <> 102) and (m_TargetCret.m_btRaceServer <> 104) then begin//³ý×æÂê¹Ö,²Å·Å»ðÇ½ 20081217
                 m_SkillUseTick[22] := GetTickCount;
                 Result := 22;  //Áö¿°¼ú
        ...
        if AllowUseMagic(33) then begin//ºù¼³Ç³
          Result := 33;
          Exit;
        end;
        if AllowUseMagic(32) and (m_TargetCret.m_Abil.Level < g_Config.nMagTurnUndeadLevel) and
        (m_TargetCret.m_btLifeAttrib = LA_UNDEAD) and (m_TargetCret.m_WAbil.Level < m_WAbil.Level -1) then begin //Ä¿±êÎª²»ËÀÏµ
          Result := 32; //»çÀÚÀ±È¸
          Exit;
        end;          
        if AllowUseMagic(24) and (CheckTargetXYCount(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, 3) > 2) then begin//µØÓüÀ×¹â
          Result := 24;
          Exit;
        end;
        if AllowUseMagic(23) then begin//Æø¿­ÆÄ
          Result := 23;
          Exit;
        end;
        if AllowUseMagic(11) then begin//°­°Ý
          Result := 11;
          Exit;
        end;
    ...
    2: begin //¿µ¿õµµ»ç  SelectMagic

          if (m_TargetCret <> nil) and (not m_TargetCret.m_boDeath) and (m_TargetCret <> m_Master) then begin
            if AllowUseMagic(6) and (GetTickCount - m_SkillUseTick[6] > 1000 * 15) then begin   //¾Ï¿¬¼ú
              if CheckHeroAmulet2(1, 5, 6, 1, nAmuletIdx) then begin    //¾Ï¿¬¼ú
            ...
            if AllowUseMagic(13) and (GetTickCount - m_SkillUseTick[13] > 1800) then begin
              if CheckHeroAmulet2(1, 5, 13, 1, nAmuletIdx) then begin //Æø»ì°è
                Result := 13;
                Exit;
              end;
            end;
            if AllowUseMagic(28) and (GetTickCount - m_SkillUseTick[28] > 1000 * 10) and   //Å½±âÆÄ¿¬
              (not m_TargetCret.m_boShowHP) and (m_TargetCret.m_Abil.Level > 99) then begin
              if CheckHeroAmulet2(1, 5, 28, 1, nAmuletIdx) then begin  //Å½±âÆÄ¿¬
                Result := 28;
                Exit;
              end;
            end;
          ...
          if m_WAbil.HP <= m_WAbil.MaxHP * 70 div 100 then begin
            if GetTickCount - m_SkillUseTick[29] > 1000 * 2 then begin
              if m_nIncSelfHealthCount <= 3 then begin
                if AllowUseMagic(2) and AllowUseMagic(29) then begin      //¿µ¿õÈ¸º¹¼ú ´ëÈ¸º¹¼ú
                  if GetRangeFriendCount(m_nCurrX, m_nCurrY, 3) > 1 then begin    //»ç¶÷ÀÌ 1¸í ÀÌ»ó ´ëÈ¸º¹¼ú
                    m_boSelSelf := True;
                    Inc(m_nIncSelfHealthCount);
                    Result := 29;
                    m_SkillUseTick[29] := GetTickCount;
                  end else begin                    //È¥ÀÚ È¸º¹¼ú
                    m_boSelSelf := True;
                    Inc(m_nIncSelfHealthCount);
                    Result := 2;
                ...
                if AllowUseMagic(29) then begin {´ëÈ¸º¹¼ú}
                  m_boSelSelf := True;
                  Inc(m_nIncSelfHealthCount);
                  Result := 29;
                  m_SkillUseTick[29] := GetTickCount;
                ...
                if AllowUseMagic(2) then begin {¿µ¿õÈ¸º¹¼ú}
                  m_boSelSelf := True;
                  Inc(m_nIncSelfHealthCount);
                  Result := 2;
                  m_SkillUseTick[2] := GetTickCount;
          ...
          if AllowUseMagic(83) and (GetTickCount - m_SkillUseTick[83] > 1000 * UseMagicDelay(83)) then begin      //À½¾ç¿ÀÇàÁø
            if (CheckTargetXYCount(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, 5) >= 5) then begin  //À½¾ç¿ÀÇàÁø Å¸°ÙÁÖÀ§ 2 Àû 5¸í ÀÌ»ó
          ...
          if AllowUseMagic(55) and (GetTickCount - m_SkillUseTick[55] > 1000 * 5) then begin      //µ¶¹«
            if (CheckTargetXYCount(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, 3) >= 3) then begin  //µ¶¹« Å¸°ÙÁÖÀ§ 2 Àû 3¸í ÀÌ»ó
              if CheckHeroAmulet2(2, 7, 55, 1, nAmuletIdx) then begin     //µ¶¹«
                Result := 55; //µ¶¹«
                Exit;
              end;
            end;
          ...
          if AllowUseMagic(91) and (GetTickCount - m_SkillUseTick[91] > 1000 * 7) then begin      //µ¶Ä§¼ú
            if (CheckTargetXYCount(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, 3) >= 2) then begin  //µ¶Ä§¼ú Å¸°ÙÁÖÀ§ 2 Àû 3¸í ÀÌ»ó
              if CheckHeroAmulet2(2, 5, 91, 1, nAmuletIdx) then begin  //µ¶Ä§¼ú
                Result := 91; //µ¶Ä§¼ú
                Exit;
              end;
            end;
          ...
          if AllowUseMagic(46) and (GetTickCount - m_SkillUseTick[46] > 1000 * 20) then begin      //ÀúÁÖ¼ú
            if (CheckTargetXYCount(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, 3) >= 2) then begin  //ÀúÁÖ¼ú Å¸°ÙÁÖÀ§ 2 Àû 3¸í ÀÌ»ó
              if CheckHeroAmulet2(1, 5, 46, 1, nAmuletIdx) then begin    //ÀúÁÖ¼ú
                Result := 46; //ÀúÁÖ¼ú
                Exit;
              end;
            end;
          ...
          if (m_Master <> nil) and boVisibleActors then begin
            if AllowUseMagic(15) and (m_Master.m_wStatusTimeArr[STATE_DEFENCEUP] = 0) and (GetTickCount - m_SkillUseTick[15] > 1000 * 3) then begin
              if CheckHeroAmulet2(1, 5, 15, 1, nAmuletIdx) then begin //´ëÁö¿øÈ£
                m_boSelSelf := False;
                Result := 15;
                Exit;
              end;
            ...
            if AllowUseMagic(14) and (m_Master.m_wStatusTimeArr[STATE_MAGDEFENCEUP] = 0) and (GetTickCount - m_SkillUseTick[14] > 1000 * 3) then begin
              if CheckHeroAmulet2(1, 5, 14, 1, nAmuletIdx) then begin //Ç×¸¶Áø¹ý
                m_boSelSelf := False;
                Result := 14;
                Exit;
              end;
            ...
            if (m_Master.m_dwStatusArrTimeOutTick[15] <= 0) and AllowUseMagic(36) and (GetTickCount - m_SkillUseTick[36] > 1000 * 3) then begin
              if CheckHeroAmulet2(1, 5, 36, 1, nAmuletIdx) then begin //¹«±ØÁø±â
                m_boSelSelf := False;
                Result := 36;
                Exit;
              end;
            end;
            if AllowUseMagic(40) and m_Master.CheckPrivateClean and (GetTickCount - m_SkillUseTick[40] > 1000 * 3) then begin //Á¤È­¼ú
              m_boSelSelf := False;
              Result := 40;
              Exit;
            end;
          ...
          if AllowUseMagic(40) and CheckPrivateClean and (GetTickCount - m_SkillUseTick[40] > 1000 * 3) then begin     //Á¤È­¼ú
            m_boSelSelf := True;
            Result := 40;
            Exit;
          end;
    ...
    3: begin     //¿µ¿õÀÚ°´    SelectMagic
          if m_UseItems[U_Weapon].wIndex > 0 then
            StdItem:=UserEngine.GetStdItem(m_UseItems[U_Weapon].wIndex);

          if AllowUseMagic(59) and (m_AssassinHitSkill <> nil) then begin  //Àý¸í°Ë¹ý ¹è¿ü´ÂÁö
          ...
          if AllowUseMagic(139) and (GetTickCount - m_SkillUseTick[139] > 1000 * (6 + UseMagicDelay(139))) then begin     //¿ë¼ÚÀ½
            if (nRangeTargetCount >= 2) then begin //¿µ¿õ ¿ë¼ÚÀ½  º»ÀÎÁÖº¯ ¸÷ÀÌ 3¸¶¸® ÀÌ»ó
              Result := 139;
              Exit;
            end;
          end;

          if AllowUseMagic(80) and (GetTickCount - m_SkillUseTick[80] > 1000 *  (5 + UseMagicDelay(80))) then begin     //¹¦¼³¶õ
            if (CheckTargetXYCount(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, 1) >= 1) then begin  //¿µ¿õ ¹¦¼³¶õ
              Result := 80;
              Exit;
            end;
          end;

          if AllowUseMagic(72) and (GetTickCount - m_SkillUseTick[72] > 1000 * 10) then begin     //¿ùÇÏ³­¹«
            if (CheckTargetXYCount1(m_nCurrX, m_nCurrY, 1) > 2) and (StdItem.StdMode = 99) then begin //¿µ¿õ¿ùÇÏ³­¹«  º»ÀÎÁÖº¯ ¸÷ÀÌ 3¸¶¸® ÀÌ»ó
              Result := 72;
              Exit;
            end;
          end;
          ...
          if AllowUseMagic(140) and (GetTickCount - m_SkillUseTick[140] > 1000 * (4 + UseMagicDelay(140))) then begin     //¼ö¶óÃµ¸êÁø
            if (nRangeTargetCount >= 2) then begin //¿µ¿õ¼ö¶óÃµ¸êÁø  º»ÀÎÁÖº¯ ¸÷ÀÌ 3¸¶¸® ÀÌ»ó
              Result := 140;
              Exit;
            end;
          end;

          if AllowUseMagic(63) then begin     //ÃµÀÌ°Ë
            if GetAttackDir(m_TargetCret, 2, btDir) and (GetTickCount - m_SkillUseTick[63] > 1000 * 7) and (StdItem.StdMode = 99) then begin //¿µ¿õÃµÀÌ°Ë
          ...
          if AllowUseMagic(70) then begin
            if GetAttackDir(m_TargetCret, 1, btDir)  and (GetTickCount - m_SkillUseTick[70] > 1000 * 10) then begin   //¿µ¿õ¸Íµ¶°Ë±â
              if CheckHeroAmulet2(1, 5, 70, 0, nAmuletIdx) and (m_TargetCret.m_wStatusTimeArr[POISON_DECHEALTH] = 0) then begin   //¸Íµ¶°Ë±â
                Result := 70;
                Exit;
               end;
            end;
          ...
          if AllowUseMagic(61) and (m_dwStatusArrTimeOutTick[9] <= 0) and (GetTickCount - m_SkillUseTick[61] > 1000 * 10) then begin //¿µ¿õÃ¼½ÅÇ³
            if AllowUseMagic(69) then begin  //Ç³½Å¼ú
          ...
          if AllowUseMagic(69) and (m_dwStatusArrTimeOutTick[10] <= 0) and (GetTickCount - m_SkillUseTick[69] > 1000 * 10) then begin //¿µ¿õÇ³½Å¼ú
            if AllowUseMagic(61) then begin  //Ã¼½ÅÇ³
          ...
          if AllowUseMagic(60) and (GetTickCount - m_SkillUseTick[60] > 50) and (StdItem.StdMode = 99) then begin     //¿µ¿õÇ³°Ë¼ú
...
begin
  ...
  Try
    ...
    nCheckCode:= 1;
    if m_boDupMode and (m_btStatus <> 1) and (GetTickCount()- dwTick3F4 > m_dwWalkIntervalTime) then begin //ÈÞ½ÄÀÌ ¾Æ´Ò¶§
      dwTick3F4 := GetTickCount();
      nOldX := m_nCurrX;
      nOldY := m_nCurrY;
    ...
    nCheckCode:= 2;
     ...
     case m_btJob of             //Á÷¾÷º¯°æ
      ...
      1: begin //¿µ¿õ¼ú»ç Think
          nCheckCode:= 21;
          if (m_TargetCret = nil) and (m_btStatus = 0) and (m_btAiMode <> 2) then begin
            if (GetTickCount - m_SkillUseTick[0] > 1800) then begin     //Á¢¼Ó ÈÄ 2ÃÊÈÄ¿¡
              if (AllowUseMagic(31)) and (m_wStatusTimeArr[STATE_BUBBLEDEFENCEUP] = 0) and (not m_boAbilMagBubbleDefence) and (GetTickCount - m_SkillUseTick[31] > 1000 * 15)  then begin
      ...
      2: begin //¿µ¿õµµ»ç  Think
           nCheckCode:= 28;
           //ÁÖÀÎ ¸ÕÀú
           if (m_TargetCret = nil) and (m_btStatus = 0) and (m_btAiMode <> 2) then begin
             if (GetTickCount - m_SkillUseTick[0] > 1800) then begin     //Á¢¼Ó ÈÄ 2ÃÊÈÄ¿¡
               if AllowUseMagic(15) and (m_Master.m_wStatusTimeArr[STATE_DEFENCEUP] = 0) and (GetTickCount - m_SkillUseTick[15] > 1000 * 15) and CheckHeroAmulet2(1, 5, 15, 1, nAmuletIdx) then begin //¸øÖ÷ÈË´òÄ§·À
                 if (GetTickCount - m_SkillUseTick[0] > 500 * 1) then begin
               ...
               nCheckCode:= 26;
               if AllowUseMagic(14) and (m_Master.m_wStatusTimeArr[STATE_MAGDEFENCEUP] = 0) and (GetTickCount - m_SkillUseTick[14] > 1000 * 15) and CheckHeroAmulet2(1, 5, 14, 1, nAmuletIdx)  then begin //¸øÖ÷ÈË´òÄ§·À
                 if (GetTickCount - m_SkillUseTick[0] > 500 * 1) then begin
       ...
       3: begin//¿µ¿õÀÚ°´ È¥ÀÚ ¹«°ø¾²´Â°Å,,
         nCheckCode:= 20;
         if (m_TargetCret = nil) and (m_btStatus = 0) and (m_btAiMode <> 2) then begin
           if (GetTickCount - m_SkillUseTick[0] > 1800) then begin   //Á¢¼Ó ÈÄ 2ÃÊÈÄ¿¡
             if AllowUseMagic(61) and (m_dwStatusArrTimeOutTick[9] <= 0) and (GetTickCount - m_SkillUseTick[61] > 1000 * 10) then begin //Ã¼½ÅÇ³
               if (GetTickCount - m_SkillUseTick[0] > 500 * 1) then begin
             ...
             if AllowUseMagic(69) and (m_dwStatusArrTimeOutTick[10] <= 0) and (GetTickCount - m_SkillUseTick[69] > 1000 * 10) then begin //Ç³½Å¼ú
               if (GetTickCount - m_SkillUseTick[0] > 500 * 1) then begin
     ...
     nCheckCode:= 3;
    //ÈÞ½Ä¸ðµåÀÌ¸é¼­ ¼³Á¤°ªÀÌ ÀÌµ¿°¡´ÉÀÌ°Å³ª // ÈÞ½Ä¸ðµå ¾Æ´Ò¶§.
...
//¼ì²âÖ¸¶¨·½ÏòºÍ·¶Î§ÄÚ,Ä¿±êÓëÓ¢ÐÛµÄ¾àÀë 20080426
...
begin
  ...
  if m_UseItems[U_WEAPON].btValue[3] > 0 then begin
    ...
    SysMsg('Ó¢ÐÛµÄÎäÆ÷±»×çÖäÁË¡£', c_Red, t_Hint);
  end else begin
  ...
  if m_btRaceServer = RC_HEROOBJECT then begin
    RecalcAbilitys();
    SendUpdateItem(@m_UseItems[U_WEAPON]);  //ÀúÁÖ Ç¥½Ã
    SendMsg(Self, RM_ABILITY, 0, 0, 0, 0, '');
    SendMsg(Self, RM_SUBABILITY, 0, 0, 0, 0, '');
  end;
end;
...
function THeroObject.WeaptonMakeLuck: Boolean;       //¿µ¿õ Ãà±â
var
  StdItem: TItem;
  nRand: Integer;
  boMakeLuck: Boolean;
begin
  Result := False;
  if (m_UseItems[U_WEAPON].wIndex <= 0) or (m_UseItems[U_WEAPON].btValue[19] = 1) or (m_UseItems[U_WEAPON].btValue[19] > 2) then exit;  //´ë¿©¹«±â ºÀÀÎÅÛ Ãà±â x
  nRand := 0;
  StdItem := UserEngine.GetStdItem(m_UseItems[U_WEAPON].wIndex);
  if StdItem <> nil then begin
  ...
  if Random(g_Config.nWeaponMakeUnLuckRate {20}) = 1 then begin
    ...
    if boMakeLuck then SysMsg('Ó¢ÐÛµÄÎäÆ÷»ñµÃÁËÐÒÔË¡£', c_Green, t_Hint);
    if not boMakeLuck then SysMsg('Ó¢ÐÛµÄÎäÆ÷Ã»ÓÐ»ñµÃÐ§¹û¡£', c_Green, t_Hint);
  end;
  Result := True;
end;
...
begin
  Result := False;
  if m_boTransparent and (m_boHideMode) then m_wStatusTimeArr[STATE_TRANSPARENT ] := 1;//ÒþÉí,Ò»¶¯¾ÍÏÔÉí
  if (m_wStatusTimeArr[POISON_STONE] <> 0) and (not g_Config.ClientConf.boParalyCanSpell) then Exit;//Âé±Ô²»ÄÜÅÜ¶¯
...
function THeroObject.WarrAttackTarget(wMagIdx, wHitMode: Word): Boolean;     //¿µ¿õ°ø°Ý
var
  bt06, nCode: Byte;
  dwDelayTime: LongWord;
  nRange: Integer;
...
try
  ...
  if m_TargetCret <> nil then begin
     nCode:= 2;
    nRange := 1;
    if (wMagIdx in [12, 63]) then nRange := 2;  //¾î°Ë¼ú, ÃµÀÌ°Ë
  ...
  Result := False;
    ...
    if UserMagic <> nil then begin
      if not MagicManager.IsWarrSkill( UserMagic.wMagIdx) then begin
        ...
        Result := (UserMagic.btKey > 0) or (UserMagic.wMagIdx = 7)  or (UserMagic.wMagIdx = 59);    //¿¹µµ, Àý¸í°Ë¹ý Å° ¼³Á¤ ¾ÈÇØµµ µÊ.
      end;
    end;
end;
  ...
  for I := 0 to m_MagicList.Count - 1 do begin
    ...
    for k := 1 to 7 do begin
      if (UserMagic.btKey > 0) and (Chr(UserMagic.btKey) = IntToStr(K)) and (UserMagic.wMagIdx = wMagIdx) then begin
        ...
        MainOutMessage('¼¼ÄÜ¼ì²â: ' + IntToStr(Result) + ' / '+ Chr(UserMagic.btKey) );
        Break;
      end;
    end;
  end;
...
function THeroObject.GetNearTargetCount(rage:Integer): Integer; //ÁÖº¯Å¸ÄÏ
var
  nC, n10, I: Integer;
  nX, nY: Integer;
  BaseObject: TBaseObject;
...
resourcestring
  sDisableMagicCross = '¸ÃµØÍ¼ÉÏ²»ÄÜÊ¹ÓÃ¼¼ÄÜ%s¡£';
begin
  Result := False;
  ...
  if (UserMagic.wMagIdx = SKILL_WINDBLADE) then begin   //Ç³°Ë¼ú
    if not checkweapon(Self) then begin  //ÀÚ°´ ¹«±â ¾øÀ½
      exit;
    end;
  end;

  case UserMagic.wMagIdx of //
    SKILL_ERGUM: begin //¾î°Ë¼ú()
    ...
    SKILL_WINDBLADE: begin   //¿µ¿õÇ³°Ë¼ú
     ...
     SKILL_NANHWA{131}: begin   //³­È­Ç÷Ç³Âü
      ...
      if mode = 0 then begin
        case UserMagic.wMagIdx of
            SKILL_HEALLING,              //È¸º¹¼ú
            SKILL_HANGMAJINBUB,          //Ç×¸¶Áø¹ý
            SKILL_DEJIWONHO ,            //´ëÁö¿øÈ£
            SKILL_ENHANCER,              //¹«±ØÁø±â
            SKILL_PURI,                  //Á¤È­¼ú
            SKILL_BODDYWIND,             //Ã¼½ÅÇ³
            SKILL_PUNGSIN,               //Ç³½Å¼ú
            SKILL_SHIELD,                //ÁÖ¼úÀÇ¸·
            SKILL_BIGHEALLING : begin    //´ëÈ¸º¹¼ú
...
var
  ...
  function GetRPow(wInt: Integer): Word; //È¡ÉËº¦ÖÐ¼äËæ»úÖµ
  begin
...
begin
  ...
  case UserMagic.MagicInfo.wMagicId of //
    SKILL_FIREBALL {1}, //È­¿°Àå
    SKILL_FIREBALL2 {5}: begin //±Ý°­È­¿°Àå
    ...
    SKILL_AMYOUNSUL{6}: begin {¾Ï¿¬¼ú}
        ...
        if IsProperTarget(TargeTBaseObject) then begin
          ...
          if CheckAmulet(Self,1,5,nAmuletIdx) then begin  //µ¶°¡·ç È®ÀÎ
    ...
    SKILL_SHOOTLIGHTEN{10}: begin   //·ÚÀÎÀå
      n1C:=GetNextDirection(m_nCurrX,m_nCurrY,nTargetX,nTargetY);
      if m_PEnvir.GetNextPosition(m_nCurrX,m_nCurrY,n1C,1,n14,n18) then begin
    ...
    SKILL_FIRECHARM{13},   //Æø»ì°è
    SKILL_HANGMAJINBUB{14},  //Ç×¸¶Áø¹ý
    SKILL_DEJIWONHO{15},     //´ëÁö¿øÈ£
    SKILL_ENHANCER{36},    //¹«±ØÁø±â
    SKILL_CURSE{46}     //ÀúÁÖ¼ú
    : begin
      boSpellFail:=True;
        if CheckAmulet(Self,1,5,nAmuletIdx) then begin   //ºÎÀû È®ÀÎ
          UseAmulet(Self,1,5,nAmuletIdx);
          case UserMagic.MagicInfo.wMagicId of
    ...
    SKILL_VAMP{48}: begin //ÈíÇ÷¼ú
      if IsProperTarget (TargeTBaseObject) then begin
        if (Random(50) >= TargeTBaseObject.m_nAntiMagic) then begin
          ...
          SendDelayRefMsg(RM_10205,0,m_nCurrX,m_nCurrY,13,'0',1000);   //ÈíÇ÷ ÀÌÆÑÆ®
          RecalcAbilitys();
          SendMsg(Self,RM_ABILITY,0,0,0,0,'');
          boTrain:=True;
        end else TargeTBaseObject:=nil
    ...
    SKILL_MASSPOISON{55}: begin {PoisonCloud - Taoist}  //µ¶¹«
      if CheckAmulet2(Self,2,5,55,nAmuletIdx) then begin  //È¸»ö µ¶°¡·ç È®ÀÎ
        StdItem:=UserEngine.GetStdItem(m_UseItems[nAmuletIdx].wIndex);
        if StdItem <> nil then begin
          UseAmulet(Self,2,5,nAmuletIdx);     //È¸»ö µ¶°¡·ç È®ÀÎ
          case StdItem.Shape of
            7: begin
    ...
    SKILL_REDBANWOL{56}: begin    //ÀÏ¼¶
         ...
         if m_PEnvir.GetNextPosition(m_nCurrX,m_nCurrY,n1C,1,n14,n18) then begin
           ...
           if MagPassIlsum(n14,n18,nTargetX,nTargetY,n1C,nPower,True) > 0 then
             ...
             if MagicManager.MagChangePosition(Self, nTargetX, nTargetY) then  //ÀÏ¼¶
         end;
    end;
    ...
    SKILL_PUNGSIN{69}: begin   //Ç³½Å¼ú
        if m_wStatusTimeArr[STATE_WINDMOON] > 0 then begin  //Ç³½Å¼ú °­È­
          case UserMagic.btLevel of
            0 : nPower := 5;
            1 : nPower := 6;
            2 : nPower := 7;
    ...
    SKILL_POISONBLADE{70}: begin     //¸Íµ¶°Ë±â
        if CheckWeapon(Self) then begin
          if CheckAmulet(Self,1,5,nAmuletIdx) then begin     //È¸»ö µ¶°¡·ç È®ÀÎ
            UseAmulet(Self,1,5,nAmuletIdx);
            n1C:=GetNextDirection(m_nCurrX,m_nCurrY,nTargetX,nTargetY);
            if m_PEnvir.GetNextPosition(m_nCurrX,m_nCurrY,n1C,1,n14,n18) then begin
              m_PEnvir.GetNextPosition(m_nCurrX,m_nCurrY,n1C,1,nTargetX,nTargetY);
    ...
    SKILL_MOONBLADE{72}: begin
        if CheckWeapon(Self) then begin    //¿ùÇÏ³­¹«
    ...
    SKILL_OHAENG{83}: begin //À½¾ç¿ÀÇàÁø
        if CheckAmulet2(Self,5,5,55,nAmuletIdx) then begin  //ºÎÀû È®ÀÎ
          StdItem:=UserEngine.GetStdItem(m_UseItems[nAmuletIdx].wIndex);
          if StdItem <> nil then begin
            UseAmulet(Self,5,5,nAmuletIdx);     //ºÎÀû »ç¿ë
            nPower:= GetAttackPower (GetPower(MPow(UserMagic)) + LoWord(m_WAbil.SC),
    ...
    SKILL_STING{91}: begin //µ¶Ä§¼ú
      boSpellFail:=True;
      if CheckAmulet(Self,2,5,nAmuletIdx) then begin   //µ¶°¡·ç È®ÀÎ
        StdItem:=UserEngine.GetStdItem(m_UseItems[nAmuletIdx].wIndex);
        if StdItem <> nil then begin
    ...
    SKILL_UPREDBANWOL{130}: begin    //¸¶Ç÷±¤¼¶
       ...
       if m_PEnvir.GetNextPosition(m_nCurrX,m_nCurrY,n1C,1,n14,n18) then begin
         ...
         m_PEnvir.GetNextPosition(m_nCurrX,m_nCurrY,n1C,2,nTargetX,nTargetY);
         if MagicManager.MagMaChangePosition(Self, nTargetX, nTargetY, m_nCurrX, m_nCurrY) then  //¸¶Ç÷±¤¼¶
         SendDelayRefMsg(RM_10205, 0, nTargetX, nTargetY, NE_MAILSUM, IntToStr(directionDir(m_btDirection)), 600);   //¸¶Ç÷±¤¼¶ ÀÌÆÑÆ®
       end;
    end;

    SKILL_HWECHUN{132}: begin  //È¸Ãµ°Ý - ¿µ¿õ Àü»ç
      nPower := GetAttackPower(GetPower(MPow(UserMagic)) + LoWord(m_WAbil.DC),
...
begin
  ...
  try
    ...
    case m_btJob of            //Á÷¾÷º¯°æ
      0: begin
          Result := WarrorAttackTarget(wMagIdx);     //¿µ¿õÀü»ç  StartAttack
        end;
      1: begin
          Result := WizardAttackTarget(wMagIdx);      //¿µ¿õ¼ú»ç   StartAttack
        end;
      2: begin
          Result := TaoistAttackTarget(wMagIdx);     //¿µ¿õµµ»ç    StartAttack
        end;
      3: begin
          Result := AssassinAttackTarget(wMagIdx);     //¿µ¿õÀÚ°´   StartAttack
        end;
    end;
...
function THeroObject.WarrorAttackTarget(wMagIdx: Word): Boolean; //¿µ¿õÀü»ç
var
...
begin
  ...
  if wMagIdx > 0 then begin
    ...
    if UserMagic <> nil then begin
      case wMagIdx of
        7: m_wHitMode := 6; //¿¹µµ°Ë¹ý
        12: m_wHitMode := 4; //¾î°Ë¼ú
        25: m_wHitMode := 5; //¹Ý¿ù°Ë¹ý
        26: m_wHitMode := 7; //¿°È­°á
        38: m_wHitMode := 9; //½Ö·æÂü
        131: m_wHitMode := 29; //³­È­Ç÷Ç³Âü
        56, 130, 132: begin        //ÀÏ¼¶, ¸¶Ç÷±¤¼¶, È¸Ãµ°Ý
          nError := 4;
...
function THeroObject.WizardAttackTarget(wMagIdx: Word): Boolean; //¿µ¿õ¼ú»ç
var
  wHitMode: Word;
  nSelectMagic, nError: Integer;
  UserMagic: pTUserMagic;
...
function THeroObject.TaoistAttackTarget(wMagIdx: Word): Boolean; //¿µ¿õµµ»ç   TaoistAttackTarget
var
  wHitMode: Word;
  nSelectMagic, nError: Integer;
  UserMagic: pTUserMagic;
...
function THeroObject.AssassinAttackTarget(wMagIdx: Word): Boolean; //¿µ¿õÀÚ°´
var
  nSelectMagic, nError: Integer;
  UserMagic: pTUserMagic;
  dwDelayTime: LongWord;
      ...
      case wMagIdx of
        60: begin
          ...
          if (GetSpellPoint(UserMagic) <= m_WAbil.MP) and (StdItem.StdMode = 99) then begin
            m_wHitMode := 15; //¿µ¿õÇ³°Ë¼ú
          end;
        end;
        63, 72: begin //¿µ¿õ ÃµÀÌ°Ë ¿ùÇÏ³­¹«
          if StdItem.StdMode = 99 then begin
            nError := 6;
            Result := UseSpell(UserMagic, m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, m_TargetCret, 0);
            Exit;
...
begin
  ...
  if (GetTickCount - m_SkillUseTick[0] > (800 - m_nHitSpeed * g_Config.dwHeroItemAttackTime) ) then begin
    ...
    if wMagIdx > 0 then begin
      ...
      if UserMagic <> nil then begin
        boIsWarrSkill := MagicManager.IsWarrSkill(UserMagic.wMagIdx);
        if (wMagIdx in [56, 72, 80, 130, 132]) then begin  //ÀÏ¼¶ ¿ùÇÏ³­¹« ¸¶Ç÷±¤¼¶ È¸Ãµ°Ý ¹¦¼³¶õ
          ...
          if (wMagIdx in [46, 53, 55, 83, 91, 134, 135, 139, 140]) then begin  //Ãµ»ó³«¿° µ¶¹« Ãµ»óÆø¿° ³«Ãµ·Ú ÀúÁÖ¼ú µ¶Ä§¼ú À½¾ç¿ÀÇàÁø
...
var
  ...
  UserMagic:pTUserMagic;
  StorageItems:pTStorageItems;         //Ã¢°í
begin
  ...
  HumanRcd.nPKPoint:=m_nPkPoint;    //ÇÇÄÉÀÌ
  ...
  HumanRcd.btReLevel:=m_btReLevel;       //ÀüÁ÷
  ...
  StorageItems:=@HumanRcd.StorageItems;        //Ã¢°í
...
procedure THeroObject.ClientaddBundleItems(mode,MaxAmount:integer;ItemIdx:String); //¹°¾à2
var
...
begin
  ...
  if mode in [0,1,3,45,46] then modeidx := MaxAmount;       //¹°¾à °³¼ö 20°³
  for I := 0 to m_ItemList.Count - 1 do begin
...
procedure THeroObject.ClientSeparateItems(bMakeIndex,namount:integer);   //¹°¾à2
var
 I,inde:Integer;
...
begin
  if (bMakeIndex <= 0) or (namount < 1) then Exit;
    ...
    SysMsg ('ÄãµÄ±³°üÂúÁË¡£', c_Red,t_Hint);
    Exit;
  end;
  inde := 0;
  for I := 0 to m_ItemList.Count - 1 do begin
    ...
    if (sUItem.MakeIndex = bMakeIndex) and (sUItem.Amount > namount) then begin
      ...
      if sUItem.Amount > StdItem.MaxAmount then begin     //¹°¾à ¹ö±× ºÐ¸®
         Dispose(sUItem);
         m_ItemList.Delete (i);
         WeightChanged();
         exit;
...
procedure THeroObject.ClientaddBundleStorageItems(mode,MaxAmount:integer;ItemIdx:String); //¹°¾à2
var
 I,II,minc,dinc,inde,modeidx:Integer;
 MUItem,DUItem:PTUserItem;
 mItemIdx,dItemIdx:Integer;
...
begin
  ...
  dItemIdx := strtoint(ItemIdx);
  mItemIdx := StrToInt(stre);
  if mode in [0,1,3,45,46,49] then modeidx := MaxAmount;       //¹°¾à °³¼ö 20°³
  for I := 0 to m_StorageItemList.Count - 1 do begin
    ...
    if (UserItem <> nil) and (UserItem.MakeIndex = nItemIdx) then begin
      ...
      if not (StdItem.StdMode in [2,46]) then begin
        ...
        if (self.m_btJob = Jtaos) or (self.m_btJob = JUptao) then
          UserItem.wIndex:=UserEngine.GetStdItemIdx(StdItemName + '3');
        if (self.m_btJob = JAssassin) or (self.m_btJob = jUpAssa) then                       //ÀÚ°´
          UserItem.wIndex:=UserEngine.GetStdItemIdx(StdItemName + '4');
        if (self.m_btJob = jBonze) or (self.m_btJob = jUpBoz) then                         //½Â·Á
          UserItem.wIndex:=UserEngine.GetStdItemIdx(StdItemName + '5');
          StdItem:=UserEngine.GetStdItem(UserItem.wIndex);
        end;
      end;
      ...
      if CheckTakeOnItems(btWhere, StdItem58) then begin
        ...
        if btWhere in [0..MAXHEROUSEITEM] then begin
          ...
          if TakeOffItem <> nil then begin
            ...
            if (m_btJob in [3,8]) and (StdItemOff.StdMode = 99 ) and (m_MagicWindbladeSkill <> nil) and m_boUseWindblade then begin  //Ç³°Ë¼ú
               WindbladeOnOff(FALSE);
             //  SendSocket(nil,'+UWB');
            end;
          end;
      ...
      if m_UseItems[btWhere].MakeIndex = nItemIdx then begin
        ...
        if CompareText(sUserItemName, sItemName) = 0 then begin
          ...
          if AddItemToBag(UserItem, boot) then begin
            ...
            if (m_btJob in [3,8]) and (StdItem.StdMode = 99 ) and (m_MagicWindbladeSkill <> nil) and m_boUseWindblade then begin  //Ç³°Ë¼ú
              WindbladeOnOff(FALSE);
             // SendSocket(nil,'+UWB');
            end;
...
begin
  Result:=False;
  if (StdItem.StdMode in [10,12]) and (m_btGender <> gMan) then begin    //³²ÀÚ¿Ê
    SysMsg(sWearNotOfWoMan,c_Red,t_Hint);
    exit;
  end;
  if (StdItem.StdMode = 11) and (m_btGender <> gWoMan) then begin   //¿©ÀÚ¿Ê
  ...
  if (StdItem.StdMode in [10,11,12]) and (StdItem.AniCount = 110) and m_PEnvir.Flag.boNOPENETRATE then begin
    SysMsg('Äã²»ÄÜÔÚÕâÀïÊ¹ÓÃ¡£',c_Red,t_Hint);
    exit;
  end;

  if (StdItem.StdMode in [10,11,12]) and (StdItem.AniCount = 110) and (GetTickCount - m_PenetrateLastTick < 20 * 1000) then begin    //¸ÁÀÚÀÇ¿Ê
    SysMsg(IntToStr( 20 - (GetTickCount - m_PenetrateLastTick) div 1000) + 'ÃëÒÔºó¿ÉÒÔÊ¹ÓÃ¡£',c_Red,t_Hint);
    exit;
  end;
      ...
      if StdItem.Shape = 3 then begin
         ...
         if TPlayObject(self).fLover <> nil then begin
            ...
            if strtemp = '' then begin
               SysMsg ('Ö»ÓÐÇéÂÂ²Å¿ÉÒÔÊ¹ÓÃ¡£',c_Red,t_Hint);
               exit;
            end;
         end;
         ...
         if TPlayObject(self).fLover <> nil then begin
            strtemp := TPlayObject(self).fLover.GetLoverName;
            if strtemp <> '' then begin
               SysMsg ('ÇéÂÂ²»¿ÉÒÔÊ¹ÓÃ¡£',c_Red,t_Hint);
               exit;
            end;
         end;
      end;
  ...
  if (StdItem.CheckBonze) and not (m_btJob in [g_Bonze, g_BonzeUp]) then begin   //½Â·Á¸¸Âø¿ë
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if (StdItem.StdMode in [12, 90, 103]) and not (m_btJob in [g_Bonze, g_BonzeUp]) then begin //½Â·Á¿Ê ¹«±â Å¸Á÷¾÷ ¸ø ÀÔÀ½.  Å»°Í
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if (StdItem.StdMode in [10, 11, 12, 90, 5, 6, 7, 99]) then begin     //½Â·Á°¡ Âø¿ë°¡´ÉÇÑ ¾ÆÀÌÅÛ
  ...
  if (StdItem.StdMode = 99) and not (m_btJob in [3,8]) then begin   //99¹«±â ÀÚ°´¸¸Âø¿ë       //Á÷¾÷º¯°æ
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if (StdItem.HumUp = 1) and (m_btJob <> g_WarriorUp) then begin    //ÈÉ1 º®Ç÷ Àü»ç¸¸
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if (StdItem.HumUp = 2) and (m_btJob <> g_WizardUp) then begin    //ÈÉ2 È«Çö ¼ú»ç¸¸
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if (StdItem.HumUp = 3) and (m_btJob <> g_MonkUp)  then begin     //ÈÉ3 ÀÍ¼± µµ»ç¸¸
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if (StdItem.HumUp = 4) and (m_btJob <> g_AssassinUp) then begin   //ÈÉ4 ºñ¿¬ ÀÚ°´¸¸
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;

  if (StdItem.StdMode in [5,6,7]) and not (StdItem.Shape in [19,34..36,37,39..48,55,56,57,73..77]) and (m_btJob = g_WarriorUp) then begin  //º®Ç÷Àü»ç Âø¿ë ¹«±â
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if (StdItem.StdMode in [5,6,7]) and not (StdItem.Shape in [19,34..36,37,39..48,55,56,57,78..81]) and (m_btJob = g_WizardUp) then begin   //È«Çö¼ú»ç Âø¿ë ¹«±â
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if (StdItem.StdMode in [5,6,7]) and not (StdItem.Shape in [19,34..36,37,39..48,55,56,57,82..85]) and (m_btJob = g_MonkUp) then begin     //ÀÍ¼±µµ»ç Âø¿ë ¹«±â
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if (StdItem.StdMode in [5,6,7,99,90]) and not (StdItem.Shape in [19,34..36,37,39..48,55,56,57,86..89,96..98,100..102,104]) and (m_btJob = g_AssassinUp) then begin  //ºñ¿¬ÀÚ°´ Âø¿ë ¹«±â
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;

  if ((StdItem.StdMode = 10) or (StdItem.StdMode = 11)) and (StdItem.CheckNoAss) and (m_btJob = g_Assassin) then begin  //±×·¡ÇÈ½º ¿Ê
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if ((StdItem.StdMode = 10) or (StdItem.StdMode = 11)) and ((StdItem.shape >= 100) or (StdItem.CheckAss)){and (116 > StdItem.shape)} and (m_btJob <> g_Assassin) then begin //ÀÚ°´¿Ê Å¸Á÷¾÷ ¸øÀÔÀ½
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if ((StdItem.StdMode = 10) or (StdItem.StdMode = 11)) and (((StdItem.Shape > 2) and (StdItem.shape < 9)) or ((StdItem.Shape > 12) and (StdItem.shape < 19))) and (m_btJob = g_Assassin) then begin    //Àü»ç,¼ú»ç,µµ»ç ¿ÊÁß ÀÚ°´ Âø¿ë °¡´É
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if ((StdItem.StdMode = 10) or (StdItem.StdMode = 11)) and ((StdItem.Shape > 24) and (StdItem.shape < 28)) and (m_btJob = g_Assassin) then begin   //Àü»ç,¼ú»ç,µµ»ç ¿ÊÁß ÀÚ°´ Âø¿ë °¡´É
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if ((StdItem.StdMode = 10) or (StdItem.StdMode = 11)) and ((StdItem.Shape > 30) and (StdItem.shape < 34)) and (m_btJob = g_Assassin) then begin   //Àü»ç,¼ú»ç,µµ»ç ¿ÊÁß ÀÚ°´ Âø¿ë °¡´É
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if (StdItem.StdMode in [10,11,12]) and (StdItem.HumUp = 0) and not (StdItem.Shape in [0,9,10,11,30]) and not (m_btJob in [0..4]) then begin   //Èâ0 À» Á÷¾÷ 0..3 ÀÌ ¾Æ´Ñ Ä³¸¯(µî¼±) Âø¿ë ÇÒ °æ¿ì         //Á÷¾÷º¯°æ
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if (StdItem.StdMode in [101]) and not (m_btJob in [0..4]) then begin   //Å»°Í µî¼± ¾Æ´ÑÄÌ         //Á÷¾÷º¯°æ
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  if (StdItem.StdMode in [102]) and not (m_btJob in [5..9]) then begin   //Å»°Í µî¼± ÄÌ       //Á÷¾÷º¯°æ
    SysMsg(sWearNotOfJob,c_Red,t_Hint);
    exit;
  end;
  ...
  case StdItem.Need of    //
    0: begin        //ÇÊ¿ä·¹º§
      if m_Abil.Level >= StdItem.NeedLevel then begin
        ...
        SysMsg('Ó¢ÐÛµÄ ' + g_sLevelNot,c_Red,t_Hint);
      end;
    end;
    1: begin        //ÇÊ¿ä ÆÄ±«
    ...
    8: begin      //ÇÊ¿ä ÆÄ±« ¸¶¹ý
      if (HiWord(m_WAbil.DC) >= StdItem.NeedLevel) and (HiWord(m_WAbil.SC) >= StdItem.NeedOption) then begin
        ...
        if (HiWord(m_WAbil.DC) < StdItem.NeedLevel) then
          SysMsg('¹¥»÷Á¦²»×ã¡£', c_Red, t_Hint);
        if (HiWord(m_WAbil.SC) < StdItem.NeedOption) then
          SysMsg('Ä§·¨Á¦²»×ã¡£', c_Red, t_Hint);
      end;
    end;
    10: begin
    ...
    4: begin
      if m_btReLevel >= StdItem.NeedLevel then begin   //ÀüÁ÷
        Result:=True;
      end else begin
    ...
    7: begin
      Castle:=g_CastleManager.CastleMember(Self,'É³°Í¿Ë');
      if (m_MyGuild <> nil) and (Castle <> nil) then begin  //»çºÏ¹®
        Result:=True;
      end else begin
    ...
    9: begin           //³²¸¸¹®¿ø
      Castle:=g_CastleManager.CastleMember(Self,'ÄÏÂú');
      if (m_MyGuild <> nil) and (Castle <> nil) then begin
        Result:=True;
      end else begin
    ...
    20: begin
      if m_btColumn > 0 then begin
        ...
        SysMsg('Ö»ÓÐ×¨À¸ÁÐ±í²Å¿ÉÒÔÊ¹ÓÃ¡£',c_Red,t_Hint);
      end;
    end;
    21: begin
      if m_btColumn = 1 then begin
        ...
        SysMsg('Ö»ÓÐÐû´«ÍÅ³¤²Å¿ÉÒÔÊ¹ÓÃ¡£',c_Red,t_Hint);
      end;
    end;
    22: begin
      if m_btColumn = 2 then begin
        ...
        SysMsg('Ö»ÓÐÐû´«³ÉÔ±²Å¿ÉÒÔÊ¹ÓÃ¡£',c_Red,t_Hint);
      end;
    end;
    23: begin
      if m_btColumn = 3 then begin
        ...
        SysMsg('Ö»ÓÐ¼ÇÕßÍÅ³¤²Å¿ÉÒÔÊ¹ÓÃ¡£',c_Red,t_Hint);
      end;
    end;
    24: begin
      if m_btColumn = 4 then begin
        ...
        SysMsg('Ö»ÓÐ¼ÇÕß³ÉÔ±²Å¿ÉÒÔÊ¹ÓÃ¡£',c_Red,t_Hint);
      end;
    end;
    25: begin
      if m_btColumn = 5 then begin
        ...
        SysMsg('Ö»ÓÐ¾«Ó¢ÍÅ³¤²Å¿ÉÒÔÊ¹ÓÃ¡£',c_Red,t_Hint);
      end;
    end;
    26: begin
      if m_btColumn = 6 then begin
        ...
        SysMsg('Ö»ÓÐ¾«Ó¢³ÉÔ±²Å¿ÉÒÔÊ¹ÓÃ¡£',c_Red,t_Hint);
      end;
    end;
...
procedure THeroObject.ClientUseItems(nItemIdx: Integer; sItemName: string); //Ó¢ÐÛ³ÔÒ©
...
begin
  ...
  boSendUpDate := False;
  StdItem := nil;
//  if ((GetTickCount - m_dwCanUseItemTick > g_Config.nUseItemSpeed) or (g_Config.nUseItemSpeed <= 0)) then begin //2008-05-16Ôö¼ÓÎïÆ·Ê¹ÓÃ¼ä¸ô
  if (not m_boStore) then begin   //°³ÀÎ»óÁ¡
    for I := m_ItemList.Count - 1 downto 0 do begin
      if m_ItemList.Count <= 0 then Break;
      UserItem := m_ItemList.Items[I];
      if (UserItem <> nil) and (UserItem.MakeIndex = nItemIdx) then begin
          ...
          case StdItem.StdMode of
            0,1,3: begin //¹°¾à2
                if EatItems(StdItem,UserItem) then begin
                  if (UserItem.Amount > StdItem.MaxAmount) then begin  //¹°¾à ¹ö±×
                    Dispose(UserItem);
                    m_ItemList.Delete(I);
                    boEatOK:=True;
                  end else begin
            ...
            16: begin  //ÀÚµ¿¹°¾àÁÖ¹®¼­
                if m_btHeroAutoDurg in [0,1] then begin
                  if m_btHeroAutoDurg < 2 then begin
                   ...
                   m_btHeroAutoDurg := 2;
                  SysMsg('Ó¢ÐÛ×Ô¶¯Ê¹ÓÃÒ©Æ·À¸»¹ÓÐ' + IntToStr(m_btHeroAutoDurg) + '¡£',c_Red,t_Hint);
                  m_ItemList.Delete(I);
                  Dispose(UserItem);
                  UserItem := nil;
                  boEatOK := True;
                end else begin
                  SysMsg('Ó¢ÐÛ×Ô¶¯Ê¹ÓÃÒ©Æ·À¸ÒÑÂú¡£',c_Red,t_Hint);
                end;
              end;
            17: begin  //ÇØ¹æ¼­
                if ((m_btLimitLevel * 10) = m_Abil.Level) and (StdItem.NeedLevel = m_Abil.Level) then begin
                  ...
                  if Much = 0 then begin
                    if StdItem.AniCount = 0 then begin
                      ...
                      '¹§Ï²£¬Ó¢ÐÛµÄµÈ¼¶ÏÞÖÆÒÑ³É¹¦½âËø¡£\ÏÖÔÚÄã¿ÉÒÔÉý¼¶µ½ ' + IntToStr((m_btLimitLevel+1) * 10) +'¼¶¡£');
                       m_btLimitLevel := 0;
                      ...
                      if (Ani = 0) and (StdItem.Source > 0) then begin
                        ...
                        HasLevelUp(2);
                        TPlayObject(m_Master).SendMsg(Self, RM_MENU_OK, 0, Integer(Self), 0, 0,
                        'Ó¢ÐÛµÄµÈ¼¶ÏÞÖÆÒÑ¾­½âËø¡£\Äã¿ÉÒÔÉý¼¶' + IntToStr((m_btLimitLevel+1) * 10) +'¸öµÈ¼¶¡£\×£ºØÄã£¬ÄãµÄÓ¢ÐÛÉý¼¶ÁË¡£');
                         m_btLimitLevel := 0;
                      end else begin
                        TPlayObject(m_Master).SendMsg(Self, RM_MENU_OK, 0, Integer(Self), 0, 0,
                       '¹§Ï²£¬Ó¢ÐÛµÄµÈ¼¶ÏÞÖÆÒÑ¾­½âËø¡£\Äã¿ÉÒÔÉý¼¶' + IntToStr((m_btLimitLevel+1) * 10) +'¸öµÈ¼¶¡£');
                        m_btLimitLevel := 0;
                        HasLevelDown(m_Abil.Level);
                      end;
                    end;
                    ...
                    if (StdItem.AniCount = 0) and (Rand <> 0) then begin
                      ...
                      HasLevelDown(m_Abil.Level);
                      TPlayObject(m_Master).SendMsg(Self, RM_MENU_OK, 0, Integer(Self), 0, 0,
                       'ºÜ±§Ç¸£¬Ó¢ÐÛµÈ¼¶½âËøÊ§°Ü¡£\µÈ¼¶ÏÞÖÆÏÂ½µÁË' + IntToStr(Rand) +'¼¶¡£');
                       m_btLimitLevel := 0;
                    end else begin
                      TPlayObject(m_Master).SendMsg(Self, RM_MENU_OK, 0, Integer(Self), 0, 0, 'ºÜ±§Ç¸£¬Ó¢ÐÛµÈ¼¶½âËøÊ§°Ü¡£');
                    end;
                  end;
                  m_ItemList.Delete(I);
                  Dispose(UserItem);
                  ...
                  SysMsg('Ã»ÓÐ×ã¹»µÄµÈ¼¶»òÄÜÁ¦µÈ¼¶¡£', c_Red, t_Hint);
                end;
              end;
            31: begin //½â°üÎïÆ·
                if StdItem.AniCount = 0 then begin
...
begin
            ...
            0: begin //¹°¾à2
              if EatItems(StdItem,UserItem) then begin
                if (UserItem.Amount > StdItem.MaxAmount) then begin  //¹°¾à ¹ö±×
                  ...
                  if (UserItem.Amount > 1) then begin
                    Dec(UserItem.Amount,1);
                    SendAddAutoItemMode(UserItem);  //¹°¾à
                    boEatOK:=True;
                  end else begin
                    Dispose(UserItem);
                    m_AutoItem.Delete(I);
  ...
  if boEatOK then begin
    WeightChanged();
    SysMsg('Ó¢ÐÛ×Ô¶¯Ò©Ë®À¸Ê¹ÓÃÁËÎïÆ· '+ StdItem.Name + '¡£', c_Red, t_Hint);
    if StdItem.NeedIdentify = 1 then
      AddGameDataLog('11' + #9 +
        m_sMapName + #9 +
        IntToStr(m_nCurrX) + #9 +
...
begin
  Result := m_sCharName + ' ' +
    'Ö°Òµ:' + IntToJob(m_btJob) + ' ' +
    'µØÍ¼:' + m_sMapName + '(' + m_PEnvir.sMapDesc + ') ' +
    '×ø±ê:' + IntToStr(m_nCurrX) + '/' + IntToStr(m_nCurrY) + ' ' +
    'µÈ¼¶:' + IntToStr(m_Abil.Level) + ' ' +
    '¾­ÑéÖµ:' + IntToStr(m_Abil.Exp) + ' ' +
    '×ÊÖÊ: ' + IntToHeroGrade(m_nHeroGrade);
end;
...
function THeroObject.AddItemToBagCategories(UserItem: PTUserItem;var boot:Boolean): Boolean;     //¹°¾à2
var
...
begin
  ...
  Item:=UserEngine.GetStdItem(UserItem.wIndex);
   if Item = nil then exit;
   if (Item.StdMode in [0,1,3,45,46]) and (UserItem.Amount = 1) then begin      //¹°¾à
     if (Item.StdMode in [0,1,3,45,46]) then
     itemidx := Item.MaxAmount; //¹°¾à °¹¼ö
     for I := 0 to m_ItemList.Count - 1 do begin
...
///////////////////////////////////////////////////////////////
//added by sonmg...
procedure THeroObject.CmdUpgradeItem (seedname, jewelryname: string; seedindex, jewelryindex, ExecCount : integer);     //½ºÃÄ
var
   iResult: integer;
   i, j, k, iVal: integer;
      ...
      if puSeed.wIndex > 0 then begin
         if CheckJewelryItem(psJewelry.StdMode) then begin
            ...
            if iVal = 2 then begin
               ...
               case iResult of
                  2: begin //¼º°ø
                     ...
                     if DoUpgradeItem (puSeed, psSeed, psJewelry) = 0 then begin
                        SysMsg('´ËÊôÐÔÎÞ·¨Éý¼¶¡£', c_Green,t_Hint);
                        exit;
                     end;
                     ...
                     SysMsg (seedname + '·¢³öÒ»µÀè­è²µÄ¹âÃ¢£¬' + strResult + strEtc + '¡£', c_Blue,t_Hint);
                     ...
                     SendDefMessage (SM_UPGRADEITEM_RESULT, seedindex, iResult, 0, 0, seedname);    //º¸¿Á
                  ...
                  1: begin // ºÒº¯
                     ...
                     // ÀÓ½Ã È®ÀÎ ¸Þ½ÃÁö.
                     SysMsg (seedname + 'Ã»ÓÐ·¢ÉúÈÎºÎ±ä»¯.', c_Green,t_Hint);
                  ...
                  0: begin // ½ÇÆÐ(ÆÄ±«)
                     ...
                     QuestTakeCheckItem (puJewelry);
                     // Seed ¾ÆÀÌÅÛ »èÁ¦.
                     QuestTakeCheckItem2 ( puSeed, Word(true) ); // ÆÄ±« È¿°ú¸¦ À§ÇÑ ÆÐÅ¶

                     SysMsg ('ÎïÆ·('+ seedname +')ÆÆËéÁË¡£', c_Red,t_Hint);
               ...
               SysMsg('¸ÃÎïÆ·²»ÄÜÉý¼¶´ËÏîÊôÐÔ¡£', c_Red,t_Hint)
            end else if iVal = 3 then begin
               SysMsg('ÌØÊâÎïÆ·ÎÞ·¨Éý¼¶¡£', c_Red,t_Hint)
            end else if iVal = 4 then begin
               SysMsg('×âÁÞµÄÎïÆ·ÎÞ·¨Éý¼¶¡£', c_Red,t_Hint)
            end else if iVal = 5 then begin
               SysMsg('·âÓ¡µÄÎïÆ·²»ÄÜÉý¼¶¡£', c_Red,t_Hint)
            // ¹Ù´ÀÁú¿ëÇ°, »À¸ÁÄ¡.
               ...
               SysMsg('´ËÎïÆ·ÎÞ·¨ÐÞÀí¡£', c_Red,t_Hint)
            end else
               SysMsg (seedname + ' : ÎÞ·¨Ê¹ÓÃ¡£', c_Red,t_Hint);
         end;
      end;
...
// º¸¿Á·ù ¾ÆÀÌÅÛ Ã¼Å©.
function THeroObject.CheckJewelryItem (iStdMode: integer) : Boolean;         //½ºÃÄ
begin
   // º¸¿Á,½ÅÁÖ,³ë²ö.
...
////////////////////////
// SEED ¾ÆÀÌÅÛ Ã¼Å©.
function THeroObject.CheckSeedItem (puSeed: PTUserItem ;psSeed, psJewelry: TItem) : Integer;            //½ºÃÄ
begin
   ...
   // ¹«±â,¿Ê,Åõ±¸,¸ñ°ÉÀÌ,¹ÝÁö,ÆÈÂî, ½Å¹ß,º§Æ®.
   if (puSeed.btValue[19] = 1) then begin     //´ë¿© º¸¿Á ±ÝÁö
     Result := 4;
     exit;
   end;

   if (puSeed.btValue[19] > 2) then begin     //ºÀÀÎ º¸¿Á ±ÝÁö
     Result := 5;
     exit;
   end;
   ...
   case psSeed.StdMode of
      5, 6, 99, 90: // ¹«±â         ¹æ¾î ¸¶Ç× Á¤È® ¹ÎÃ¸ ÁßÀúÇ× ¸¶ÀúÇ× ¾ÈµÊ
         begin
      ...
      10, 11, 12: // ¿Ê          //ÆÄ±« ¸¶¹ý µµ·Â Á¤È® °ø¼Ó Áßµ¶ µÐÈ­ ¾ÈµÊ
         begin
      ...
      15: // Åõ±¸      ÆÄ±« ¸¶¹ý µµ·Â ¹ÎÃ¸ °ø¼Ó µÐÈ­ Áßµ¶ ¾ÈµÊ
         begin
            if (psJewelry.DC > 0) or (psJewelry.MC > 0) or (psJewelry.SC > 0) or
      ...
      19,20,21: // ¸ñ°ÉÀÌ         //¹æ¾î,¸¶Ç× ¹ÎÃ¸ Áßµ¶ÀúÇ× ¾ÈµÊ
         begin
      ...
      22,23: // ¹ÝÁö      //Á¤È® ¹ÎÃ¸ Áßµ¶ÀúÇ× ¸¶¹ýÀúÇ× ¾ÈµÊ
         begin
      ...
      24,26: // ÆÈÂî26       //°ø¼Ó ¸¶ÀúÇ× Áßµ¶ÀúÇ× µÐÈ­ Áßµ¶ ¾ÈµÊ
         begin
      ...
      62: // ½Å¹ß         ÆÄ±« ¸¶¹ý µµ·Â Á¤È® °ø¼Ó ÁßÀúÇ× µÐÈ­ Áßµ¶ ÁßÀúÇ× ¾ÈµÊ
         begin
            if (psJewelry.DC > 0) or (psJewelry.MC > 0) or (psJewelry.SC > 0) or
      ...
      64: // º§Æ®        //ÆÄ±« µµ·Â ¸¶¹ý °ø¼Ó ¸¶¹ýÀúÇ× µÐÈ­ Áßµ¶ ¾ÈµÊ
         begin
            if (psJewelry.DC > 0) or (psJewelry.MC > 0) or (psJewelry.SC > 0) or
...
///////////////////////////////////
// È®·ü °è»ê ¹× °á°ú ¸®ÅÏ ÇÔ¼ö.
function THeroObject.CalcUpgradeProbability(puSeedItem, puJewelryItem: PTUserItem; psSeedItem, psJewelryItem: TItem; iExecCount: Integer; var iRetSum: integer; var fRetProb: Double) : integer;      //½ºÃÄ
var
...
begin
   ...
   for i := 0 to iExecCount - 1 do begin
      ...
      if psJewelryItem.StdMode = 60 then begin
         if psSeedItem.StdMode in [5,6,99,90] then begin //¹«±â
         ...
         end else if psSeedItem.StdMode in [10,11,12] then begin //¿Ê
            iSucceed := _MIN( UpProb[iSum].iBase, Round( UpProb[iSum].iValue[0] * ABS( (29 + m_nBodyLuckLevel) / 30 ) ) );
         end else if psSeedItem.StdMode in [24,26,62] then begin  //ÆÈÂî,½Å¹ß
...
//         iFail := Integer( Round( 0.7 * (UpProb[iSum].iBase - iSucceed) ) );
   ...
   end;
 {
   ...
   if psJewelryItem.StdMode = 60 then begin
      MainOutMessage( '[UpgradeItem:È®·ü] ' + '=> '
         ...
         + ', BodyÇà¿î:' + IntToStr(m_nBodyLuckLevel) + ', ¹«±âÇà¿î:' + IntToStr(LOBYTE(psSeedItem.AC) + puSeedItem.btValue[3])
         + ', ¹«±âºÒ¿î:' + IntToStr(LOBYTE(psSeedItem.MAC) + puSeedItem.btValue[4]) );
   end else if psJewelryItem.StdMode = 61 then begin
      MainOutMessage( '[UpgradeItem:È®·ü] ' + '=> '
         + '¿É¼ÇÇÕ:' + IntToStr(iSum) + ', ¼º°øÈ®·ü:' + IntToStr(iSucceed)
...
function THeroObject.GetTotalValueOfOption( pu: PTUserItem; pstd, psJewelry: TItem; var strResult, strEtc: string ): integer;      //½ºÃÄ
var
            ...
            case iOptionIndex of
               ...
               106://Á¤È®                    //¹«±â Á¤È®
                  begin
                     ...
                     iUpgradeValue := pu.btValue[5];          //¹«±â Á¤È®
                     Result := iBaseValue + iUpgradeValue;
                  end;
               107://¹ÎÃ¸
                  begin
               ...
               108://°ø¼Ó
                  begin
                     ...
                     iUpgradeValue := ItemUnit.RealAttackSpeed( pu.btValue[6] );   //¹«±â °ø¼Ó
                     Result := iBaseValue + iUpgradeValue;
                  end;
               109://µÐÈ­
                  begin
                     iBaseValue := pstd.Slowdown;
                     iUpgradeValue := pu.btValue[8];   //¹«±â µÐÈ­
                     Result := iBaseValue + iUpgradeValue;
                  end;
               110://Áßµ¶
                  begin
                     iBaseValue := pstd.Tox;
                     iUpgradeValue := pu.btValue[9];  //¹«±â Áßµ¶
                     Result := iBaseValue + iUpgradeValue;
                  end;
               111://¸¶ÀúÇ×
                  begin
            ...
            case iOptionIndex of
               ...
               107://¹ÎÃ¸
                  begin
                     ...
                     iUpgradeValue := pu.btValue[6];        //¿Ê ¹ÎÃ¸
                     Result := iBaseValue + iUpgradeValue;
                  end;
               108://°ø¼Ó
                  begin
               ...
               111://¸¶ÀúÇ×
                  begin
                     ...
                     iUpgradeValue := pu.btValue[8];           //¿Ê ¸¶¹ýÀúÇ×
                     Result := iBaseValue + iUpgradeValue;
                  end;
               112://ÁßÀúÇ×
                  begin
                     iBaseValue := pstd.ToxAvoid;
                     iUpgradeValue := pu.btValue[7];             //¿Ê Áßµ¶ÀúÇ×
                     Result := iBaseValue + iUpgradeValue;
                  end;
            end;//case iOptionIndex of
         end;
         ...
         begin
            case iOptionIndex of
               ...
               103://¹æ¾î
                  begin
                     iBaseValue := HIBYTE(pstd.AC);
                     iUpgradeValue := pu.btValue[3];               //Åõ±¸ ¹æ¾î
                     Result := iBaseValue + iUpgradeValue;
                  end;
               104://¸¶Ç×
                  begin
                     iBaseValue := HIBYTE(pstd.MAC);
                     iUpgradeValue := pu.btValue[4];              //Åõ±¸ ¸¶¹æ
                     Result := iBaseValue + iUpgradeValue;
                  end;
               105://³»±¸
                  begin
               ...
               106://Á¤È®
                  begin // Åõ±¸¿¡¸¸ ÇØ´ç
                     iBaseValue := pstd.Accurate;
                     iUpgradeValue := pu.btValue[8];         //Åõ±¸ Á¤È®
                     Result := iBaseValue + iUpgradeValue;
                  end;
               107://¹ÎÃ¸
                  begin // ¿Ê¿¡¸¸ ÇØ´ç
               ...
               111://¸¶ÀúÇ×
                  begin
                     iBaseValue := pstd.MagAvoid;
                     iUpgradeValue := pu.btValue[10];             //Åõ±¸ ¸¶¹ýÀúÇ×
                     Result := iBaseValue + iUpgradeValue;
                  end;
               112://ÁßÀúÇ×
                  begin
                     iBaseValue := pstd.ToxAvoid;
                     iUpgradeValue := pu.btValue[9];           //Åõ±¸ Áßµ¶ÀúÇ×
                     Result := iBaseValue + iUpgradeValue;
                  end;
            end;//case iOptionIndex of
         end;
      ///////////////////////////////////////////////////////
      19,20,21: // ¸ñ°ÉÀÌ19             //¸ñ°ÉÀÌ19
         begin
            case iOptionIndex of
               100://ÆÄ±«
                  begin
               ...
               106://Á¤È®
                  begin
                     iBaseValue := pstd.Accurate;
                     iUpgradeValue := pu.btValue[15];           //¸ñ°ÉÀÌ Á¤È®
                     Result := iBaseValue + iUpgradeValue;
                  end;
               107://¹ÎÃ¸
                  begin
               ...
               108://°ø¼Ó
                  begin
                     ...
                     iUpgradeValue := pu.btValue[17];           //¸ñ°ÉÀÌ °ø¼Ó
                     Result := iBaseValue + iUpgradeValue;
                  end;
               109://µÐÈ­
                  begin
                     iBaseValue := pstd.Slowdown;
                     iUpgradeValue := pu.btValue[14];            //¸ñ°ÉÀÌ µÐÈ­
                     Result := iBaseValue + iUpgradeValue;
                  end;
               110://Áßµ¶
                  begin
                     iBaseValue := pstd.Tox;
                     iUpgradeValue := pu.btValue[9];          //¸ñ°ÉÀÌ Áßµ¶
                     Result := iBaseValue + iUpgradeValue;
                  end;
               111://¸¶ÀúÇ×
                  begin
                     iBaseValue := pstd.MagAvoid;
                     iUpgradeValue := pu.btValue[12];         //¸ñ°ÉÀÌ ¸¶ÀúÇ×
                     Result := iBaseValue + iUpgradeValue;
                  end;
               112://ÁßÀúÇ×
                  begin
         ...
         begin
            case iOptionIndex of
               ...
               108://°ø¼Ó
                  begin
                     iBaseValue := pstd.AtkSpd;
                     iUpgradeValue := pu.btValue[16];               //¹ÝÁö °ø¼Ó
                     Result := iBaseValue + iUpgradeValue;
                  end;
               109://µÐÈ­
                  begin
                     iBaseValue := pstd.Slowdown;
                     iUpgradeValue := pu.btValue[14];              //¹ÝÁö µÐÈ­
                     Result := iBaseValue + iUpgradeValue;
                  end;
               110://Áßµ¶
                  begin
                     iBaseValue := pstd.Tox;
                     iUpgradeValue := pu.btValue[9];                //¹ÝÁö Áßµ¶
                     Result := iBaseValue + iUpgradeValue;
                  end;
               111://¸¶ÀúÇ×
                  begin
         ...
         begin
            case iOptionIndex of
               ...
               106://Á¤È®
                  begin
                     iBaseValue := pstd.Accurate;
                     iUpgradeValue := pu.btValue[15];                 //ÆÈÂî Á¤È®
                     Result := iBaseValue + iUpgradeValue;
                  end;
               107://¹ÎÃ¸
                  begin
                     iBaseValue := pstd.Agility;
                     iUpgradeValue := pu.btValue[16];                 //ÆÈÂî ¹ÎÃ¸
                     Result := iBaseValue + iUpgradeValue;
                  end;
               108://°ø¼Ó
                  begin
         ...
         begin
            case iOptionIndex of
               ...
               107://¹ÎÃ¸
                  begin
                     iBaseValue := pstd.Agility;
                     iUpgradeValue := pu.btValue[16];                 //½Å¹ß ¹ÎÃ¸
                     Result := iBaseValue + iUpgradeValue;
                  end;
               108://°ø¼Ó
                  begin
         ...
         begin
            case iOptionIndex of
               ...
               106://Á¤È®
                  begin
                     iBaseValue := pstd.Accurate;
                     iUpgradeValue := pu.btValue[15];                        //º§Æ® Á¤È®
                     Result := iBaseValue + iUpgradeValue;
                  end;
               107://¹ÎÃ¸
                  begin
                     iBaseValue := pstd.Agility;
                     iUpgradeValue := pu.btValue[16];                       //º§Æ® ¹ÎÃ¸
                     Result := iBaseValue + iUpgradeValue;
                  end;
               108://°ø¼Ó
                  begin
               ...
               112://ÁßÀúÇ×
                  begin
                     iBaseValue := pstd.ToxAvoid;
                     iUpgradeValue := pu.btValue[8];        //º§Æ® ÁßÀúÇ×
                     Result := iBaseValue + iUpgradeValue;
                  end;
            end;//case iOptionIndex of
         end;
...
// added by sonmg.
function THeroObject.DoUpgradeItem( puSeed: PTUserItem; psSeed, psJewelry: TItem ): integer;  //½ºÃÄ
begin
   Result := 1;
   ...
   case psSeed.StdMode of
      5, 6, 99, 90: // ¹«±â
         begin
            ...
            puSeed.btValue[8] := puSeed.btValue[8] + psJewelry.Slowdown;   //¹«±â µÐÈ­
            puSeed.btValue[9] := puSeed.btValue[9] + psJewelry.Tox;        //¹«±â Áßµ¶
         end;
      10, 11, 12: // ¿Ê
         begin
      ...
      15: // Åõ±¸
         begin
            puSeed.btValue[3] := puSeed.btValue[3] + psJewelry.AC;
            puSeed.btValue[4] := puSeed.btValue[4] + psJewelry.MAC;
            puSeed.btValue[8] := puSeed.btValue[8] + psJewelry.Accurate;     //Åõ±¸ Á¤È®
            puSeed.btValue[9] := puSeed.btValue[9] + psJewelry.ToxAvoid;     //Åõ±¸ Áßµ¶ÀúÇ×
            puSeed.btValue[10] := puSeed.btValue[10] + psJewelry.MagAvoid;   //Åõ±¸ ¸¶¹ýÀúÇ×
         end;
      19,20,21: // ¸ñ°ÉÀÌ19           //¸ñ°ÉÀÌ19
         begin
            puSeed.btValue[0] := puSeed.btValue[0] + psJewelry.DC;
      ...
      22,23: // ¹ÝÁö    //¹ÝÁö22     //¹ÝÁö23
         begin
            ...
            if psJewelry.AtkSpd > 0 then begin
               ...
               // °ø¼Ó ÇÑ°è¸¦ ³ÑÀ» °æ¿ì ÇÑ°è°ªÀ» ´ëÀÔÇÔ.
               puSeed.btValue[16] := _MIN(15, puSeed.btValue[16]);            //¹ÝÁö °ø¼Ó
            end;
            puSeed.btValue[14] := puSeed.btValue[14] + psJewelry.Slowdown;    //¹ÝÁö µÐÈ­
            puSeed.btValue[9] := puSeed.btValue[9] + psJewelry.Tox;          //¹ÝÁö Áßµ¶
         end;
      24,26: // ÆÈÂî26      //ÆÈÂî26
         begin
            puSeed.btValue[0] := puSeed.btValue[0] + psJewelry.DC;
      ...
      64: // º§Æ®
         begin
            ...
            puSeed.btValue[8] := puSeed.btValue[8] + psJewelry.ToxAvoid;   //º§Æ® Áßµ¶ÀúÇ×
         end;
      else
         begin
            Result := 0;
...
begin
   ...
   if psSeed <> nil then begin
      ...
      if repair > 0 then begin
         ...
         SysMsg ('''' + psSeed.Name + '''' + ' ÐÞÀíÍê±Ï¡£', c_Green,t_Hint);
         Result := TRUE;
      end else begin
         SysMsg ('´Ë×°±¸²»ÐèÒªÐÞÀí¡£', c_Red,t_Hint);
      end;
   end;
end;
...
begin
   ...
   if psSeed <> nil then begin
      ...
      repair := _MIN(65000, _MAX(0, puSeed.DuraMax - puSeed.Dura));   //DURAMAX¼öÁ¤
      if repair > 0 then begin
         puSeed.Dura := _MIN(puSeed.Dura + repair, puSeed.DuraMax);
         if m_btRaceServer = RC_HEROOBJECT then begin
            hum := THeroObject(self);
...
/////////////////////////
// ±âÁ¸ ¼Ó¼º°ªÀÇ ÇÕ.
function THeroObject.SumOfOptions(puSeedItem: PTUserItem; psSeedItem: TItem) : integer;            //½ºÃÄ
begin
   Result := 0;
   case psSeedItem.StdMode of
      5, 6, 99, 90:  // ¹«±â           ÆÄ±« ¸¶¹ý µµ·Â °ø¼Ó Áßµ¶ µÐÈ­  (Á¤È® µµ Æ÷ÇÔ)
         begin
      ...
      10, 11, 12: // ¿Ê     ¹æ¾î ¸¶Ç× Áßµ¶ÀúÇ× ¸¶¹ýÀúÇ× ¹ÎÃ¸
         begin
            Result := puSeedItem.btValue[3] + puSeedItem.btValue[4]
                     + puSeedItem.btValue[6] + puSeedItem.btValue[7] + puSeedItem.btValue[8];       //¿Ê ¹ÎÃ¸
         end;
      15:  //Åõ±¸
         begin
            Result := puSeedItem.btValue[3] + puSeedItem.btValue[4]
                     + puSeedItem.btValue[8] + puSeedItem.btValue[9] + puSeedItem.btValue[10];      //Åõ±¸ Á¤È®
         end;
      ...
      64: // º§Æ®
         begin
            Result := puSeedItem.btValue[3] + puSeedItem.btValue[4] + puSeedItem.btValue[15]
                     + puSeedItem.btValue[16] + puSeedItem.btValue[8];          //º§Æ® ÁßÀúÇ×
         end;
   end;
...
procedure THeroObject.SendDelItemWithFlag (UserItem: pTUserItem; wBreakdown: Word);       //½ºÃÄ
var
  StdItem:TItem;
  ClientItem:TClientItem;
  opt: integer;
...
function THeroObject.GemItem(Gem:Integer;Itemidx:integer):integer;       //º¸¿Á(±¸ ½Ã½ºÅÛ)
var
...
begin
  ...
  if (Gem = Itemidx) or (Gem = 0) or (Itemidx = 0) then begin
    SendMsg(Self,RM_MENU_OK,0,Integer(self),0,0,'²»ÄÜÇ¿»¯ËùÑ¡µÄÎïÆ·¡£');
...
{check if the gem used is allowed on the item we using it on}
  if (UItem.Unique) or (UserItem.btValue[19] = 1) or (UserItem.btValue[19] > 2) then begin      //´ë¿©ÅÛ ºÀÀÎ º¸¿Á x
    if (UItem.Unique) then
      SysMsg('ÌØÊâÎïÆ·ÎÞ·¨Éý¼¶¡£', c_Red,t_Hint);
    if (UserItem.btValue[19] = 1) then
      SysMsg('×âÁÞµÄÎïÆ·ÎÞ·¨Éý¼¶¡£', c_Red,t_Hint);
    if (UserItem.btValue[19] > 2) then
      SysMsg('·âÓ¡µÄµ¥Æ·²»ÄÜÉý¼¶¡£', c_Red,t_Hint);
    exit; //cant be that the gem is also the item we using so :p
  end;
  stattype:= GItem.AC;
  case GItem.AC of
    0,1,2: begin // dc,mc,sc
      if not (UItem.StdMode in [5,6,90,19,20,21,22,23,24,26,99]) then begin
        SysMsg('´ËÎïÆ·²»ÄÜÉý¼¶¸ÃÏîÊôÐÔ¡£', c_Red,t_Hint);
        exit; //cant be that the gem is also the item we using so :p
      end;
    end;
    3,4: begin  //ac, amc
...
{now add the stat to the item}
  if Succes then begin //if the gem worked
    case stattype of
      0,1,2,3,4,9: begin //ÆÄ±« ¸¶¹ý µµ·Â ¹æ¾î ¸¶¹ý¹æ¾î Áßµ¶
      ...
      5: begin //°ø¼Ó
        if UItem.StdMode in [5,6,99,90] then begin//¹«±â btvalue 6 »ç¿ë ±×¸®°í  °ø¼Ó +/- ½Ã½ºÅÛ (°ª 10 > +, °ª 10 > -)
        ...
        if UItem.StdMode in [19,20,21] then begin //¸ñ°ÉÀÌ
      ...
      6: begin //È¸ÇÇ
        if UItem.StdMode in [10,11,12] then begin //¿Ê
          if Random(40) <= SuccesRate - UserItem.btValue[StatType] then begin
            BreakItem:=False;
            Succes:=FALSE;
          end else
      ...
      8: begin //Áßµ¶ÀúÇ×
        if UItem.StdMode in [64] then begin //º§Æ®
          if Random(40) <= SuccesRate - UserItem.btValue[StatType] then begin
            BreakItem:=False;
            Succes:=FALSE;
          end else
      ...
      12: begin //¸¶¹ýÀúÇ×
        ...
        if UItem.StdMode in [15] then begin //Åõ±¸
      ...
      14: begin //È¤ÇÑ µÐÈ­
        if UItem.StdMode in [5,6,90,99] then begin //¹«±â
      ...
      15: begin //Á¤È®
        if UItem.StdMode in [19,20,21,24,26,64] then begin //¸ñ°ÉÀÌ ÆÈÂî Çã¸®¶ì
          if Random(40) <= SuccesRate - UserItem.btValue[StatType] then begin
            BreakItem:=False;
            Succes:=FALSE;
          end else
...
{take away the used up orb + if a gem the failed item}
 { for i := 0 to m_ItemList.Count - 1 do begin //taking away our gem
  ...
  if (not Succes) and (BreakItem) then begin //if the orbing/geming failed and the item should break then take it away
    ...
    SendDefMessage (SM_UPGRADEITEM_RESULT, UserItem.MakeIndex , 2, 0, 0, UItem.Name);    //º¸¿Á
    Dispose(UserItem);
    ItmeNames := FilterStdShowName2(UItem, UItem.Name);
    SysMsg('ÎïÆ·('+ItmeNames+')ÒÑÆÆËé¡£', c_Red, t_Hint);
  ...
  if succes then begin
    ...
    ItmeNames := FilterStdShowName2(UItem, UItem.Name);
    case GItem.AC of
      0: SysMsg(''+ItmeNames+'·¢³öÒ»µÀè­è²µÄÁÁ¹â£¬¹¥»÷µÃµ½ÁË'+ IntToStr(GItem.MAC2) + 'µãÌáÉý¡£', c_Blue,t_Hint);
      1: SysMsg(''+ItmeNames+'·¢³öÒ»µÀè­è²µÄÁÁ¹â£¬Ä§·¨µÃµ½ÁË'+ IntToStr(GItem.MAC2) +'µãÌáÉý¡£', c_Blue,t_Hint);
      2: SysMsg(''+ItmeNames+'·¢³öÒ»µÀè­è²µÄÁÁ¹â£¬µÀÊõµÃµ½ÁË'+ IntToStr(GItem.MAC2) +'µãÌáÉý¡£', c_Blue,t_Hint);
      3: SysMsg(''+ItmeNames+'·¢³öÒ»µÀè­è²µÄÁÁ¹â£¬·ÀÓùµÃµ½ÁË'+ IntToStr(GItem.MAC2) +'µãÌáÉý¡£', c_Blue,t_Hint);
      4: SysMsg(''+ItmeNames+'·¢³öÒ»µÀè­è²µÄÁÁ¹â£¬Ä§·¨·ÀÓùµÃµ½ÁË'+ IntToStr(GItem.MAC2) +'µãÌáÉý¡£', c_Blue,t_Hint);
      5: SysMsg(''+ItmeNames+'·¢³öÒ»µÀè­è²µÄÁÁ¹â£¬¹¥»÷ËÙ¶ÈµÃµ½ÁË'+ IntToStr(GItem.MAC2) +'µãÌáÉý¡£', c_Blue,t_Hint);
      6: SysMsg(''+ItmeNames+'·¢³öÒ»µÀè­è²µÄÁÁ¹â£¬Ãô½ÝµÃµ½ÁË'+ IntToStr(GItem.MAC2) +'µãÌáÉý¡£', c_Blue,t_Hint);
      7: SysMsg(''+ItmeNames+'·¢³öÒ»µÀè­è²µÄÁÁ¹â£¬³Ö¾ÃµÃµ½ÁË'+ IntToStr(GItem.MAC2) +'µãÌáÉý¡£', c_Blue,t_Hint);
      8: SysMsg(''+ItmeNames+'·¢³öÒ»µÀè­è²µÄÁÁ¹â£¬¶¾Îï¶ã±ÜµÃµ½ÁË'+ IntToStr(GItem.MAC2) +'µãÌáÉý¡£', c_Blue,t_Hint);
      9: SysMsg(''+ItmeNames+'·¢³öÒ»µÀè­è²µÄÁÁ¹â£¬ÖÐ¶¾µÃµ½ÁË'+ IntToStr(GItem.MAC2) +'µãÌáÉý¡£', c_Blue,t_Hint);
     12: SysMsg(''+ItmeNames+'·¢³öÒ»µÀè­è²µÄÁÁ¹â£¬Ä§·¨¶ã±ÜµÃµ½ÁË'+ IntToStr(GItem.MAC2) +'µãÌáÉý¡£', c_Blue,t_Hint);
     14: SysMsg(''+ItmeNames+'·¢³öÒ»µÀè­è²µÄÁÁ¹â£¬¼õËÙµÃµ½ÁË'+ IntToStr(GItem.MAC2) +'µãÌáÉý¡£', c_Blue,t_Hint);
     15: SysMsg(''+ItmeNames+'·¢³öÒ»µÀè­è²µÄÁÁ¹â£¬×¼È·µÃµ½ÁË'+ IntToStr(GItem.MAC2) +'µãÌáÉý¡£', c_Blue,t_Hint);
    end;
    ...
    ItmeNames := FilterStdShowName2(UItem, UItem.Name);
    SysMsg(''+ItmeNames+'Ã»ÓÐ·¢ÉúÈÎºÎ±ä»¯¡£', c_Green,t_Hint);
    SendDefMessage (SM_UPGRADEITEM_RESULT, UserItem.MakeIndex , 2, 0, 0, UItem.Name);    //º¸¿Á
...
procedure THeroObject.GemItemFail(UserItem:pTUserItem;msg:String);         //º¸¿Á(±¸ ½Ã½ºÅÛ)
begin
