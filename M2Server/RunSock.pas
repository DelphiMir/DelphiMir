...
interface
uses
  {svn, }Windows, Classes, SysUtils, StrUtils, SyncObjs, JSocket, ObjBase, Grobal2,SDK, FrnEngn,UsrEngn;
type
...
var
  ...
  Gate:pTGateInfo;
ResourceString
  sGateOpen = '游戏网关[%d](%s:%d)已开启...';
  sKickGate = '游戏网关已断开连接: %s';
begin
    ...
    if GateUser.sAccount = '' then begin
      if TagCount(sMsg,'!') > 0 then begin
        ...
        if GetCertification(sMsg,sAccount,sChrName,nSessionID,nClientVersion,boFlag) then begin
          ...
          SessInfo:=FrmIDSoc.GetAdmission(sChrName,sAccount,GateUser.sIPaddr,nSessionID,nPayMode,nPayMent);
        ...
        for II := 0 to Gate.UserList.Count - 1 do begin
          ...
          if (GateUserInfo.sAccount = sAccount) or (GateUserInfo.nSessionID = nSessionID) then begin
            ...
            if GateUserInfo.PlayObject <> nil then begin
              nCheckCode:=9;
              TPlayObject(GateUserInfo.PlayObject).SendMsg(TPlayObject(GateUserInfo.PlayObject),RM_SIMPLE_OK,0,Integer(Self),0,0,sKickUserMsg);
...
begin
  ...
  //SVNRevision('$Id: RunSock.pas 500 2006-11-01 20:40:19Z damian $');
end;
finalization
begin
