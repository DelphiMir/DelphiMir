...
uses
  ...
  const
    DRAGON_MAX_LEVEL    =13;
    DRAGON_RESETTIME    = 15 * 60 * 1000; // ×î´ó 15·ÖÖÓÄÚ ²»¸´Î».
    MAP_ATTACK_TIME     = 10 * 1000 ;   // ¾¬µÄ ¶ÔÈËÃÇÀ´Ëµ ¹¥»÷µÄ Ê±¼ä
  ...
  type
     ...
     TDragonSystem = class(TObject)
      ...
      public
        ...
        procedure   OnAttackTarget( Envir_ : TEnvirnoment ;user_ : TBaseObject ; Mode_ :integer);
...
begin
   ...
   FLastAttackTme      := GetTickCount; // ×îºó µØÍ¼ÉÏ ×Ô¶¯¹¥»÷µÄ Ê±¼ä
...
begin
    ...
    IsSuccess := true;
    Result := 'ÁúÏµÍ³¼ÓÔØ³É¹¦ µÈ¼¶:'+IntToStr(LevelCount) +' ,°æÇèÄ¡Ä«¿îÆ®:'+IntToStr(ExpCount) +' ,¾ÆÀÌÅÛ' + IntToStr(ItemCount)+
              ' µØÍ¼: '+FDropMapName+' ÇøÓò: '+
...
procedure TDragonSystem.OnAttackTarget( Envir_ : TEnvirnoment ;user_ : TBaseObject ; Mode_ : integer );
var
    pwr , dam : integer;
begin
    if ( not user_.m_boDeath ) and
       ( not user_.m_boGhost ) and
       ( not user_.m_boAdminMode ) and
       ( not user_.m_boObMode ) and
       ( not user_.m_boPenetrateMode ) and
       ( not user_.m_btRaceServer = RC_PET)
    then
    begin

      // ÀÌÆåÆ® ³¯·ÁÁÖ°í...
      case Mode_ of
       1: user_.SendRefMsg (RM_10205, 0, user_.m_nCurrX, user_.m_nCurrY, NE_THUNDER, '0');
       2: user_.SendRefMsg (RM_10205, 0, user_.m_nCurrX, user_.m_nCurrY, NE_FIRE, '0');
      end;

      // µ¥¹ÌÁö °è»êÈÄ ³»·ÁÁØ´Ù.
      pwr := 20 * (random(3)+1);
      dam := user_.GetMagStruckDamage (nil, pwr);
      user_.StruckDamage (dam);
      user_.SendDelayMsg (TBaseObject(RM_STRUCK), RM_10101, dam{wparam},
      user_.m_WAbil.HP{lparam1}, user_.m_WAbil.MaxHP{lparam2}, Longint(nil){hiter}, '', 200);
...
begin
    ...
    begin
        Tempuser := TBaseObject( UserList[i]);

        // »ç¶÷¸¸ °ø°ÝÇÑ´Ù.
        if TempUser.m_btRaceServer = RC_PLAYOBJECT then
        begin
           //·£´ý°ª¿¡ °É¸®¸é °ø°ÝÇÏÀÚ
           if  random( 2 ) = 0 then
           begin
...
begin
    ...
    FDopItemEnvir := g_MapManager.FindMap( FDropMapName );
    FDropItemRect := Area_;
    ...
    begin
      ...
      begin
          ...
          begin
            ...
            if FEXP >= GetNextLevelExp then
            begin
                //Éý¼¶ ÐÐÁË µÀ¾ß ÈÃËûÀë¿ª°É.(Î»ÖÃÐÞÕý sonmg 2006/01/27)
                OnLevelup(FLEVEL);

                FLEVEL := FLEVEL + 1;
                FEXP  := 0;

                MainOutMessage( 'ÆÆÌìÄ§Áú(ÎïÆ·ÒÑµôÂä)µÈ¼¶ÌáÉýÎª:'+IntToStr( FLEVEL ) + '¼¶' );
...
begin
    ...
    begin
      FLevel := 1;
      FExp   := 0;
      MainOutMessage('ÆÆÌìÄ§Áú ÖØÖÃµÈ¼¶');
    end;
end;
