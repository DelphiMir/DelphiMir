...
begin
  if (ListBoxActiveList.ItemIndex >=0) and (ListBoxActiveList.ItemIndex < ListBoxActiveList.Items.Count) then begin
    if Application.MessageBox(PChar('해당 IP를 접속차단 하겠습니까?'),
                              PChar('차단 - ' + ListBoxActiveList.Items.Strings[ListBoxActiveList.ItemIndex]),MB_OKCANCEL + MB_ICONQUESTION) <> IDOK then exit;
...
begin
  if (ListBoxActiveList.ItemIndex >=0) and (ListBoxActiveList.ItemIndex < ListBoxActiveList.Items.Count) then begin
    ...
    if Application.MessageBox(PChar('해당 IP를 차단(1) 목록에 추가시키겠습니까?'),
                              PChar('등록 - ' + ListBoxActiveList.Items.Strings[ListBoxActiveList.ItemIndex]),
...
begin
  if (ListBoxActiveList.ItemIndex >=0) and (ListBoxActiveList.ItemIndex < ListBoxActiveList.Items.Count) then begin
    sIPaddr:=ListBoxActiveList.Items.Strings[ListBoxActiveList.ItemIndex];
    if Application.MessageBox(PChar('해당 IP를 차단(2) 목록에 추가시키겠습니까?'),
                              PChar('등록 - ' + ListBoxActiveList.Items.Strings[ListBoxActiveList.ItemIndex]),
                              MB_OKCANCEL + MB_ICONQUESTION
                              ) <> IDOK then exit;
...
begin
  sIPaddress:='';
  if not InputQuery('IP 입력','차단할 IP입력',sIPaddress) then exit;
  if not IsIPaddr(sIPaddress) then begin
    Application.MessageBox('이미 존재합니다.','오류',MB_OK + MB_ICONERROR);
    exit;
  end;
...
begin
  ...
  if not InputQuery('IP 입력','차단할 IP입력',sIPaddress) then exit;
  if not IsIPaddr(sIPaddress) then begin
    if Application.MessageBox('이미 존재합니다.','오류',MB_YESNO + MB_ICONQUESTION) <> ID_YES then
    exit;
  end;
...
initialization 	 
  SVNRevision('$Id: IPaddrFilter.pas 318 2006-08-24 02:18:15Z Dataforce $');
end.
