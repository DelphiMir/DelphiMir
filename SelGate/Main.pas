...
begin
  ...
  if IsBlockIP(sRemoteIPaddr) then begin
    MainOutMessage('차단IP 연결 시도: ' + sRemoteIPaddr,1);
    Socket.Close;
    exit;
  end;
  ...
  if IsConnLimited(sRemoteIPaddr) then begin
    ...
    MainOutMessage('허용갯수 초과 IP접속시도 : ' + sRemoteIPaddr,1);
    exit;
  end;
  ...
  if boGateReady then begin
    ...
    if Socket.nIndex >= 0 then begin
      ClientSocket.Socket.SendText('%O' +
                                   ...
                                   sLocalIPaddr +
                                   '$');
      MainOutMessage('접속: ' + sRemoteIPaddr,5);
    end else begin
      Socket.Close;
      MainOutMessage('연결 끊기: ' + sRemoteIPaddr,1);
    end;
  ...
  if (nSockIndex >= 0) and (nSockIndex < GATEMAXSESSION) then begin
    ...
    if boGateReady then begin
      ClientSocket.Socket.SendText('%X' +
                                   IntToStr(Socket.SocketHandle) +
                                   '$');
      MainOutMessage('연결 끊기: ' + sRemoteIPaddr,5);
    end;
  end;
end;
...
begin
  ...
  if Application.MessageBox('종료 하시겠습니까?',
                            '종료',
...
begin
  ...
  if not boGateReady then begin
    StatusBar.Panels[1].Text:='준비';
  end else begin
    if boKeepAliveTimcOut then begin
      StatusBar.Panels[1].Text:='실패';
    end else begin
      StatusBar.Panels[1].Text:='열림';
    ...
    while (True) do begin
     ...
     for nSocketIndex:= 0 to GATEMAXSESSION - 1 do begin
       ...
       if (GetTickCount - g_SessionArray[nSocketIndex].dwConnctCheckTick) > dwKeepConnectTimeOut then begin
         ...
         MainOutMessage('접속시간 초과/차단: ' + sRemoteIPaddr,1);
         Continue;
       end;
...
begin
  ...
  frmMain.Left := Conf.ReadInteger('SelectGate','Left',(Screen.Width Div 2)-(frmMain.Width Div 2));
  frmMain.Top := Conf.ReadInteger('SelectGate','Top',(Screen.Height Div 2)-(frmMain.Height Div 2)-30);
  Conf.Free;
...
begin
try
    MainOutMessage('서비스 시작...',3);
    SendGameCenterMsg(SG_FORMHANDLE,IntToStr(Self.Handle));
    SendGameCenterMsg(SG_STARTNOW, '셀게이트 시작중');
    boServiceStart:=True;
    boGateReady:=False;
    boServerReady:=False;    
    nSessionCount:=0;
    ...
    MainOutMessage('소켓 오픈...',3);
    SendGameCenterMsg(SG_STARTOK, '셀게이트 시작완료');
  except
...
begin
  if Application.MessageBox('중지하시겠습니까?',
                            '중지',
...
begin
  if Application.MessageBox('로그를 삭제하시겠습니까?',
                            '로그삭제',
...
begin
  ...
  ini.WriteInteger('SelectGate','Left',frmMain.Left);
  ini.WriteInteger('SelectGate','Top',frmMain.Top);
  ini.UpdateFile;
  ini.Free; 
end;
...
initialization
  SVNRevision('$Id: Main.pas 410 2006-09-11 00:08:44Z Fugly $');
end.
